---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for  deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review,  cron scheduling for periodic checks, or version status checks on a machine running  (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running , then align it to a user-defined risk tolerance without breaking access. Use  security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim  changes the host firewall, SSH, or OS updates; it does not.
...
5.  gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if  can access sensitive data.
...
As part of the default read-only checks, run ` security audit --deep`. Only offer alternatives if the user requests them:

1. ` security audit` (faster, non-probing)
2. ` security audit --json` (structured output)

Offer to apply  safe defaults (numbered):

1. ` security audit --fix`

Be explicit that `--fix` only tightens  defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check  version/update status (read-only)

As part of the default read-only checks, run ` update status`.
...
- Credential hygiene notes (location of  creds, prefer disk encryption)
...
After  install or first hardening pass, run at least one baseline audit and version check:

- ` security audit`
- ` security audit --deep`
- ` update status`

Ongoing monitoring is recommended. Use the  cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. â€œDo you want me to schedule periodic audits (e.g., daily/weekly) via ` cron add`?â€

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule ` update status`
...
Before creating, ` cron list` and match on exact `name`. If found, ` cron edit <id> ...`.
If not found, ` cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. ` update status` (preferred for source checkouts and channels)
2. `npm view  version` (published npm version)
...
- ` health --json`
- ` update status`
- ` cron add|list|runs|run`

Do not invent CLI flags or imply  enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by  compaction:
