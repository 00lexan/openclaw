...
metadata:
  { "": { "emoji": "ðŸ§µ", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
...
- Use `_TMUX_SOCKET_DIR` (legacy `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$_TMUX_SOCKET_DIR/.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `_TMUX_SOCKET_DIR`).
...
- Check for shell prompt (`â¯` or `$`) to detect completion
