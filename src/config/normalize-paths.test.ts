...
describe("normalizeConfigPaths", () => {
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".", "logs", ".log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".", "hooks.json5"));
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".", "agents", "main"));
