...
import type { Config, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function coerceConfig(value: unknown): Config {
...
function stampConfigVersion(cfg: Config): Config {
...
function warnIfConfigFromFuture(cfg: Config, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer  (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: Config, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): Config {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as Config, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as Config, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: Config) {
    clearConfigCache();
...
// module scope. `_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env._CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env._DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
