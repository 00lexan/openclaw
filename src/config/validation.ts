...
import type { Config, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: Config): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: Config } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as Config);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as Config);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as Config)),
    ),
  };
}
