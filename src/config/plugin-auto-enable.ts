...
function isTelegramConfigured(cfg: Config, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: Config, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: Config, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: Config): boolean {
...
function isIMessageConfigured(cfg: Config): boolean {
...
function isWhatsAppConfigured(cfg: Config): boolean {
...
function isGenericChannelConfigured(cfg: Config, channelId: string): boolean {
...
function collectModelRefs(cfg: Config): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: Config, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: Config, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: Config, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: Config, pluginId: string): Config {
...
function enablePluginEntry(cfg: Config, pluginId: string): Config {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
