...
This directory contains hooks that ship with . These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs..ai/hooks/soul-evil
...
Hooks can be configured in `~/./.json`:
...
2. Restart gateway: `pkill -9 -f '.*gateway' && pnpm  gateway`
3. Enable the hook: ` hooks enable my-hook`
