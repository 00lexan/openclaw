...
homepage: https://docs..ai/hooks#command-logger
metadata:
  {
    "":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/./logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/./logs/commands.log ~/./logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/`:
   ```
   /home/username/./logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
