...
`;
    ...
    expect(result.metadata).toBe('{"": {"events": ["test"]}}');
  });
...
describe("resolveMetadata", () => {
  it("extracts  metadata from parsed frontmatter", () => {
    ...
    const result = resolveMetadata(frontmatter);
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        : {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with " },
            { id: "npm", kind: "npm", package: "@/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@/hook");
  });
...
`;
    ...
    const  = resolveMetadata(frontmatter);
    expect().toBeDefined();
