...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env._WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env._PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env._TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env._TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
