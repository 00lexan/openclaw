...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env._STATE_DIR;
  const configPath = env._CONFIG_PATH;
  return {
    HOME: env.HOME,
    ...
    _GATEWAY_PORT: String(port),
    _GATEWAY_TOKEN: token,
    _LAUNCHD_LABEL: resolvedLaunchdLabel,
    _SYSTEMD_UNIT: systemdUnit,
    _SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    _SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    _STATE_DIR: stateDir,
    _CONFIG_PATH: configPath,
    _LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    _SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    _WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    _TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    _LOG_PREFIX: "node",
    _SERVICE_MARKER: NODE_SERVICE_MARKER,
    _SERVICE_KIND: NODE_SERVICE_KIND,
    _SERVICE_VERSION: VERSION,
  };
}
