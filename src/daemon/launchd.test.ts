...
async function withLaunchctlStub(
  ...
  const originalLogPath = process.env._TEST_LAUNCHCTL_LOG;
  const originalListOutput = process.env._TEST_LAUNCHCTL_LIST_OUTPUT;

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "-launchctl-test-"));
  try {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const logPath = process.env._TEST_LAUNCHCTL_LOG;",
        "if (logPath) {",
        '  fs.appendFileSync(logPath, JSON.stringify(args) + "\\n", "utf8");',
        "}",
        'if (args[0] === "list") {',
        '  const output = process.env._TEST_LAUNCHCTL_LIST_OUTPUT || "";',
    ...
    process.env._TEST_LAUNCHCTL_LOG = logPath;
    process.env._TEST_LAUNCHCTL_LIST_OUTPUT = options.listOutput ?? "";
    ...
    if (originalLogPath === undefined) {
      delete process.env._TEST_LAUNCHCTL_LOG;
    } else {
      process.env._TEST_LAUNCHCTL_LOG = originalLogPath;
    }
    if (originalListOutput === undefined) {
      delete process.env._TEST_LAUNCHCTL_LIST_OUTPUT;
    } else {
      process.env._TEST_LAUNCHCTL_LIST_OUTPUT = originalListOutput;
    }
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    await withLaunchctlStub({ listOutput: "123 0 ai..gateway\n" }, async ({ env }) => {
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when _PROFILE is default", () => {
    const env = { HOME: "/Users/test", _PROFILE: "default" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai..gateway.plist",
    );
  });

  it("uses default label when _PROFILE is unset", () => {
  ...
  it("uses profile-specific label when _PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", _PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai..jbphoenix.plist",
    );
  });

  it("prefers _LAUNCHD_LABEL over _PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      _PROFILE: "jbphoenix",
      _LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from _LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      _LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty _LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      _PROFILE: "myprofile",
      _LAUNCHD_LABEL: "   ",
    ...
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai..myprofile.plist",
    );
  });
  ...
  it("trims whitespace from _PROFILE", () => {
    const env = { HOME: "/Users/test", _PROFILE: "  myprofile  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai..myprofile.plist",
    );
  });
