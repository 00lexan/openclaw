...
const EXTRA_MARKERS = ["", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["_service_marker"];
  const kindKeys = ["_service_kind"];
...
function isGatewayTaskName(name: string): boolean {
  ...
  return normalized === defaultName || normalized.startsWith(" gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "" && isGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "" && isGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
