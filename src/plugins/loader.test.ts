...
const prevBundledDir = process.env._BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `-plugin-${randomUUID()}`);
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env._BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env._BUNDLED_PLUGINS_DIR = bundledDir;
    ...
    const enabledRegistry = loadPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
