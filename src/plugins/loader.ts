...
function resolvePluginModuleExport(moduleExport: unknown): {
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as PluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as PluginDefinition;
...
export function loadPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = jiti(candidate.source) as PluginModule;
    } catch (err) {
