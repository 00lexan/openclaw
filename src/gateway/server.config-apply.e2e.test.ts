...
beforeAll(async () => {
  previousToken = process.env._GATEWAY_TOKEN;
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env._GATEWAY_TOKEN = previousToken;
  }
});
    ...
    try {
      ...
      ws.send(
        JSON.stringify({
          ...
          method: "config.apply",
          params: {
            raw: '{ "agents": { "list": [{ "id": "main", "workspace": "~/" }] } }',
      ...
      const sentinelPath = path.join(os.homedir(), ".", "restart-sentinel.json");
