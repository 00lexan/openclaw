...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env._GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env._GATEWAY_PORT = String(gatewayPort);
  process.env._GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env._GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
