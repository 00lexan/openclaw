...
import type { Config, ModelProviderConfig } from "../config/types.js";
import { resolveAgentDir } from "../agents/agent-paths.js";
...
import { ensureModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env._LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env._LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env._LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<Config["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env._AGENT_DIR,
  ...
  const hostAgentDir = resolveAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "-live-state-"));
  ...
  process.env._AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveAgentDir();
  const sanitizedCfg: Config = {
    ...params.cfg,
  ...
  try {
    ...
    process.env._AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env._STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env._LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
