...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env._STATE_DIR;
  previousConfigPath = process.env._CONFIG_PATH;
  previousSkipBrowserControl = process.env._SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env._SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env._SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env._SKIP_BROWSER_CONTROL_SERVER = "1";
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "-test-"))
    : path.join(tempHome, ".-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env._CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env._SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env._SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env._SKIP_GMAIL_WATCHER;
    } else {
      process.env._SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env._SKIP_CANVAS_HOST;
    } else {
      process.env._SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env._GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env._GATEWAY_TOKEN = fallbackToken;
  }
