...
export function applyCliProfileEnv(params: {
  ...
  const stateDir = env._STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env._STATE_DIR?.trim()) {
    env._STATE_DIR = stateDir;
  }

  if (!env._CONFIG_PATH?.trim()) {
    env._CONFIG_PATH = path.join(stateDir, ".json");
  }

  if (profile === "dev" && !env._GATEWAY_PORT?.trim()) {
    env._GATEWAY_PORT = "19001";
  }
}
