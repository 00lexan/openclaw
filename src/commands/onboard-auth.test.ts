...
const requireAgentDir = () => {
  const agentDir = process.env._AGENT_DIR;
  if (!agentDir) {
    throw new Error("_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under _AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "-oauth-"));
    process.env._STATE_DIR = tempStateDir;
    process.env._AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env._AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to _AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "-minimax-"));
    process.env._STATE_DIR = tempStateDir;
    process.env._AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env._AGENT_DIR;
