...
const SEARCH_TOOL = "https://docs..ai/mcp.Search";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    ...
    if (isRich()) {
      ...
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand(' docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs..ai/");
      runtime.log(`Search: ${formatCliCommand(' docs "your query"')}`);
    }
    return;
  }
