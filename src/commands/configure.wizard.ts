...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? " update wizard" : " configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: Config = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand(" doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env._GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env._GATEWAY_PASSWORD,
    });
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? process.env._GATEWAY_TOKEN;
        const password =
          nextConfig.gateway?.auth?.password ?? process.env._GATEWAY_PASSWORD;
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env._GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env._GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env._GATEWAY_TOKEN;
