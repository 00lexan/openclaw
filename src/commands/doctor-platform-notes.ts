...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: Config): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["_GATEWAY_TOKEN", await getenv("_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["_GATEWAY_PASSWORD", await getenv("_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
