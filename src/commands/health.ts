...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env._DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: Config },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env._DEBUG_HEALTH);
    if (opts.verbose) {
