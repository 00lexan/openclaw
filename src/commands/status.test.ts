...
beforeAll(() => {
  previousProfile = process.env._PROFILE;
  process.env._PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env._PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/-root.js", () => ({
  resolvePackageRoot: vi.fn().mockResolvedValue("/tmp/"),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (l) =>
          l.includes(" status --all") ||
          l.includes(" --profile isolated status --all") ||
          l.includes(" status --all") ||
          l.includes(" --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env._GATEWAY_TOKEN = "abcd1234";
    try {
