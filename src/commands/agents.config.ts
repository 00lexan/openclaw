...
function resolveAgentName(cfg: Config, agentId: string) {
...
function resolveAgentModel(cfg: Config, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: Config): AgentSummary[] {
