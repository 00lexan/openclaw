...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    ...
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\\\src\\cli\\index.ts"])).toBe(
      true,
    );
...
describe("buildGatewayInstallPlan", () => {
  it("uses provided nodePath and returns plan", async () => {
    ...
    mocks.buildServiceEnvironment.mockReturnValue({ _PORT: "3000" });
    ...
    expect(plan.environment).toEqual({ _PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment._PORT).toBe("3000");
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:|)( --profile isolated)? gateway install/,
    );
  });
});
