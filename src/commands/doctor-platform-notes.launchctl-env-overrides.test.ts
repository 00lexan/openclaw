...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv _GATEWAY_TOKEN");
    expect(message).not.toContain("_GATEWAY_PASSWORD");
  });
