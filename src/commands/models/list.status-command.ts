...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand(" models auth setup-token")}\` or \`${formatCliCommand(" configure")}\`.`
          : `Run \`${formatCliCommand(" configure")}\` or set an API key env var.`;
