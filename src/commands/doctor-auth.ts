...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token â†’ ${formatCliCommand(" models auth setup-token")}`,
    );
  }
  if (deprecated.has(CODEX_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CODEX_CLI_PROFILE_ID} (OpenAI Codex): use OAuth â†’ ${formatCliCommand(
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(" models auth setup-token")} or ${formatCliCommand(
      " configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      " models auth login --provider openai-codex",
    )} or ${formatCliCommand(" configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand(" configure")}\` or \`${formatCliCommand(" onboard")}\`.`;
}
...
function formatAuthIssueLine(issue: AuthIssue): string {
  ...
  return `- ${issue.profileId}: ${issue.status}${remaining}${hint ? ` â€” ${hint}` : ""}`;
}
