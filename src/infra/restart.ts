...
export function triggerRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env._SYSTEMD_UNIT,
        process.env._PROFILE,
      );
  ...
  const label =
    process.env._LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env._PROFILE);
