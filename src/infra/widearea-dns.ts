...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env._WIDE_AREA_DOMAIN ?? null;
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  ...
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "-gw");
  ...
  records.push(`_-gw._tcp IN PTR ${instanceLabel}._-gw._tcp`);
  records.push(`${instanceLabel}._-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
