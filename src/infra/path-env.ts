...
function candidateBinDirs(opts: EnsurePathOpts): string[] {
  ...
  // Bundled macOS app: `` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "");
  ...
  if (isExecutable(path.join(localBinDir, ""))) {
 ...
 */
export function ensureCliOnPath(opts: EnsurePathOpts = {}) {
  if (isTruthyEnvValue(process.env._PATH_BOOTSTRAPPED)) {
    return;
  }
