...
import { ensureCliOnPath } from "./path-env.js";

describe("ensureCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling  exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "-path-"));
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const cliPath = path.join(appBinDir, "");
      await fs.writeFile(cliPath, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(cliPath, 0o755);

      const originalPath = process.env.PATH;
      const originalFlag = process.env._PATH_BOOTSTRAPPED;
      process.env.PATH = "/usr/bin";
      delete process.env._PATH_BOOTSTRAPPED;
      try {
        ensureCliOnPath({
          execPath: cliPath,
          cwd: tmp,
        ...
        if (originalFlag === undefined) {
          ...
          process.env._PATH_BOOTSTRAPPED = originalFlag;
        }
      }
    } finally {
  ...
  it("is idempotent", () => {
    ...
    process.env._PATH_BOOTSTRAPPED = "1";
    try {
      ensureCliOnPath({
