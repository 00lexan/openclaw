...
function resolveActiveHoursTimezone(cfg: Config, raw?: string): string {
...
function hasExplicitHeartbeatAgents(cfg: Config) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: Config, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: Config): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: Config, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: Config, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
