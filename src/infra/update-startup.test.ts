...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "-update-check-"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolvePackageRoot } = await import("./-root.js");
    ...
    vi.mocked(resolvePackageRoot).mockResolvedValue("/opt/");
