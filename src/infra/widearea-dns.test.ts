...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expect(txt).toContain(`_-gw._tcp IN PTR studio-london._-gw._tcp`);
    expect(txt).toContain(`studio-london._-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio ()`);
