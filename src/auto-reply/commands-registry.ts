...
export function isCommandEnabled(cfg: Config, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: Config): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: Config): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: Config) {
