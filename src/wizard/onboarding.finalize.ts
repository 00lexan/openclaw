...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/./.json (gateway.auth.token) or _GATEWAY_TOKEN.",
        "Web UI stores a copy in this browser's localStorage (.control.settings.v1).",
        `Get the tokenized link anytime: ${formatCliCommand(" dashboard --no-open")}`,
      ].join("\n"),
      "Token",
    );
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand(" dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    "Running agents on your computer is risky â€” harden your setup: https://docs..ai/security",
    "Security",
  );
  ...
  await prompter.note(
    hasWebSearchKey
      ...
      : [
          ...
          "",
          " uses Brave Search for the `web_search` tool. Without a Brave Search API key, web search wonâ€™t work.",
          "",
          "Set it up interactively:",
          `- Run: ${formatCliCommand(" configure --section web")}`,
  ...
  await prompter.note(
    'What now: https://.ai/showcase ("What People Are Building").',
    "What now",
  );
