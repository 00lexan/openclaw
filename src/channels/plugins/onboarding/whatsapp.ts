...
function setWhatsAppDmPolicy(cfg: Config, dmPolicy: DmPolicy): Config {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: Config, allowFrom?: string[]): Config {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: Config, selfChatMode: boolean): Config {
...
async function detectWhatsAppLinked(cfg: Config, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so  can allowlist you.",
      "WhatsApp number",
    );
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand(" channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
