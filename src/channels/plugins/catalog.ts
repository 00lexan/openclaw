...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, PackageManifest>>;
...
const ENV_CATALOG_PATHS = ["_PLUGIN_CATALOG_PATHS", "_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<PackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverPlugins({ workspaceDir: options.workspaceDir });
