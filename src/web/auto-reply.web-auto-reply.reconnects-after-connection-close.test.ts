...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "-web-home-"));
...
const makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "-session-"));
  ...
  it("processes inbound messages without batching and preserves timestamps", async () => {
    ...
    try {
      ...
      expect(firstArgs.Body).toMatch(
        new RegExp(`\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${firstPattern}\\] \\[\\] first`),
      );
      expect(firstArgs.Body).not.toContain("second");
      expect(secondArgs.Body).toMatch(
        new RegExp(`\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${secondPattern}\\] \\[\\] second`),
      );
