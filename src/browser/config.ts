import type { BrowserConfig, BrowserProfileConfig, Config } from "../config/config.js";
...
import {
  DEFAULT__BROWSER_COLOR,
  DEFAULT__BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT__BROWSER_PROFILE_NAME,
} from "./constants.js";
...
/**
 * Ensure the default "" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT__BROWSER_PROFILE_NAME]) {
    result[DEFAULT__BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an  browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for / CDP).
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT__BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "";
