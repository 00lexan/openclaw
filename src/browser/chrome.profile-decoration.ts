...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".-profile-decorated");
}
 ...
 */
export function decorateProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT__BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT__BROWSER_COLOR).toUpperCase();
