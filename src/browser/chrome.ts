...
export function resolveUserDataDir(profileName = DEFAULT__BROWSER_PROFILE_NAME) {
...
export async function launchChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT__BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`ðŸ¦ž  browser profile decorated (${profile.color})`);
    } catch (err) {
  ...
  log.info(
    `ðŸ¦ž  browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopChrome(running: RunningChrome, timeoutMs = 2500) {
