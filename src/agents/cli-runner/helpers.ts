...
/**
 * Cleanup suspended  CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: Config) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "-cli-images-"));
  const paths: string[] = [];
