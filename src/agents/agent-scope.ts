...
function listAgents(cfg: Config): AgentEntry[] {
...
export function listAgentIds(cfg: Config): string[] {
...
export function resolveSessionAgentIds(params: { sessionKey?: string; config?: Config }): {
...
export function resolveAgentModelPrimary(cfg: Config, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: Config, agentId: string) {
  const id = normalizeAgentId(agentId);
  ...
  return path.join(os.homedir(), ".", `workspace-${id}`);
}

export function resolveAgentDir(cfg: Config, agentId: string) {
  const id = normalizeAgentId(agentId);
