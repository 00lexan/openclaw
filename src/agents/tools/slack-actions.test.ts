...
describe("handleSlackAction", () => {
  it("adds reactions", async () => {
    const cfg = { channels: { slack: { botToken: "tok" } } } as Config;
    ...
    expect(reactSlackMessage).toHaveBeenCalledWith("C1", "123.456", "âœ…");
  });
  ...
  it("removes reactions when remove flag set", async () => {
    ...
    expect(removeSlackReaction).toHaveBeenCalledWith("C1", "123.456", "âœ…");
  });
