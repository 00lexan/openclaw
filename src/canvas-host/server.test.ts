...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("CanvasA2UIAction");
    expect(out).toContain("SendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("SendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/____/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      ...
      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/____/a2ui/a2ui.bundle.js`,
      );
