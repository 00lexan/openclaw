...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as App);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as App);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as App);
  }
  if (host.tab === "agents") {
    const app = host as unknown as App;
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as App);
    await loadDevices(host as unknown as App);
    await loadConfig(host as unknown as App);
    await loadExecApprovals(host as unknown as App);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as App);
    await loadConfig(host as unknown as App);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as App);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as App, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.___CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as App, false),
    loadPresence(host as unknown as App),
    loadSessions(host as unknown as App),
    loadCronStatus(host as unknown as App),
    loadDebug(host as unknown as App),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as App, true),
    loadConfigSchema(host as unknown as App),
    loadConfig(host as unknown as App),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as App, false),
    loadCronStatus(host as unknown as App),
    loadCronJobs(host as unknown as App),
  ]);
}
