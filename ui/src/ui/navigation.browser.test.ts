...
function mountApp(pathname: string) {
  ...
  const app = document.createElement("-app") as App;
...
beforeEach(() => {
  App.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.___CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps//cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps//cron");
  });

  it("honors explicit base path overrides", async () => {
    window.___CONTROL_UI_BASE_PATH__ = "/";
    const app = mountApp("//sessions");
    await app.updateComplete;
