...
import "@a2ui/lit/ui";
import { themeContext } from "@/a2ui-theme-context";
...
class A2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(---a2ui-inset-top, 0px)
        var(---a2ui-inset-right, 0px)
        var(---a2ui-inset-bottom, 0px)
        var(---a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(---a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(---a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(---a2ui-empty-top, var(---a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    for (const eventName of [":a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__LastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.CanvasA2UIAction ??
      globalThis.CanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.CanvasA2UIAction) {
...
if (!customElements.get("-a2ui-host")) {
