...
If the page fails to load, start the Gateway first: ` gateway`.
...
you revoke it with ` devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/./.json` (`config.get`, `config.set`)
...
```bash
 gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
 ...
 **blocks** Control UI connections without device identity.
...
```bash
_CONTROL_UI_BASE_PATH=// pnpm ui:build
```
