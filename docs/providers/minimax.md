...
```bash
 plugins enable minimax-portal-auth  # skip if already loaded.
 gateway restart  # restart if gateway is already running
...
See [MiniMax OAuth plugin README](https://github.com///tree/main/extensions/minimax-portal-auth) for details.
...
- Use ` models list` and ` models set minimax/MiniMax-M2.1` to switch.
...
- Running ` configure` and selecting **MiniMax M2.1**, or
