---
summary: "How  memory works (workspace files + automatic memory flush)"
read_when:
...
When a session is **close to auto-compaction**,  triggers a **silent,
...
## Vector memory search
 ...
 can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Uses remote embeddings by default. If `memorySearch.provider` is not set,  auto-selects:
...
BM25 + vectors + reranking. Markdown stays the source of truth;  shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/./agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/./agents/<agentId>/qmd/` (config + cache + sqlite DB).
- Collections are rewritten from `memory.qmd.paths` (plus default workspace
  ...
  on a configurable interval (`memory.qmd.update.interval`, default 5â€¯m).
- Searches run via `qmd query --json`. If QMD fails or the binary is missing,
   automatically falls back to the builtin SQLite manager so memory tools
  keep working.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  -  sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index 
    uses), run a one-off query with the agentâ€™s XDG dirs.

    â€™s QMD state lives under your **state dir** (defaults to `~/.`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
     uses:

    ```bash
    # Pick the same state dir  uses
    STATE_DIR="${_STATE_DIR:-$HOME/.}"
    if [ -d "$HOME/.moltbot" ] && [ ! -d "$HOME/." ] \
      && [ -z "${_STATE_DIR:-}" ]; then
...
- When `memory.qmd.sessions.enabled = true`,  exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/./agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- Index storage: per-agent SQLite at `~/./memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
- Freshness: watcher on `MEMORY.md` + `memory/` marks the index dirty (debounce 1.5s). Sync is scheduled on session start, on search, or on an interval and runs asynchronously. Session transcripts use delta thresholds to trigger background sync.
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change,  automatically resets and reindexes the entire store.
...
If full-text search is unavailable on your platform,  falls back to vector-only search.
...
### Embedding cache
 ...
 can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/./agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available,  stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load,  logs the
