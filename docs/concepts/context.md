...
read_when:
  - You want to understand what â€œcontextâ€ means in 
...
â€œContextâ€ is **everything  sends to the model for a run**. It is bounded by the modelâ€™s **context window** (token limit).

Beginner mental model:

- **System prompt** (-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How  builds the system prompt

The system prompt is **-owned** and rebuilt each run. It includes:
...
By default,  injects a fixed set of workspace files (if present):
