...
- `/usage cost` in chats: local cost summary aggregated from  session logs.
- CLI: ` status --usage` prints a full per-provider breakdown.
- CLI: ` channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
