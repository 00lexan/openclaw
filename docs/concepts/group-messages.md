...
Add a `groupChat` block to `~/./.json` so display-name pings work even when WhatsApp strips the visual `@` in the text body:
...
- The regexes are case-insensitive; they cover a display-name ping like `@` and the raw number with or without `+`/spaces.
...
1. Add your WhatsApp account (the one running ) to the group.
2. Say `@ â€¦` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Manual smoke:
  - Send an `@` ping in the group and confirm a reply that references the sender name.
