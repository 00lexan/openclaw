---
summary: "CLI reference for ` models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
` models status` shows the resolved default/fallbacks plus an auth overview.
...
the command uses `_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
` plugins list` to see which providers are installed.
