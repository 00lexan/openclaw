---
summary: " CLI reference for `` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.-<name>`.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for ` update` (source installs only).
...
## Color palette
 ...
 uses a lobster palette for CLI output.
...
Note: plugins can add additional top-level commands (for example ` voicecall`).

## Security

- ` security audit` â€” audit config + local state for common security foot-guns.
- ` security audit --deep` â€” best-effort live Gateway probe.
- ` security audit --fix` â€” tighten safe defaults and chmod state/config.

## Plugins

Manage extensions and their config:

- ` plugins list` â€” discover plugins (use `--json` for machine output).
- ` plugins info <id>` â€” show details for a plugin.
- ` plugins install <path|.tgz|npm-spec>` â€” install a plugin (or add a plugin path to `plugins.load.paths`).
- ` plugins enable <id>` / `disable <id>` â€” toggle `plugins.entries.<id>.enabled`.
- ` plugins doctor` â€” report plugin load errors.
...
- ` memory status` â€” show index stats.
- ` memory index` â€” reindex memory files.
- ` memory search "<query>"` â€” semantic search over memory.
...
- `--workspace <dir>`: agent workspace path (default `~/./workspace`).
...
Non-interactive config helpers (get/set/unset). Running ` config` with no
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use ` health` or ` status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to ` doctor`).
...
### Usage tracking
 ...
 can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- ` status --usage` (prints full provider breakdown)
...
` models` is an alias for `models status`.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [` browser`](/cli/browser) and the [Browser tool](/tools/browser).
