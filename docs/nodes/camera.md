...
# Camera capture (agent)
 ...
 supports **camera capture** for agent workflows:
...
- **Settings â†’ General â†’ Allow Camera** (`.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return â€œCamera disabled by userâ€.

### CLI helper (node invoke)

Use the main `` CLI to invoke camera commands on the macOS node.
...
- ` nodes camera snap` defaults to `maxWidth=1600` unless overridden.
