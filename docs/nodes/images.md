...
- Send media with optional captions via ` message send --media`.
...
- When media is present, the web sender resolves local paths or URLs using the same pipeline as ` message send`.
- Multiple media entries are sent sequentially if provided.

## Inbound Media to Commands (Pi)

- When inbound web messages include media,  downloads to a temp file and exposes templating variables:
