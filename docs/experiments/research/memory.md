...
read_when:
  - Designing workspace memory (~/./workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep  integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/./workspace`) where â€œmemoryâ€ is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/./workspace` as canonical human-readable memory.
...
Recommendation: **deep integration in **, but keep a separable core library.
...
- You want the agent itself to call the tools:
  - ` memory recall "â€¦" --k 25 --since 30d`
  - ` memory reflect --since 7d`
