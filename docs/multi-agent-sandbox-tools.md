...
For debugging â€œwhy is this blocked?â€, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and ` sandbox explain`.
...
Legacy `agent.*` configs are migrated by ` doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
4. **Monitor logs:**
   ```exec
   tail -f "${_STATE_DIR:-$HOME/.}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
