...
- Replaces the legacy `gateway` command. CLI entry point: ` gateway`.
...
- Config hot reload watches `~/./.json` (or `_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/____/canvas/` from `~/./workspace/canvas`. Disable with `canvasHost.enabled=false` or `_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com..*` may still exist)
- Linux: `-gateway-<profile>.service`
- Windows: ` Gateway (<profile>)`

Install metadata is embedded in the service config:

- `_SERVICE_MARKER=`
- `_SERVICE_KIND=gateway`
...
```bash
 --dev setup
 --dev gateway --allow-unconfigured
...
- `_STATE_DIR=~/.-dev`
- `_CONFIG_PATH=~/.-dev/.json`
- `_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/./workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `_CANVAS_HOST_PORT` / config override)
...
```bash
 --profile main gateway install
 --profile rescue gateway install
```

Example:

```bash
_CONFIG_PATH=~/./a.json _STATE_DIR=~/.-a  gateway --port 19001
_CONFIG_PATH=~/./b.json _STATE_DIR=~/.-b  gateway --port 19002
```
...
- `health` â€” full health snapshot (same shape as ` health --json`).
...
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - ` gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com..*` is cleaned up).
  - ` doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are  profile services.
  ...
  - Cleanup: ` gateway uninstall` (current service) and ` doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use ` gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- .app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com..*` labels still unload cleanly).
- To stop it cleanly, use ` gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use ` gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use ` gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

 installs a **systemd user service** by default on Linux/WSL2. We
...
` gateway install` writes the user unit. ` doctor` audits the
...
Create `/etc/systemd/system/-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now -gateway[-<profile>].service
```
...
- ` gateway health|status` â€” request health/status over the Gateway WS.
- ` message send --target <num> --message "hi" [--media ...]` â€” send via Gateway (idempotent for WhatsApp).
- ` agent --message "hi" --to <num>` â€” run an agent turn (waits for final by default).
- ` gateway call <method> --params '{"k":"v"}'` â€” raw method invoker for debugging.
- ` gateway stop|restart` â€” stop/restart the supervised gateway service (launchd/systemd).
