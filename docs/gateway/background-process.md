...
# Background Exec + Process Tool
 ...
 runs shell commands through the `exec` tool and keeps longâ€‘running tasks in memory. The `process` tool manages those background sessions.
...
- `_BASH_PENDING_MAX_OUTPUT_CHARS`: pending stdout/stderr cap per stream (chars)
