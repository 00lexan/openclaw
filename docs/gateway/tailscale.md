...
# Tailscale (Gateway dashboard)
 ...
 can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `token` (default when `_GATEWAY_TOKEN` is set)
- `password` (shared secret via `_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
valid Serve proxy requests can authenticate via Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password.  verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
 only treats a request as Serve when it arrives from loopback with
...
Prefer `_GATEWAY_PASSWORD` over committing a password to disk.
...
- Set `gateway.tailscale.resetOnExit` if you want  to undo `tailscale serve`
