...
- During heartbeat runs,  treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from ` sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false,  skips the heartbeat run entirely (no model call).
