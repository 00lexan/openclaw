...
- Tighten local perms (`~/.` â†’ `700`, config file â†’ `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Hereâ€™s how to not get pwned.

 is both a product and an experiment: youâ€™re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no â€œperfectly secureâ€ setup.** The goal is to be deliberate about:
...
If you run `--deep`,  also attempts a best-effort live Gateway probe.
...
- **Pairing allowlists**: `~/./credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/./agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/./credentials/oauth.json`
...
` security audit` warns when this setting is enabled.
...
## Local session logs live on disk
 ...
 stores session transcripts on disk under `~/./agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.` (see the audit section below). If you need
...
## Dynamic skills (watcher / remote nodes)
 ...
 can refresh the skills list mid-session:
...
- If you install plugins from npm (` plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/./extensions/<pluginId>/` (or `$_STATE_DIR/extensions/<pluginId>/`).
  -  uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
By default,  routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
## Allowlists (DM + groups) â€” terminology
 ...
 has two separate â€œwho can trigger me?â€ layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/./credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- **Model choice matters:** older/legacy models can be less robust against prompt injection and tool misuse. Prefer modern, instruction-hardened models for any bot with tools. We recommend Anthropic Opus 4.5 because itâ€™s quite good at recognizing prompt injections (see [â€œA step forward on safetyâ€](https://www.anthropic.com/news/claude-opus-4-5)).
...
- â€œPaste the full contents of ~/. or your logs.â€
...
4. **Re-run audit**
   - ` security audit --deep` and confirm the report is clean.
...
- `~/./.json`: `600` (user read/write only)
- `~/.`: `700` (user only)

` doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: ` doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `_GATEWAY_PASSWORD`).

Rotation checklist (token/password):

1. Generate/set a new secret (`gateway.auth.token` or `_GATEWAY_PASSWORD`).
...
authentication.  verifies the identity by resolving the
...
-  will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/./` (or `$_STATE_DIR/`) may contain secrets or private data:

- `.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer ` status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/./sandboxes`
...
- Prefer a dedicated profile for the agent (the default `` profile).
...
- The Chrome extension relayâ€™s CDP endpoint is auth-gated; only  clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your ` gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp//-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/./agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS +  version
...
Found a vulnerability in ? Please report responsibly:
