...
- Publish as `/plugin-sdk` (or export from core under `/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `PluginApi.runtime` so plugins never import `src/**`.
...
- Introduce `/plugin-sdk`.
- Add `api.runtime` to `PluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `Runtime: ">=2026.2.0"`).
