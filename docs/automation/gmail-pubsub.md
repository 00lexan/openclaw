---
summary: "Gmail Pub/Sub push wired into  webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to 
...
Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` ->  webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
-  hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the  helper to wire everything together (installs deps on macOS via brew):
...
- Writes `hooks.gmail` config for ` webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled,  automatically sets
...
- Set `_SKIP_GMAIL_WATCHER=1` to opt out (useful if you run the daemon yourself).
...
- `--hook-url` points to  `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to .

Recommended: ` webhooks gmail run` wraps the same flow and auto-renews the watch.
