---
summary: "How  vendors Apple device model identifiers for friendly names in the macOS app."
read_when:
...
To keep builds deterministic, the JSON files are pinned to specific upstream commits (recorded in `apps/macos/Sources//Resources/DeviceModels/NOTICE.md`).

## Updating the database

1. Pick the upstream commits you want to pin to (one for iOS, one for macOS).
2. Update the commit hashes in `apps/macos/Sources//Resources/DeviceModels/NOTICE.md`.
...
4. Ensure `apps/macos/Sources//Resources/DeviceModels/LICENSE.apple-device-identifiers.txt` still matches upstream (replace it if the upstream license changes).
