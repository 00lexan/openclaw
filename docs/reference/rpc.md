...
# RPC adapters
 ...
 integrates external CLIs via JSON-RPC. Two patterns are used today.
...
-  owns lifecycle when `channels.signal.autoStart=true`.
...
-  spawns `imsg rpc` as a child process (legacy iMessage integration).
