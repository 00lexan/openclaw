...
When you **explicitly** switch channels with `--channel`,  also aligns
the install method:

- `dev` ensures a git checkout (default `~/`, override with `_GIT_DIR`),
...
When you switch channels with ` update`,  also syncs plugin sources:
