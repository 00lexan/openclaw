...
read_when:
  - You want to understand `.ai/install.sh`
...
# Installer internals
 ...
 ships two installer scripts (served from `.ai`):

- `https://.ai/install.sh` â€” â€œrecommendedâ€ installer (global npm install by default; can also install from a GitHub checkout)
- `https://.ai/install-cli.sh` â€” non-root-friendly CLI installer (installs into a prefix with its own Node)
- `https://.ai/install.ps1` â€” Windows PowerShell installer (npm by default; optional git install)

To see the current flags/behavior, run:

```bash
curl -fsSL https://.ai/install.sh | bash -s -- --help
```

Windows (PowerShell) help:

```powershell
& ([scriptblock]::Create((iwr -useb https://.ai/install.ps1))) -?
```

If the installer completes but `` is not found in a new terminal, itâ€™s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
- Choose install method:
  - `npm` (default): `npm install -g @latest`
...
- If upgrading an existing install: runs ` doctor --non-interactive` (best effort).
- For git installs: runs ` doctor --non-interactive` after install/update (best effort).
...
If you _want_ `sharp` to link against a globally-installed libvips (or youâ€™re debugging), set:

```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://.ai/install.sh | bash
```

### Discoverability / â€œgit installâ€ prompt

If you run the installer while **already inside a  source checkout** (detected via `package.json` + `pnpm-workspace.yaml`), it prompts:

- update and use this checkout (`git`)
- or migrate to the global npm install (`npm`)

In non-interactive contexts (no TTY / `--no-prompt`), you must pass `--install-method git|npm` (or set `_INSTALL_METHOD`), otherwise the script exits with code `2`.
...
This script installs `` into a prefix (default: `~/.`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you donâ€™t want to touch the system Node/npm.

Help:

```bash
curl -fsSL https://.ai/install-cli.sh | bash -s -- --help
```
...
- Choose install method:
  - `npm` (default): `npm install -g @latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs ` doctor --non-interactive` on upgrades and git installs (best effort).

Examples:

```powershell
iwr -useb https://.ai/install.ps1 | iex
```

```powershell
iwr -useb https://.ai/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\"
```

Environment variables:

- `_INSTALL_METHOD=git|npm`
- `_GIT_DIR=...`
