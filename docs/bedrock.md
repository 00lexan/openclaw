---
summary: "Use Amazon Bedrock (Converse API) models with "
read_when:
  - You want to use Amazon Bedrock models with 
...
# Amazon Bedrock
 ...
 can use **Amazon Bedrock** models via piâ€‘aiâ€™s **Bedrock Converse**
...
If AWS credentials are detected,  can automatically discover Bedrock
...
When running  on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, 's credential detection currently only checks for environment
...
-  surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
