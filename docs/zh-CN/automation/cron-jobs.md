...
- ä»»åŠ¡æŒä¹…åŒ–å­˜å‚¨åœ¨ `~/./cron/` ä¸‹ï¼Œå› æ­¤é‡å¯ä¸ä¼šä¸¢å¤±è®¡åˆ’ã€‚
- ä¸¤ç§æ‰§è¡Œæ–¹å¼ï¼š
...
å®šæ—¶ä»»åŠ¡é»˜è®¤æŒä¹…åŒ–å­˜å‚¨åœ¨ Gatewayç½‘å…³ä¸»æœºçš„ `~/./cron/jobs.json` ä¸­ã€‚Gatewayç½‘å…³å°†æ–‡ä»¶åŠ è½½åˆ°å†…å­˜ä¸­ï¼Œå¹¶åœ¨æ›´æ”¹æ—¶å†™å›žï¼Œå› æ­¤ä»…åœ¨ Gatewayç½‘å…³åœæ­¢æ—¶æ‰‹åŠ¨ç¼–è¾‘æ‰æ˜¯å®‰å…¨çš„ã€‚è¯·ä¼˜å…ˆä½¿ç”¨ ` cron add/edit` æˆ–å®šæ—¶ä»»åŠ¡å·¥å…·è°ƒç”¨ API è¿›è¡Œæ›´æ”¹ã€‚
...
- ä»»åŠ¡å­˜å‚¨ï¼š`~/./cron/jobs.json`ï¼ˆGatewayç½‘å…³ç®¡ç†çš„ JSONï¼‰ã€‚
- è¿è¡ŒåŽ†å²ï¼š`~/./cron/runs/<jobId>.jsonl`ï¼ˆJSONLï¼Œè‡ªåŠ¨æ¸…ç†ï¼‰ã€‚
...
- `cron.run`ï¼ˆå¼ºåˆ¶æˆ–åˆ°æœŸï¼‰ã€`cron.runs`
  å¦‚éœ€ä¸åˆ›å»ºä»»åŠ¡ç›´æŽ¥å‘é€ç³»ç»Ÿäº‹ä»¶ï¼Œè¯·ä½¿ç”¨ [` system event`](/cli/system)ã€‚

## æ•…éšœæŽ’é™¤

### "æ²¡æœ‰ä»»ä½•ä»»åŠ¡è¿è¡Œ"

- æ£€æŸ¥å®šæ—¶ä»»åŠ¡æ˜¯å¦å·²å¯ç”¨ï¼š`cron.enabled` å’Œ `_SKIP_CRON`ã€‚
