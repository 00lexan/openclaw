---
read_when:
  - å°† Gmail æ”¶ä»¶ç®±è§¦å‘å™¨æŽ¥å…¥ 
  - ä¸ºæ™ºèƒ½ä½“å”¤é†’è®¾ç½® Pub/Sub æŽ¨é€
summary: é€šè¿‡ gogcli å°† Gmail Pub/Sub æŽ¨é€æŽ¥å…¥  webhooks
title: Gmail PubSub
x-i18n:
...
ç›®æ ‡ï¼šGmail watch -> Pub/Sub æŽ¨é€ -> `gog gmail watch serve` ->  webhookã€‚
...
- å·²å¯ç”¨  hooksï¼ˆå‚è§ [Webhooks](/automation/webhook)ï¼‰ã€‚
...
ä½¿ç”¨  åŠ©æ‰‹å°†æ‰€æœ‰å†…å®¹æŽ¥å…¥åœ¨ä¸€èµ·ï¼ˆåœ¨ macOS ä¸Šé€šè¿‡ brew å®‰è£…ä¾èµ–ï¼‰ï¼š
...
- ä¸º ` webhooks gmail run` å†™å…¥ `hooks.gmail` é…ç½®ã€‚
- å¯ç”¨ Gmail hook é¢„è®¾ï¼ˆ`hooks.presets: ["gmail"]`ï¼‰ã€‚

è·¯å¾„è¯´æ˜Žï¼šå½“å¯ç”¨ `tailscale.mode` æ—¶ï¼Œ ä¼šè‡ªåŠ¨å°† `hooks.gmail.serve.path` è®¾ç½®ä¸º `/`ï¼Œå¹¶å°†å…¬å…±è·¯å¾„ä¿æŒåœ¨ `hooks.gmail.tailscale.path`ï¼ˆé»˜è®¤ `/gmail-pubsub`ï¼‰ï¼Œå› ä¸º Tailscale åœ¨ä»£ç†ä¹‹å‰ä¼šå‰¥ç¦»è®¾ç½®çš„è·¯å¾„å‰ç¼€ã€‚
...
- è®¾ç½® `_SKIP_GMAIL_WATCHER=1` å¯é€€å‡ºï¼ˆå¦‚æžœä½ è‡ªå·±è¿è¡Œå®ˆæŠ¤è¿›ç¨‹åˆ™å¾ˆæœ‰ç”¨ï¼‰ã€‚
...
- `--hook-url` æŒ‡å‘  `/hooks/gmail`ï¼ˆå·²æ˜ å°„ï¼›éš”ç¦»è¿è¡Œ + æ‘˜è¦å‘é€åˆ°ä¸»çº¿ç¨‹ï¼‰ã€‚
- `--include-body` å’Œ `--max-bytes` æŽ§åˆ¶å‘é€åˆ°  çš„æ­£æ–‡ç‰‡æ®µã€‚

æŽ¨èï¼š` webhooks gmail run` å°è£…äº†ç›¸åŒçš„æµç¨‹å¹¶è‡ªåŠ¨ç»­æœŸ watchã€‚
...
```bash
gog gmail watch status --account @gmail.com
gog gmail history --account @gmail.com --since <historyId>
```
...
```bash
gog gmail watch stop --account @gmail.com
