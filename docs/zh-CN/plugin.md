...
summary:  æ’ä»¶/æ‰©å±•ï¼šå‘çŽ°ã€é…ç½®å’Œå®‰å…¨
title: æ’ä»¶
x-i18n:
...
æ’ä»¶åªæ˜¯ä¸€ä¸ª**å°åž‹ä»£ç æ¨¡å—**ï¼Œç”¨é¢å¤–åŠŸèƒ½ï¼ˆå‘½ä»¤ã€å·¥å…·å’Œ Gateway ç½‘å…³ RPCï¼‰æ‰©å±• ã€‚

å¤§å¤šæ•°æ—¶å€™ï¼Œå½“ä½ æƒ³è¦ä¸€ä¸ªå°šæœªå†…ç½®åˆ°æ ¸å¿ƒ  çš„åŠŸèƒ½ï¼ˆæˆ–ä½ æƒ³å°†å¯é€‰åŠŸèƒ½æŽ’é™¤åœ¨ä¸»å®‰è£…ä¹‹å¤–ï¼‰æ—¶ï¼Œä½ ä¼šä½¿ç”¨æ’ä»¶ã€‚
...
- ä»Ž 2026.1.15 èµ· Microsoft Teams ä»…ä½œä¸ºæ’ä»¶æä¾›ï¼›å¦‚æžœä½¿ç”¨ Teamsï¼Œè¯·å®‰è£… `@/msteams`ã€‚
...
- Copilot Proxyï¼ˆæä¾›å•†è®¤è¯ï¼‰â€” æœ¬åœ° VS Code Copilot Proxy æ¡¥æŽ¥ï¼›ä¸Žå†…ç½® `github-copilot` è®¾å¤‡ç™»å½•ä¸åŒï¼ˆæ†ç»‘ï¼Œé»˜è®¤ç¦ç”¨ï¼‰
 ...
 æ’ä»¶æ˜¯é€šè¿‡ jiti åœ¨è¿è¡Œæ—¶åŠ è½½çš„ **TypeScript æ¨¡å—**ã€‚**é…ç½®éªŒè¯ä¸ä¼šæ‰§è¡Œæ’ä»¶ä»£ç **ï¼›å®ƒä½¿ç”¨æ’ä»¶æ¸…å•å’Œ JSON Schemaã€‚å‚è§ [æ’ä»¶æ¸…å•](/plugins/manifest)ã€‚
...
4. æ†ç»‘æ‰©å±•ï¼ˆéš  ä¸€èµ·å‘å¸ƒï¼Œ**é»˜è®¤ç¦ç”¨**ï¼‰

- `<>/extensions/*`

æ†ç»‘æ’ä»¶å¿…é¡»é€šè¿‡ `plugins.entries.<id>.enabled` æˆ– ` plugins enable <id>` æ˜¾å¼å¯ç”¨ã€‚å·²å®‰è£…çš„æ’ä»¶é»˜è®¤å¯ç”¨ï¼Œä½†å¯ä»¥ç”¨ç›¸åŒæ–¹å¼ç¦ç”¨ã€‚

æ¯ä¸ªæ’ä»¶å¿…é¡»åœ¨å…¶æ ¹ç›®å½•ä¸­åŒ…å« `.plugin.json` æ–‡ä»¶ã€‚å¦‚æžœè·¯å¾„æŒ‡å‘æ–‡ä»¶ï¼Œåˆ™æ’ä»¶æ ¹ç›®å½•æ˜¯æ–‡ä»¶çš„ç›®å½•ï¼Œå¿…é¡»åŒ…å«æ¸…å•ã€‚

å¦‚æžœå¤šä¸ªæ’ä»¶è§£æžåˆ°ç›¸åŒçš„ idï¼Œä¸Šè¿°é¡ºåºä¸­çš„ç¬¬ä¸€ä¸ªåŒ¹é…é¡¹èŽ·èƒœï¼Œè¾ƒä½Žä¼˜å…ˆçº§çš„å‰¯æœ¬è¢«å¿½ç•¥ã€‚

### åŒ…é›†åˆ

æ’ä»¶ç›®å½•å¯ä»¥åŒ…å«å¸¦æœ‰ `.extensions` çš„ `package.json`ï¼š
...
æ¸ é“æ’ä»¶å¯ä»¥é€šè¿‡ `.channel` å¹¿æ’­æ–°æ‰‹å¼•å¯¼å…ƒæ•°æ®ï¼Œé€šè¿‡ `.install` å¹¿æ’­å®‰è£…æç¤ºã€‚è¿™ä½¿æ ¸å¿ƒç›®å½•ä¿æŒæ— æ•°æ®ã€‚
...
```
 ...
 è¿˜å¯ä»¥åˆå¹¶**å¤–éƒ¨æ¸ é“ç›®å½•**ï¼ˆä¾‹å¦‚ï¼ŒMPM æ³¨å†Œè¡¨å¯¼å‡ºï¼‰ã€‚å°† JSON æ–‡ä»¶æ”¾åœ¨ä»¥ä¸‹ä½ç½®ä¹‹ä¸€ï¼š

- `~/./mpm/plugins.json`
- `~/./mpm/catalog.json`
- `~/./plugins/catalog.json`

æˆ–å°† `_PLUGIN_CATALOG_PATHS`ï¼ˆæˆ– `_MPM_CATALOG_PATHS`ï¼‰æŒ‡å‘ä¸€ä¸ªæˆ–å¤šä¸ª JSON æ–‡ä»¶ï¼ˆé€—å·/åˆ†å·/`PATH` åˆ†éš”ï¼‰ã€‚æ¯ä¸ªæ–‡ä»¶åº”åŒ…å« `{ "entries": [ { "name": "@scope/pkg", "": { "channel": {...}, "install": {...} } } ] }`ã€‚
...
å¦‚æžœæ’ä»¶å¯¼å‡º `id`ï¼Œ ä¼šä½¿ç”¨å®ƒï¼Œä½†å½“å®ƒä¸Žé…ç½®çš„ id ä¸åŒ¹é…æ—¶ä¼šå‘å‡ºè­¦å‘Šã€‚
...
- æ’ä»¶é…ç½®ä½¿ç”¨åµŒå…¥åœ¨ `.plugin.json`ï¼ˆ`configSchema`ï¼‰ä¸­çš„ JSON Schema è¿›è¡ŒéªŒè¯ã€‚
...
æŽ§åˆ¶ç•Œé¢ä½¿ç”¨ `config.schema`ï¼ˆJSON Schema + `uiHints`ï¼‰æ¥æ¸²æŸ“æ›´å¥½çš„è¡¨å•ã€‚
 ...
 åœ¨è¿è¡Œæ—¶æ ¹æ®å‘çŽ°çš„æ’ä»¶å¢žå¼º `uiHints`ï¼š
...
æ’ä»¶ä¹Ÿå¯ä»¥æ³¨å†Œè‡ªå·±çš„é¡¶çº§å‘½ä»¤ï¼ˆä¾‹å¦‚ï¼š` voicecall`ï¼‰ã€‚
...
- æ’ä»¶ç®¡ç†çš„é’©å­åœ¨ ` hooks list` ä¸­æ˜¾ç¤ºä¸º `plugin:<id>`ã€‚
- ä½ ä¸èƒ½é€šè¿‡ ` hooks` å¯ç”¨/ç¦ç”¨æ’ä»¶ç®¡ç†çš„é’©å­ï¼›è€Œæ˜¯å¯ç”¨/ç¦ç”¨æ’ä»¶ã€‚

## æä¾›å•†æ’ä»¶ï¼ˆæ¨¡åž‹è®¤è¯ï¼‰

æ’ä»¶å¯ä»¥æ³¨å†Œ**æ¨¡åž‹æä¾›å•†è®¤è¯**æµç¨‹ï¼Œä»¥ä¾¿ç”¨æˆ·å¯ä»¥åœ¨  å†…è¿è¡Œ OAuth æˆ– API å¯†é’¥è®¾ç½®ï¼ˆæ— éœ€å¤–éƒ¨è„šæœ¬ï¼‰ã€‚
...
- ä¸»åŒ…ï¼š``ï¼ˆæœ¬ä»“åº“ï¼‰
- æ’ä»¶ï¼š`@/*` ä¸‹çš„ç‹¬ç«‹ npm åŒ…ï¼ˆä¾‹å¦‚ï¼š`@/voice-call`ï¼‰

å‘å¸ƒå¥‘çº¦ï¼š

- æ’ä»¶ `package.json` å¿…é¡»åŒ…å«å¸¦æœ‰ä¸€ä¸ªæˆ–å¤šä¸ªå…¥å£æ–‡ä»¶çš„ `.extensions`ã€‚
- å…¥å£æ–‡ä»¶å¯ä»¥æ˜¯ `.js` æˆ– `.ts`ï¼ˆjiti åœ¨è¿è¡Œæ—¶åŠ è½½ TSï¼‰ã€‚
- ` plugins install <npm-spec>` ä½¿ç”¨ `npm pack`ï¼Œæå–åˆ° `~/./extensions/<id>/`ï¼Œå¹¶åœ¨é…ç½®ä¸­å¯ç”¨å®ƒã€‚
