...
- **ä¼šè¯å­˜å‚¨**ï¼ˆèŠå¤©åŽ†å² + è·¯ç”±çŠ¶æ€ï¼‰ä½äºŽ `~/./agents/<agentId>/sessions` ä¸‹ã€‚
...
Skills é€šè¿‡æ¯ä¸ªå·¥ä½œåŒºçš„ `skills/` æ–‡ä»¶å¤¹å®žçŽ°æ¯æ™ºèƒ½ä½“ç‹¬ç«‹ï¼Œå…±äº«çš„ Skills å¯ä»Ž `~/./skills` èŽ·å–ã€‚å‚è§ [Skillsï¼šæ¯æ™ºèƒ½ä½“ vs å…±äº«](/tools/skills#per-agent-vs-shared-skills)ã€‚
...
- é…ç½®ï¼š`~/./.json`ï¼ˆæˆ– `_CONFIG_PATH`ï¼‰
- çŠ¶æ€ç›®å½•ï¼š`~/.`ï¼ˆæˆ– `_STATE_DIR`ï¼‰
- å·¥ä½œåŒºï¼š`~/./workspace`ï¼ˆæˆ– `~/./workspace-<agentId>`ï¼‰
- æ™ºèƒ½ä½“ç›®å½•ï¼š`~/./agents/<agentId>/agent`ï¼ˆæˆ– `agents.list[].agentDir`ï¼‰
- ä¼šè¯ï¼š`~/./agents/<agentId>/sessions`

### å•æ™ºèƒ½ä½“æ¨¡å¼ï¼ˆé»˜è®¤ï¼‰

å¦‚æžœä½ ä»€ä¹ˆéƒ½ä¸åšï¼Œ è¿è¡Œå•ä¸ªæ™ºèƒ½ä½“ï¼š

- `agentId` é»˜è®¤ä¸º **`main`**ã€‚
- ä¼šè¯é”®ä¸º `agent:main:<mainKey>`ã€‚
- å·¥ä½œåŒºé»˜è®¤ä¸º `~/./workspace`ï¼ˆæˆ–å½“è®¾ç½®äº† `_PROFILE` æ—¶ä¸º `~/./workspace-<profile>`ï¼‰ã€‚
- çŠ¶æ€é»˜è®¤ä¸º `~/./agents/main/agent`ã€‚
...
- å¯¹äºŽå…±äº«ç¾¤ç»„ï¼Œå°†ç¾¤ç»„ç»‘å®šåˆ°ä¸€ä¸ªæ™ºèƒ½ä½“æˆ–ä½¿ç”¨ [å¹¿æ’­ç¾¤ç»„](/broadcast-groups)ã€‚
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // å¯é€‰è¦†ç›–ã€‚é»˜è®¤ï¼š~/./credentials/whatsapp/personal
...
å‚è§ [å¤šæ™ºèƒ½ä½“æ²™ç®±å’Œå·¥å…·](/multi-agent-sandbox-tools) äº†è§£è¯¦ç»†ç¤ºä¾‹ã€‚
