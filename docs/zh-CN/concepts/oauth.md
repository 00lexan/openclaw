...
# OAuth
 ...
 æ”¯æŒé€šè¿‡ OAuth è¿›è¡Œ"è®¢é˜…è®¤è¯"ï¼Œé€‚ç”¨äºŽæä¾›æ­¤åŠŸèƒ½çš„æä¾›å•†ï¼ˆç‰¹åˆ«æ˜¯ **OpenAI Codexï¼ˆChatGPT OAuthï¼‰**ï¼‰ã€‚å¯¹äºŽ Anthropic è®¢é˜…ï¼Œè¯·ä½¿ç”¨ **setup-token** æµç¨‹ã€‚æœ¬é¡µè¯´æ˜Žï¼š

- OAuth **ä»¤ç‰Œäº¤æ¢**çš„å·¥ä½œåŽŸç†ï¼ˆPKCEï¼‰
- ä»¤ç‰Œ**å­˜å‚¨**åœ¨å“ªé‡Œï¼ˆä»¥åŠåŽŸå› ï¼‰
- å¦‚ä½•å¤„ç†**å¤šè´¦æˆ·**ï¼ˆé…ç½®æ–‡ä»¶ + æŒ‰ä¼šè¯è¦†ç›–ï¼‰

 è¿˜æ”¯æŒ**æä¾›å•†æ’ä»¶**ï¼Œå®ƒä»¬è‡ªå¸¦ OAuth æˆ– API å¯†é’¥æµç¨‹ã€‚é€šè¿‡ä»¥ä¸‹å‘½ä»¤è¿è¡Œï¼š
...
- ä½ é€šè¿‡  _å’Œ_ Claude Code / Codex CLI ç™»å½• â†’ å…¶ä¸­ä¸€ä¸ªç¨åŽä¼šéšæœº"ç™»å‡º"

ä¸ºå‡å°‘è¿™ç§æƒ…å†µï¼Œ å°† `auth-profiles.json` è§†ä¸º**ä»¤ç‰Œæ±‡èšç‚¹**ï¼š
...
- è¿è¡Œæ—¶ç¼“å­˜ï¼ˆè‡ªåŠ¨ç®¡ç†ï¼›è¯·å‹¿ç¼–è¾‘ï¼‰ï¼š`~/./agents/<agentId>/agent/auth.json`

ä»…ç”¨äºŽå¯¼å…¥çš„æ—§ç‰ˆæ–‡ä»¶ï¼ˆä»ç„¶æ”¯æŒï¼Œä½†ä¸æ˜¯ä¸»å­˜å‚¨ï¼‰ï¼š

- `~/./credentials/oauth.json`ï¼ˆé¦–æ¬¡ä½¿ç”¨æ—¶å¯¼å…¥åˆ° `auth-profiles.json`ï¼‰

ä»¥ä¸Šæ‰€æœ‰è·¯å¾„ä¹Ÿéµå¾ª `$_STATE_DIR`ï¼ˆçŠ¶æ€ç›®å½•è¦†ç›–ï¼‰ã€‚å®Œæ•´å‚è€ƒï¼š[/gateway/configuration](/gateway/configuration#auth-storage-oauth--api-keys)

## Anthropic setup-tokenï¼ˆè®¢é˜…è®¤è¯ï¼‰

åœ¨ä»»æ„æœºå™¨ä¸Šè¿è¡Œ `claude setup-token`ï¼Œç„¶åŽå°†å…¶ç²˜è´´åˆ°  ä¸­ï¼š
...
## OAuth äº¤æ¢ï¼ˆç™»å½•å·¥ä½œåŽŸç†ï¼‰
 ...
 çš„äº¤äº’å¼ç™»å½•æµç¨‹åœ¨ `@mariozechner/pi-ai` ä¸­å®žçŽ°ï¼Œå¹¶é›†æˆåˆ°å‘å¯¼/å‘½ä»¤ä¸­ã€‚
...
å‘å¯¼è·¯å¾„ä¸º ` onboard` â†’ è®¤è¯é€‰æ‹© `setup-token`ï¼ˆAnthropicï¼‰ã€‚
...
å‘å¯¼è·¯å¾„ä¸º ` onboard` â†’ è®¤è¯é€‰æ‹© `openai-codex`ã€‚
...
- ` channels list --json`ï¼ˆæ˜¾ç¤º `auth[]`ï¼‰
