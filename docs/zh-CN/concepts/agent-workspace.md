...
è¿™ä¸Ž `~/./` æ˜¯åˆ†å¼€çš„ï¼ŒåŽè€…å­˜å‚¨é…ç½®ã€å‡­è¯å’Œä¼šè¯ã€‚
...
å½“å¯ç”¨æ²™ç®±éš”ç¦»ä¸” `workspaceAccess` ä¸æ˜¯ `"rw"` æ—¶ï¼Œå·¥å…·åœ¨ `~/./sandboxes` ä¸‹çš„æ²™ç®±å·¥ä½œåŒºå†…æ“ä½œï¼Œè€Œä¸æ˜¯ä½ çš„ä¸»æœºå·¥ä½œåŒºã€‚

## é»˜è®¤ä½ç½®

- é»˜è®¤ï¼š`~/./workspace`
- å¦‚æžœè®¾ç½®äº† `_PROFILE` ä¸”ä¸æ˜¯ `"default"`ï¼Œé»˜è®¤å€¼å˜ä¸º
  `~/./workspace-<profile>`ã€‚
- åœ¨ `~/./.json` ä¸­è¦†ç›–ï¼š
...
` onboard`ã€` configure` æˆ– ` setup` å°†åˆ›å»ºå·¥ä½œåŒºå¹¶åœ¨ç¼ºå¤±æ—¶å¡«å……å¼•å¯¼æ–‡ä»¶ã€‚
...
æ—§ç‰ˆå®‰è£…å¯èƒ½åˆ›å»ºäº† `~/`ã€‚ä¿ç•™å¤šä¸ªå·¥ä½œåŒºç›®å½•å¯èƒ½ä¼šå¯¼è‡´æ··ä¹±çš„è®¤è¯æˆ–çŠ¶æ€æ¼‚ç§»ï¼Œå› ä¸ºåŒä¸€æ—¶é—´åªæœ‰ä¸€ä¸ªå·¥ä½œåŒºæ˜¯æ´»åŠ¨çš„ã€‚

**å»ºè®®ï¼š** ä¿æŒå•ä¸ªæ´»åŠ¨å·¥ä½œåŒºã€‚å¦‚æžœä½ ä¸å†ä½¿ç”¨é¢å¤–çš„æ–‡ä»¶å¤¹ï¼Œè¯·å½’æ¡£æˆ–ç§»è‡³åºŸçº¸ç¯“ï¼ˆä¾‹å¦‚ `trash ~/`ï¼‰ã€‚
å¦‚æžœä½ æœ‰æ„ä¿ç•™å¤šä¸ªå·¥ä½œåŒºï¼Œè¯·ç¡®ä¿ `agents.defaults.workspace` æŒ‡å‘æ´»åŠ¨çš„é‚£ä¸ªã€‚

` doctor` åœ¨æ£€æµ‹åˆ°é¢å¤–å·¥ä½œåŒºç›®å½•æ—¶ä¼šå‘å‡ºè­¦å‘Šã€‚

## å·¥ä½œåŒºæ–‡ä»¶æ˜ å°„ï¼ˆæ¯ä¸ªæ–‡ä»¶çš„å«ä¹‰ï¼‰

è¿™äº›æ˜¯  åœ¨å·¥ä½œåŒºå†…æœŸæœ›çš„æ ‡å‡†æ–‡ä»¶ï¼š
...
å¦‚æžœä»»ä½•å¼•å¯¼æ–‡ä»¶ç¼ºå¤±ï¼Œ ä¼šåœ¨ä¼šè¯ä¸­æ³¨å…¥"ç¼ºå¤±æ–‡ä»¶"æ ‡è®°å¹¶ç»§ç»­ã€‚å¤§åž‹å¼•å¯¼æ–‡ä»¶åœ¨æ³¨å…¥æ—¶ä¼šè¢«æˆªæ–­ï¼›ä½¿ç”¨ `agents.defaults.bootstrapMaxChars` è°ƒæ•´é™åˆ¶ï¼ˆé»˜è®¤ï¼š20000ï¼‰ã€‚
` setup` å¯ä»¥é‡æ–°åˆ›å»ºç¼ºå¤±çš„é»˜è®¤å€¼è€Œä¸è¦†ç›–çŽ°æœ‰æ–‡ä»¶ã€‚

## å·¥ä½œåŒºä¸­ä¸åŒ…å«çš„å†…å®¹

è¿™äº›ä½äºŽ `~/./` ä¸‹ï¼Œä¸åº”æäº¤åˆ°å·¥ä½œåŒºä»“åº“ï¼š

- `~/./.json`ï¼ˆé…ç½®ï¼‰
- `~/./credentials/`ï¼ˆOAuth tokenã€API å¯†é’¥ï¼‰
- `~/./agents/<agentId>/sessions/`ï¼ˆä¼šè¯è®°å½• + å…ƒæ•°æ®ï¼‰
- `~/./skills/`ï¼ˆæ‰˜ç®¡çš„ Skillsï¼‰
...
å¦‚æžœä½ å¿…é¡»å­˜å‚¨æ•æ„Ÿå¼•ç”¨ï¼Œè¯·ä½¿ç”¨å ä½ç¬¦å¹¶å°†çœŸæ­£çš„å¯†é’¥ä¿å­˜åœ¨å…¶ä»–åœ°æ–¹ï¼ˆå¯†ç ç®¡ç†å™¨ã€çŽ¯å¢ƒå˜é‡æˆ– `~/./`ï¼‰ã€‚
...
1. å°†ä»“åº“å…‹éš†åˆ°æ‰€éœ€è·¯å¾„ï¼ˆé»˜è®¤ `~/./workspace`ï¼‰ã€‚
2. åœ¨ `~/./.json` ä¸­å°† `agents.defaults.workspace` è®¾ç½®ä¸ºè¯¥è·¯å¾„ã€‚
3. è¿è¡Œ ` setup --workspace <path>` æ¥å¡«å……ä»»ä½•ç¼ºå¤±çš„æ–‡ä»¶ã€‚
4. å¦‚æžœä½ éœ€è¦ä¼šè¯ï¼Œè¯·å•ç‹¬ä»Žæ—§æœºå™¨å¤åˆ¶ `~/./agents/<agentId>/sessions/`ã€‚
