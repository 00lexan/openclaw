...
- å¦‚æžœè®¾ç½®äº† `_GATEWAY_TOKEN`ï¼ˆæˆ– `--token`ï¼‰ï¼Œ`connect.params.auth.token` å¿…é¡»åŒ¹é…ï¼Œå¦åˆ™å¥—æŽ¥å­—å…³é—­ã€‚
...
è¯¦æƒ…ï¼š[Gateway ç½‘å…³åè®®](/gateway/protocol)ã€[é…å¯¹](/start/pairing)ã€[å®‰å…¨](/gateway/security)ã€‚
...
- å¯åŠ¨ï¼š` gateway`ï¼ˆå‰å°ï¼Œæ—¥å¿—è¾“å‡ºåˆ° stdoutï¼‰ã€‚
