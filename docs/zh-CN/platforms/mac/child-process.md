...
Gateway ç½‘å…³ï¼›å¦‚æžœæ— æ³•è®¿é—®ï¼Œå®ƒä¼šé€šè¿‡å¤–éƒ¨ `` CLIï¼ˆæ— åµŒå…¥å¼è¿è¡Œæ—¶ï¼‰å¯ç”¨ launchd
...
- åº”ç”¨å®‰è£…æ ‡è®°ä¸º `bot.molt.gateway` çš„æŒ‰ç”¨æˆ· LaunchAgent
  ï¼ˆä½¿ç”¨ `--profile`/`_PROFILE` æ—¶ä¸º `bot.molt.<profile>`ï¼›æ”¯æŒæ—§ç‰ˆ `com..*`ï¼‰ã€‚
...
`--attach-only`ï¼ˆæˆ– `--no-launchd`ï¼‰å¯åŠ¨å®ƒã€‚è¿™ä¼šè®¾ç½® `~/./disable-launchagent`ï¼Œ
