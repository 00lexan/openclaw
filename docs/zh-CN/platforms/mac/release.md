---
read_when:
  - åˆ¶ä½œæˆ–éªŒè¯  macOS å‘å¸ƒç‰ˆæœ¬
  - æ›´æ–° Sparkle appcast æˆ–è®¢é˜…æºèµ„æº
summary:  macOS å‘å¸ƒæ¸…å•ï¼ˆSparkle è®¢é˜…æºã€æ‰“åŒ…ã€ç­¾åï¼‰
title: macOS å‘å¸ƒ
x-i18n:
...
- ç”¨äºŽ `xcrun notarytool` çš„å…¬è¯å‡­æ®ï¼ˆé’¥åŒ™ä¸²é…ç½®æ–‡ä»¶æˆ– API å¯†é’¥ï¼‰ï¼Œä»¥å®žçŽ°é€šè¿‡ Gatekeeper å®‰å…¨åˆ†å‘çš„ DMG/zipã€‚
  - æˆ‘ä»¬ä½¿ç”¨åä¸º `-notary` çš„é’¥åŒ™ä¸²é…ç½®æ–‡ä»¶ï¼Œç”± shell é…ç½®æ–‡ä»¶ä¸­çš„ App Store Connect API å¯†é’¥çŽ¯å¢ƒå˜é‡åˆ›å»ºï¼š
    - `APP_STORE_CONNECT_API_KEY_P8`ã€`APP_STORE_CONNECT_KEY_ID`ã€`APP_STORE_CONNECT_ISSUER_ID`
    - `echo "$APP_STORE_CONNECT_API_KEY_P8" | sed 's/\\n/\n/g' > /tmp/-notary.p8`
    - `xcrun notarytool store-credentials "-notary" --key /tmp/-notary.p8 --key-id "$APP_STORE_CONNECT_KEY_ID" --issuer "$APP_STORE_CONNECT_ISSUER_ID"`
...
ditto -c -k --sequesterRsrc --keepParent dist/.app dist/-2026.1.27-beta.1.zip

# å¯é€‰ï¼šåŒæ—¶æž„å»ºé€‚åˆç”¨æˆ·ä½¿ç”¨çš„æ ·å¼åŒ– DMGï¼ˆæ‹–æ‹½åˆ° /Applicationsï¼‰
scripts/create-dmg.sh dist/.app dist/-2026.1.27-beta.1.dmg

# æŽ¨èï¼šæž„å»º + å…¬è¯/è£…è®¢ zip + DMG
# é¦–å…ˆï¼Œåˆ›å»ºä¸€æ¬¡é’¥åŒ™ä¸²é…ç½®æ–‡ä»¶ï¼š
#   xcrun notarytool store-credentials "-notary" \
#     --apple-id "<apple-id>" --team-id "<team-id>" --password "<app-specific-password>"
NOTARIZE=1 NOTARYTOOL_PROFILE=-notary \
BUNDLE_ID=bot.molt.mac \
APP_VERSION=2026.1.27-beta.1 \
APP_BUILD="$(git rev-list --count HEAD)" \
BUILD_CONFIG=release \
...
ditto -c -k --keepParent apps/macos/.build/release/.app.dSYM dist/-2026.1.27-beta.1.dSYM.zip
```
...
```bash
SPARKLE_PRIVATE_KEY_FILE=/path/to/ed25519-private-key scripts/make_appcast.sh dist/-2026.1.27-beta.1.zip https://raw.githubusercontent.com///main/appcast.xml
```

ä»Ž `CHANGELOG.md`ï¼ˆé€šè¿‡ [`scripts/changelog-to-html.sh`](https://github.com///blob/main/scripts/changelog-to-html.sh)ï¼‰ç”Ÿæˆ HTML å‘å¸ƒè¯´æ˜Žï¼Œå¹¶å°†å…¶åµŒå…¥ appcast æ¡ç›®ã€‚
å‘å¸ƒæ—¶ï¼Œå°†æ›´æ–°åŽçš„ `appcast.xml` ä¸Žå‘å¸ƒèµ„æºï¼ˆzip + dSYMï¼‰ä¸€èµ·æäº¤ã€‚

## å‘å¸ƒä¸ŽéªŒè¯

- å°† `-2026.1.27-beta.1.zip`ï¼ˆå’Œ `-2026.1.27-beta.1.dSYM.zip`ï¼‰ä¸Šä¼ åˆ°æ ‡ç­¾ `v2026.1.27-beta.1` å¯¹åº”çš„ GitHub å‘å¸ƒã€‚
- ç¡®ä¿åŽŸå§‹ appcast URL ä¸Žå†…ç½®çš„è®¢é˜…æºåŒ¹é…ï¼š`https://raw.githubusercontent.com///main/appcast.xml`ã€‚
- å®Œæ•´æ€§æ£€æŸ¥ï¼š
  - `curl -I https://raw.githubusercontent.com///main/appcast.xml` è¿”å›ž 200ã€‚
