...
**ç›®æ ‡ï¼š**  Gateway ç½‘å…³è¿è¡Œåœ¨ [Fly.io](https://fly.io) æœºå™¨ä¸Šï¼Œå…·æœ‰æŒä¹…å­˜å‚¨ã€è‡ªåŠ¨ HTTPS å’Œ Discord/æ¸ é“è®¿é—®ã€‚
...
| `internal_port = 3000`         | å¿…é¡»ä¸Ž `--port 3000`ï¼ˆæˆ– `_GATEWAY_PORT`ï¼‰åŒ¹é…ä»¥è¿›è¡Œ Fly å¥åº·æ£€æŸ¥ |
| `memory = "2048mb"`            | 512MB å¤ªå°ï¼›æŽ¨è 2GB                                                      |
| `_STATE_DIR = "/data"` | åœ¨å·ä¸ŠæŒä¹…åŒ–çŠ¶æ€                                                          |
...
- éž loopback ç»‘å®šï¼ˆ`--bind lan`ï¼‰å‡ºäºŽå®‰å…¨éœ€è¦ `_GATEWAY_TOKEN`ã€‚
- åƒå¯¹å¾…å¯†ç ä¸€æ ·å¯¹å¾…è¿™äº› tokenã€‚
- **ä¼˜å…ˆä½¿ç”¨çŽ¯å¢ƒå˜é‡è€Œä¸æ˜¯é…ç½®æ–‡ä»¶**æ¥å­˜å‚¨æ‰€æœ‰ API å¯†é’¥å’Œ tokenã€‚è¿™å¯ä»¥é¿å…å¯†é’¥å‡ºçŽ°åœ¨ `.json` ä¸­ï¼Œé˜²æ­¢æ„å¤–æš´éœ²æˆ–è®°å½•ã€‚
...
**æ³¨æ„ï¼š** ä½¿ç”¨ `_STATE_DIR=/data` æ—¶ï¼Œé…ç½®è·¯å¾„æ˜¯ `/data/.json`ã€‚
...
æˆ–è®¿é—® `https://my-.fly.dev/`

ç²˜è´´ä½ çš„ Gateway ç½‘å…³ tokenï¼ˆæ¥è‡ª `_GATEWAY_TOKEN` çš„é‚£ä¸ªï¼‰è¿›è¡Œè®¤è¯ã€‚
...
**ä¿®å¤ï¼š** ç¡®ä¿ `internal_port` ä¸Ž Gateway ç½‘å…³ç«¯å£åŒ¹é…ï¼ˆè®¾ç½® `--port 3000` æˆ– `_GATEWAY_PORT=3000`ï¼‰ã€‚
...
å¦‚æžœä½¿ç”¨ `--allow-unconfigured`ï¼ŒGateway ç½‘å…³ä¼šåˆ›å»ºæœ€å°é…ç½®ã€‚ä½ åœ¨ `/data/.json` çš„è‡ªå®šä¹‰é…ç½®åº”è¯¥åœ¨é‡å¯æ—¶è¢«è¯»å–ã€‚
...
**ä¿®å¤ï¼š** ç¡®ä¿ `fly.toml` ä¸­è®¾ç½®äº† `_STATE_DIR=/data` å¹¶é‡æ–°éƒ¨ç½²ã€‚
