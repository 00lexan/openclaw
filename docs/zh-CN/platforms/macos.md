...
summary:  macOS é…å¥—åº”ç”¨ï¼ˆèœå•æ  + Gateway ç½‘å…³ä»£ç†ï¼‰
title: macOS åº”ç”¨
x-i18n:
...
#  macOS é…å¥—åº”ç”¨ï¼ˆèœå•æ  + Gateway ç½‘å…³ä»£ç†ï¼‰

macOS åº”ç”¨æ˜¯  çš„**èœå•æ é…å¥—åº”ç”¨**ã€‚å®ƒæ‹¥æœ‰æƒé™ï¼Œåœ¨æœ¬åœ°ç®¡ç†/é™„åŠ åˆ° Gateway ç½‘å…³ï¼ˆlaunchd æˆ–æ‰‹åŠ¨ï¼‰ï¼Œå¹¶ä½œä¸ºèŠ‚ç‚¹å‘æ™ºèƒ½ä½“æš´éœ² macOS åŠŸèƒ½ã€‚
...
- æ ¹æ®è¯·æ±‚é€šè¿‡ npm/pnpm å®‰è£…å…¨å±€ CLIï¼ˆ``ï¼‰ï¼ˆä¸å»ºè®®ä½¿ç”¨ bun ä½œä¸º Gateway ç½‘å…³è¿è¡Œæ—¶ï¼‰ã€‚

## æœ¬åœ° vs è¿œç¨‹æ¨¡å¼

- **æœ¬åœ°**ï¼ˆé»˜è®¤ï¼‰ï¼šå¦‚æžœå­˜åœ¨è¿è¡Œä¸­çš„æœ¬åœ° Gateway ç½‘å…³ï¼Œåº”ç”¨é™„åŠ åˆ°å®ƒï¼›å¦åˆ™é€šè¿‡ ` gateway install` å¯ç”¨ launchd æœåŠ¡ã€‚
...
åº”ç”¨ç®¡ç†ä¸€ä¸ªæ ‡è®°ä¸º `bot.molt.gateway` çš„æ¯ç”¨æˆ· LaunchAgentï¼ˆä½¿ç”¨ `--profile`/`_PROFILE` æ—¶ä¸º `bot.molt.<profile>`ï¼›æ—§ç‰ˆ `com..*` ä»ä¼šå¸è½½ï¼‰ã€‚
...
å¦‚æžœ LaunchAgent æœªå®‰è£…ï¼Œä»Žåº”ç”¨ä¸­å¯ç”¨å®ƒæˆ–è¿è¡Œ ` gateway install`ã€‚
...
åº”ç”¨ä¸ºæœ¬åœ°æ“ä½œæ³¨å†Œ `://` URL æ–¹æ¡ˆã€‚
...
æç¤ºï¼šä¸Ž ` gateway discover --json` æ¯”è¾ƒï¼ŒæŸ¥çœ‹ macOS åº”ç”¨çš„å‘çŽ°ç®¡é“ï¼ˆNWBrowser + tailnet DNS-SD å›žé€€ï¼‰æ˜¯å¦ä¸Ž Node CLI åŸºäºŽ `dns-sd` çš„å‘çŽ°ä¸åŒã€‚
