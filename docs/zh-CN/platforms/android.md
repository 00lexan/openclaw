...
- ä½ å¯ä»¥åœ¨ Gateway ç½‘å…³æœºå™¨ä¸Šè¿è¡Œ CLIï¼ˆ``ï¼‰ï¼ˆæˆ–é€šè¿‡ SSHï¼‰ã€‚
...
- åœ¨ Gateway ç½‘å…³ä¸»æœºçš„ `~/./.json` ä¸­è®¾ç½® `gateway.bind: "tailnet"`ã€‚
...
1. åœ¨ Gateway ç½‘å…³ä¸»æœºä¸Šè®¾ç½® DNS-SD åŒºåŸŸï¼ˆç¤ºä¾‹ `.internal.`ï¼‰å¹¶å‘å¸ƒ `_-gw._tcp` è®°å½•ã€‚
...
1. åœ¨ Gateway ç½‘å…³ä¸»æœºä¸Šåˆ›å»º `~/./workspace/canvas/index.html`ã€‚
...
Tailnetï¼ˆå¯é€‰ï¼‰ï¼šå¦‚æžœä¸¤ä¸ªè®¾å¤‡éƒ½åœ¨ Tailscale ä¸Šï¼Œä½¿ç”¨ MagicDNS åç§°æˆ– tailnet IP è€Œä¸æ˜¯ `.local`ï¼Œä¾‹å¦‚ `http://<gateway-magicdns>:18793/____/canvas/`ã€‚

æ­¤æœåŠ¡å™¨å°†å®žæ—¶é‡è½½å®¢æˆ·ç«¯æ³¨å…¥ HTML å¹¶åœ¨æ–‡ä»¶æ›´æ”¹æ—¶é‡æ–°åŠ è½½ã€‚
A2UI ä¸»æœºä½äºŽ `http://<gateway-host>:18793/____/a2ui/`ã€‚
