---
read_when:
  - ä½ æƒ³åœ¨ GCP ä¸Š 24/7 è¿è¡Œ 
  - ä½ æƒ³è¦åœ¨è‡ªå·±çš„ VM ä¸Šè¿è¡Œç”Ÿäº§çº§ã€å¸¸é©»çš„ Gateway ç½‘å…³
  - ä½ æƒ³å®Œå…¨æŽ§åˆ¶æŒä¹…åŒ–ã€äºŒè¿›åˆ¶æ–‡ä»¶å’Œé‡å¯è¡Œä¸º
summary: åœ¨ GCP Compute Engine VMï¼ˆDockerï¼‰ä¸Š 24/7 è¿è¡Œ  Gateway ç½‘å…³å¹¶æŒä¹…åŒ–çŠ¶æ€
title: GCP
x-i18n:
...
# åœ¨ GCP Compute Engine ä¸Šè¿è¡Œ ï¼ˆDockerï¼Œç”Ÿäº§ VPS æŒ‡å—ï¼‰

## ç›®æ ‡

ä½¿ç”¨ Docker åœ¨ GCP Compute Engine VM ä¸Šè¿è¡ŒæŒä¹…åŒ–çš„  Gateway ç½‘å…³ï¼Œå…·æœ‰æŒä¹…çŠ¶æ€ã€å†…ç½®äºŒè¿›åˆ¶æ–‡ä»¶å’Œå®‰å…¨çš„é‡å¯è¡Œä¸ºã€‚

å¦‚æžœä½ æƒ³è¦" 24/7 å¤§çº¦ $5-12/æœˆ"ï¼Œè¿™æ˜¯åœ¨ Google Cloud ä¸Šçš„å¯é è®¾ç½®ã€‚
...
- åœ¨ Docker ä¸­å¯åŠ¨  Gateway ç½‘å…³
- åœ¨ä¸»æœºä¸ŠæŒä¹…åŒ– `~/.` + `~/./workspace`ï¼ˆé‡å¯/é‡å»ºåŽä»ä¿ç•™ï¼‰
...
## ä»€ä¹ˆæŒä¹…åŒ–åœ¨å“ªé‡Œï¼ˆçœŸå®žæ¥æºï¼‰
 ...
 åœ¨ Docker ä¸­è¿è¡Œï¼Œä½† Docker ä¸æ˜¯çœŸå®žæ¥æºã€‚
æ‰€æœ‰é•¿æœŸçŠ¶æ€å¿…é¡»åœ¨é‡å¯ã€é‡å»ºå’Œé‡å¯åŽä»ç„¶å­˜åœ¨ã€‚

| ç»„ä»¶             | ä½ç½®                              | æŒä¹…åŒ–æœºåˆ¶    | è¯´æ˜Ž                        |
| ---------------- | --------------------------------- | ------------- | --------------------------- |
| Gateway ç½‘å…³é…ç½® | `/home/node/./`           | ä¸»æœºå·æŒ‚è½½    | åŒ…æ‹¬ `.json`ã€ä»¤ç‰Œ  |
| æ¨¡åž‹è®¤è¯é…ç½®æ–‡ä»¶ | `/home/node/./`           | ä¸»æœºå·æŒ‚è½½    | OAuth ä»¤ç‰Œã€API å¯†é’¥        |
| Skill é…ç½®       | `/home/node/./skills/`    | ä¸»æœºå·æŒ‚è½½    | Skill çº§åˆ«çŠ¶æ€              |
| æ™ºèƒ½ä½“å·¥ä½œåŒº     | `/home/node/./workspace/` | ä¸»æœºå·æŒ‚è½½    | ä»£ç å’Œæ™ºèƒ½ä½“äº§ç‰©            |
| WhatsApp ä¼šè¯    | `/home/node/./`           | ä¸»æœºå·æŒ‚è½½    | ä¿ç•™ QR ç™»å½•                |
| Gmail å¯†é’¥çŽ¯     | `/home/node/./`           | ä¸»æœºå· + å¯†ç  | éœ€è¦ `GOG_KEYRING_PASSWORD` |
