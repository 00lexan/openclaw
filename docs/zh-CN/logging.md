...
æ‰€æœ‰æ—¥å¿—é…ç½®éƒ½åœ¨ `~/./.json` çš„ `logging` ä¸‹ã€‚
...
-  ç›®å‰é€šè¿‡ **OTLP/HTTPï¼ˆprotobufï¼‰** å¯¼å‡ºã€‚
...
- ä½ ä¹Ÿå¯ä»¥ä½¿ç”¨ ` plugins enable diagnostics-otel` å¯ç”¨æ’ä»¶ã€‚
...
- `.tokens`ï¼ˆè®¡æ•°å™¨ï¼Œå±žæ€§ï¼š`.token`ã€`.channel`ã€`.provider`ã€`.model`ï¼‰
- `.cost.usd`ï¼ˆè®¡æ•°å™¨ï¼Œå±žæ€§ï¼š`.channel`ã€`.provider`ã€`.model`ï¼‰
- `.run.duration_ms`ï¼ˆç›´æ–¹å›¾ï¼Œå±žæ€§ï¼š`.channel`ã€`.provider`ã€`.model`ï¼‰
- `.context.tokens`ï¼ˆç›´æ–¹å›¾ï¼Œå±žæ€§ï¼š`.context`ã€`.channel`ã€`.provider`ã€`.model`ï¼‰

æ¶ˆæ¯æµï¼š

- `.webhook.received`ï¼ˆè®¡æ•°å™¨ï¼Œå±žæ€§ï¼š`.channel`ã€`.webhook`ï¼‰
- `.webhook.error`ï¼ˆè®¡æ•°å™¨ï¼Œå±žæ€§ï¼š`.channel`ã€`.webhook`ï¼‰
- `.webhook.duration_ms`ï¼ˆç›´æ–¹å›¾ï¼Œå±žæ€§ï¼š`.channel`ã€`.webhook`ï¼‰
- `.message.queued`ï¼ˆè®¡æ•°å™¨ï¼Œå±žæ€§ï¼š`.channel`ã€`.source`ï¼‰
- `.message.processed`ï¼ˆè®¡æ•°å™¨ï¼Œå±žæ€§ï¼š`.channel`ã€`.outcome`ï¼‰
- `.message.duration_ms`ï¼ˆç›´æ–¹å›¾ï¼Œå±žæ€§ï¼š`.channel`ã€`.outcome`ï¼‰

é˜Ÿåˆ— + ä¼šè¯ï¼š

- `.queue.lane.enqueue`ï¼ˆè®¡æ•°å™¨ï¼Œå±žæ€§ï¼š`.lane`ï¼‰
- `.queue.lane.dequeue`ï¼ˆè®¡æ•°å™¨ï¼Œå±žæ€§ï¼š`.lane`ï¼‰
- `.queue.depth`ï¼ˆç›´æ–¹å›¾ï¼Œå±žæ€§ï¼š`.lane` æˆ– `.channel=heartbeat`ï¼‰
- `.queue.wait_ms`ï¼ˆç›´æ–¹å›¾ï¼Œå±žæ€§ï¼š`.lane`ï¼‰
- `.session.state`ï¼ˆè®¡æ•°å™¨ï¼Œå±žæ€§ï¼š`.state`ã€`.reason`ï¼‰
- `.session.stuck`ï¼ˆè®¡æ•°å™¨ï¼Œå±žæ€§ï¼š`.state`ï¼‰
- `.session.stuck_age_ms`ï¼ˆç›´æ–¹å›¾ï¼Œå±žæ€§ï¼š`.state`ï¼‰
- `.run.attempt`ï¼ˆè®¡æ•°å™¨ï¼Œå±žæ€§ï¼š`.attempt`ï¼‰

### å¯¼å‡ºçš„ spanï¼ˆåç§° + å…³é”®å±žæ€§ï¼‰

- `.model.usage`
  - `.channel`ã€`.provider`ã€`.model`
  - `.sessionKey`ã€`.sessionId`
  - `.tokens.*`ï¼ˆinput/output/cache_read/cache_write/totalï¼‰
- `.webhook.processed`
  - `.channel`ã€`.webhook`ã€`.chatId`
- `.webhook.error`
  - `.channel`ã€`.webhook`ã€`.chatId`ã€`.error`
- `.message.processed`
  - `.channel`ã€`.outcome`ã€`.chatId`ã€`.messageId`ã€`.sessionKey`ã€`.sessionId`ã€`.reason`
- `.session.stuck`
  - `.state`ã€`.ageMs`ã€`.queueDepth`ã€`.sessionKey`ã€`.sessionId`
...
- **Gateway ç½‘å…³æ— æ³•è®¿é—®ï¼Ÿ** å…ˆè¿è¡Œ ` doctor`ã€‚
