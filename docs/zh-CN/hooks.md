...
Hooks æä¾›äº†ä¸€ä¸ªå¯æ‰©å±•çš„äº‹ä»¶é©±åŠ¨ç³»ç»Ÿï¼Œç”¨äºŽå“åº”æ™ºèƒ½ä½“å‘½ä»¤å’Œäº‹ä»¶è‡ªåŠ¨æ‰§è¡Œæ“ä½œã€‚Hooks ä»Žç›®å½•ä¸­è‡ªåŠ¨å‘çŽ°ï¼Œå¯ä»¥é€šè¿‡ CLI å‘½ä»¤ç®¡ç†ï¼Œç±»ä¼¼äºŽ  ä¸­ Skills çš„å·¥ä½œæ–¹å¼ã€‚
...
- **Webhooks**ï¼šå¤–éƒ¨ HTTP webhooksï¼Œè®©å…¶ä»–ç³»ç»Ÿè§¦å‘  ä¸­çš„å·¥ä½œã€‚å‚è§ [Webhook Hooks](/automation/webhook) æˆ–ä½¿ç”¨ ` webhooks` èŽ·å– Gmail åŠ©æ‰‹å‘½ä»¤ã€‚

Hooks ä¹Ÿå¯ä»¥æ†ç»‘åœ¨æ’ä»¶ä¸­ï¼›å‚è§ [æ’ä»¶](/plugin#plugin-hooks)ã€‚
...
- åœ¨ä¸ä¿®æ”¹æ ¸å¿ƒä»£ç çš„æƒ…å†µä¸‹æ‰©å±•  çš„è¡Œä¸º

## å…¥é—¨

### æ†ç»‘çš„ Hooks

 é™„å¸¦å››ä¸ªè‡ªåŠ¨å‘çŽ°çš„æ†ç»‘ hooksï¼š

- **ðŸ’¾ session-memory**ï¼šå½“ä½ å‘å‡º `/new` æ—¶å°†ä¼šè¯ä¸Šä¸‹æ–‡ä¿å­˜åˆ°æ™ºèƒ½ä½“å·¥ä½œåŒºï¼ˆé»˜è®¤ `~/./workspace/memory/`ï¼‰
- **ðŸ“ command-logger**ï¼šå°†æ‰€æœ‰å‘½ä»¤äº‹ä»¶è®°å½•åˆ° `~/./logs/commands.log`
- **ðŸš€ boot-md**ï¼šå½“ Gateway ç½‘å…³å¯åŠ¨æ—¶è¿è¡Œ `BOOT.md`ï¼ˆéœ€è¦å¯ç”¨å†…éƒ¨ hooksï¼‰
- **ðŸ˜ˆ soul-evil**ï¼šåœ¨æ¸…é™¤çª—å£æœŸé—´æˆ–éšæœºæœºä¼šä¸‹å°†æ³¨å…¥çš„ `SOUL.md` å†…å®¹æ›¿æ¢ä¸º `SOUL_EVIL.md`
...
åœ¨æ–°æ‰‹å¼•å¯¼æœŸé—´ï¼ˆ` onboard`ï¼‰ï¼Œä½ å°†è¢«æç¤ºå¯ç”¨æŽ¨èçš„ hooksã€‚å‘å¯¼ä¼šè‡ªåŠ¨å‘çŽ°ç¬¦åˆæ¡ä»¶çš„ hooks å¹¶å‘ˆçŽ°ä¾›é€‰æ‹©ã€‚
...
2. **æ‰˜ç®¡ hooks**ï¼š`~/./hooks/`ï¼ˆç”¨æˆ·å®‰è£…ï¼Œè·¨å·¥ä½œåŒºå…±äº«ï¼‰
3. **æ†ç»‘ hooks**ï¼š`<>/dist/hooks/bundled/`ï¼ˆéš  é™„å¸¦ï¼‰
...
Hook åŒ…æ˜¯æ ‡å‡†çš„ npm åŒ…ï¼Œé€šè¿‡ `package.json` ä¸­çš„ `.hooks` å¯¼å‡ºä¸€ä¸ªæˆ–å¤šä¸ª hooksã€‚ä½¿ç”¨ä»¥ä¸‹å‘½ä»¤å®‰è£…ï¼š
...
Hook åŒ…å¯ä»¥é™„å¸¦ä¾èµ–ï¼›å®ƒä»¬å°†å®‰è£…åœ¨ `~/./hooks/<id>` ä¸‹ã€‚
...
homepage: https://docs..ai/hooks#my-hook
metadata:
  { "": { "emoji": "ðŸ”—", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
è¿™äº› hooks ä¸æ˜¯äº‹ä»¶æµç›‘å¬å™¨ï¼›å®ƒä»¬è®©æ’ä»¶åœ¨  æŒä¹…åŒ–å·¥å…·ç»“æžœä¹‹å‰åŒæ­¥è°ƒæ•´å®ƒä»¬ã€‚
...
- **æ‰˜ç®¡ hooks**ï¼ˆ`~/./hooks/`ï¼‰ï¼šè·¨å·¥ä½œåŒºå…±äº«
...
metadata: { "": { "emoji": "ðŸŽ¯", "events": ["command:new"] } }
---
...
# JSON output
 hooks info session-memory --json
```
...
**è¾“å‡º**ï¼š`<workspace>/memory/YYYY-MM-DD-slug.md`ï¼ˆé»˜è®¤ä¸º `~/./workspace`ï¼‰
...
åœ¨æ¸…é™¤çª—å£æœŸé—´æˆ–éšæœºæœºä¼šä¸‹å°†æ³¨å…¥çš„ `SOUL.md` å†…å®¹æ›¿æ¢ä¸º `SOUL_EVIL.md`ã€‚

**äº‹ä»¶**ï¼š`agent:bootstrap`

**æ–‡æ¡£**ï¼š[SOUL Evil Hook](/hooks/soul-evil)

**è¾“å‡º**ï¼šä¸å†™å…¥æ–‡ä»¶ï¼›æ›¿æ¢ä»…åœ¨å†…å­˜ä¸­å‘ç”Ÿã€‚
...
```yaml
metadata: { "": { "events": ["command:new"] } } # Specific
```

è€Œä¸æ˜¯ï¼š

```yaml
metadata: { "": { "events": ["command"] } } # General - more overhead
```
...
1. åˆ›å»º hook ç›®å½•ï¼š
   ...
   ```bash
   mkdir -p ~/./hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/./hooks/my-hook/handler.ts
   ```
...
- [æ†ç»‘ Hooks README](https://github.com///tree/main/src/hooks/bundled)
