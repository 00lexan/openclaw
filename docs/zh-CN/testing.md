...
# æµ‹è¯•
 ...
 åŒ…å«ä¸‰ä¸ª Vitest æµ‹è¯•å¥—ä»¶ï¼ˆå•å…ƒ/é›†æˆã€ç«¯åˆ°ç«¯ã€å®žæ—¶ï¼‰ä»¥åŠä¸€å°ç»„ Docker è¿è¡Œå™¨ã€‚
...
- é»˜è®¤ï¼šé€šè¿‡ `pnpm test:live` **å¯ç”¨**ï¼ˆè®¾ç½® `_LIVE_TEST=1`ï¼‰
- èŒƒå›´ï¼š
...
- é¢„æœŸï¼š
  ...
  - Anthropic å¯†é’¥è½®æ¢ï¼šè®¾ç½® `_LIVE_ANTHROPIC_KEYS="sk-...,sk-..."`ï¼ˆæˆ– `_LIVE_ANTHROPIC_KEY=sk-...`ï¼‰æˆ–å¤šä¸ª `ANTHROPIC_API_KEY*` å˜é‡ï¼›æµ‹è¯•ä¼šåœ¨é‡åˆ°é€ŸçŽ‡é™åˆ¶æ—¶é‡è¯•
...
- å¦‚ä½•å¯ç”¨ï¼š
  - `pnpm test:live`ï¼ˆæˆ–ç›´æŽ¥è°ƒç”¨ Vitest æ—¶ä½¿ç”¨ `_LIVE_TEST=1`ï¼‰
- è®¾ç½® `_LIVE_MODELS=modern`ï¼ˆæˆ– `all`ï¼Œmodern çš„åˆ«åï¼‰ä»¥å®žé™…è¿è¡Œæ­¤å¥—ä»¶ï¼›å¦åˆ™ä¼šè·³è¿‡ä»¥ä¿æŒ `pnpm test:live` ä¸“æ³¨äºŽ Gateway ç½‘å…³å†’çƒŸæµ‹è¯•
- å¦‚ä½•é€‰æ‹©æ¨¡åž‹ï¼š
  - `_LIVE_MODELS=modern` è¿è¡ŒçŽ°ä»£å…è®¸åˆ—è¡¨ï¼ˆOpus/Sonnet/Haiku 4.5ã€GPT-5.x + Codexã€Gemini 3ã€GLM 4.7ã€MiniMax M2.1ã€Grok 4ï¼‰
  - `_LIVE_MODELS=all` æ˜¯çŽ°ä»£å…è®¸åˆ—è¡¨çš„åˆ«å
  - æˆ– `_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-5,..."`ï¼ˆé€—å·åˆ†éš”çš„å…è®¸åˆ—è¡¨ï¼‰
- å¦‚ä½•é€‰æ‹©æä¾›å•†ï¼š
  - `_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"`ï¼ˆé€—å·åˆ†éš”çš„å…è®¸åˆ—è¡¨ï¼‰
- å¯†é’¥æ¥æºï¼š
  - é»˜è®¤ï¼šé…ç½®æ–‡ä»¶å­˜å‚¨å’ŒçŽ¯å¢ƒå˜é‡å›žé€€
  - è®¾ç½® `_LIVE_REQUIRE_PROFILE_KEYS=1` ä»¥å¼ºåˆ¶**ä»…ä½¿ç”¨é…ç½®æ–‡ä»¶å­˜å‚¨**
...
### ç¬¬äºŒå±‚ï¼šGateway ç½‘å…³ + å¼€å‘æ™ºèƒ½ä½“å†’çƒŸæµ‹è¯•ï¼ˆ"@"å®žé™…åšçš„äº‹ï¼‰
...
- å¦‚ä½•é€‰æ‹©æ¨¡åž‹ï¼š
  ...
  - `_LIVE_GATEWAY_MODELS=all` æ˜¯çŽ°ä»£å…è®¸åˆ—è¡¨çš„åˆ«å
  - æˆ–è®¾ç½® `_LIVE_GATEWAY_MODELS="provider/model"`ï¼ˆæˆ–é€—å·åˆ†éš”åˆ—è¡¨ï¼‰æ¥ç¼©å°èŒƒå›´
- å¦‚ä½•é€‰æ‹©æä¾›å•†ï¼ˆé¿å…"OpenRouter å…¨éƒ¨"ï¼‰ï¼š
  - `_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"`ï¼ˆé€—å·åˆ†éš”çš„å…è®¸åˆ—è¡¨ï¼‰
...
- ä»¤ç‰Œæ¥æºï¼ˆé€‰æ‹©ä¸€ä¸ªï¼‰ï¼š
  - é…ç½®æ–‡ä»¶ï¼š`_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test`
  - åŽŸå§‹ä»¤ç‰Œï¼š`_LIVE_SETUP_TOKEN_VALUE=sk-ant-oat01-...`
...
- è¦†ç›–ï¼ˆå¯é€‰ï¼‰ï¼š
  ...
  - `_LIVE_CLI_BACKEND_IMAGE_PROBE=1` å‘é€çœŸå®žå›¾åƒé™„ä»¶ï¼ˆè·¯å¾„æ³¨å…¥åˆ°æç¤ºä¸­ï¼‰ã€‚
  - `_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` å°†å›¾åƒæ–‡ä»¶è·¯å¾„ä½œä¸º CLI å‚æ•°ä¼ é€’è€Œéžæç¤ºæ³¨å…¥ã€‚
  - `_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"`ï¼ˆæˆ– `"list"`ï¼‰æŽ§åˆ¶è®¾ç½® `IMAGE_ARG` æ—¶å¦‚ä½•ä¼ é€’å›¾åƒå‚æ•°ã€‚
  - `_LIVE_CLI_BACKEND_RESUME_PROBE=1` å‘é€ç¬¬äºŒè½®å¹¶éªŒè¯æ¢å¤æµç¨‹ã€‚
- `_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` ä¿æŒ Claude Code CLI MCP é…ç½®å¯ç”¨ï¼ˆé»˜è®¤ä½¿ç”¨ä¸´æ—¶ç©ºæ–‡ä»¶ç¦ç”¨ MCP é…ç½®ï¼‰ã€‚
...
- Google ä¸“é¡¹ï¼ˆGemini API å¯†é’¥ + Antigravityï¼‰ï¼š
  - Geminiï¼ˆAPI å¯†é’¥ï¼‰ï¼š`_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
  - Antigravityï¼ˆOAuthï¼‰ï¼š`_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-5-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Gemini API ä¸Ž Gemini CLIï¼š
  - APIï¼š é€šè¿‡ HTTP è°ƒç”¨ Google æ‰˜ç®¡çš„ Gemini APIï¼ˆAPI å¯†é’¥/é…ç½®æ–‡ä»¶è®¤è¯ï¼‰ï¼›è¿™æ˜¯å¤§å¤šæ•°ç”¨æˆ·è¯´çš„"Gemini"ã€‚
  - CLIï¼š è°ƒç”¨æœ¬åœ° `gemini` äºŒè¿›åˆ¶æ–‡ä»¶ï¼›å®ƒæœ‰è‡ªå·±çš„è®¤è¯ï¼Œè¡Œä¸ºå¯èƒ½ä¸åŒï¼ˆæµå¼ä¼ è¾“/å·¥å…·æ”¯æŒ/ç‰ˆæœ¬å·®å¼‚ï¼‰ã€‚
...
- Googleï¼ˆAntigravityï¼‰ï¼š`google-antigravity/claude-opus-4-5-thinking` å’Œ `google-antigravity/gemini-3-flash`
...
åœ¨ `_LIVE_GATEWAY_MODELS` ä¸­è‡³å°‘åŒ…å«ä¸€ä¸ªæ”¯æŒå›¾åƒçš„æ¨¡åž‹ï¼ˆClaude/Gemini/OpenAI è§†è§‰èƒ½åŠ›å˜ä½“ç­‰ï¼‰ä»¥æµ‹è¯•å›¾åƒæŽ¢æµ‹ã€‚

### èšåˆå™¨/æ›¿ä»£ Gateway ç½‘å…³

å¦‚æžœä½ å¯ç”¨äº†å¯†é’¥ï¼Œæˆ‘ä»¬ä¹Ÿæ”¯æŒé€šè¿‡ä»¥ä¸‹æ–¹å¼æµ‹è¯•ï¼š

- OpenRouterï¼š`openrouter/...`ï¼ˆæ•°ç™¾ä¸ªæ¨¡åž‹ï¼›ä½¿ç”¨ ` models scan` æŸ¥æ‰¾æ”¯æŒå·¥å…·+å›¾åƒçš„å€™é€‰æ¨¡åž‹ï¼‰
...
- å¦‚æžœå®žæ—¶æµ‹è¯•è¯´"æ— å‡­è¯"ï¼Œç”¨è°ƒè¯• ` models list`/æ¨¡åž‹é€‰æ‹©ç›¸åŒçš„æ–¹å¼è°ƒè¯•ã€‚

- é…ç½®æ–‡ä»¶å­˜å‚¨ï¼š`~/./credentials/`ï¼ˆé¦–é€‰ï¼›æµ‹è¯•ä¸­"é…ç½®æ–‡ä»¶å¯†é’¥"çš„å«ä¹‰ï¼‰
...
- `_CONFIG_DIR=...`ï¼ˆé»˜è®¤ï¼š`~/.`ï¼‰æŒ‚è½½åˆ° `/home/node/.`
- `_WORKSPACE_DIR=...`ï¼ˆé»˜è®¤ï¼š`~/./workspace`ï¼‰æŒ‚è½½åˆ° `/home/node/./workspace`
- `_PROFILE_FILE=...`ï¼ˆé»˜è®¤ï¼š`~/.profile`ï¼‰æŒ‚è½½åˆ° `/home/node/.profile` å¹¶åœ¨è¿è¡Œæµ‹è¯•å‰åŠ è½½
- `_LIVE_GATEWAY_MODELS=...` / `_LIVE_MODELS=...` ç”¨äºŽç¼©å°è¿è¡ŒèŒƒå›´
- `_LIVE_REQUIRE_PROFILE_KEYS=1` ç¡®ä¿å‡­è¯æ¥è‡ªé…ç½®æ–‡ä»¶å­˜å‚¨ï¼ˆè€ŒéžçŽ¯å¢ƒå˜é‡ï¼‰
