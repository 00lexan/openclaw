...
# æ–‡æœ¬è½¬è¯­éŸ³ï¼ˆTTSï¼‰
 ...
 å¯ä»¥ä½¿ç”¨ ElevenLabsã€OpenAI æˆ– Edge TTS å°†å‡ºç«™å›žå¤è½¬æ¢ä¸ºéŸ³é¢‘ã€‚å®ƒå¯ä»¥åœ¨ä»»ä½•  èƒ½å‘é€éŸ³é¢‘çš„åœ°æ–¹å·¥ä½œï¼›Telegram ä¼šæ˜¾ç¤ºåœ†å½¢è¯­éŸ³æ¶ˆæ¯æ°”æ³¡ã€‚
...
Edge TTS **ä¸**éœ€è¦ API å¯†é’¥ã€‚å¦‚æžœæ²¡æœ‰æ‰¾åˆ° API å¯†é’¥ï¼Œ é»˜è®¤ä½¿ç”¨ Edge TTSï¼ˆé™¤éžé€šè¿‡ `messages.tts.edge.enabled=false` ç¦ç”¨ï¼‰ã€‚
...
TTS é…ç½®ä½äºŽ `.json` ä¸­çš„ `messages.tts` ä¸‹ã€‚å®Œæ•´ schema åœ¨ [Gateway ç½‘å…³é…ç½®](/gateway/configuration)ä¸­ã€‚
...
- å¦‚æžœ `provider` **æœªè®¾ç½®**ï¼Œ ä¼˜å…ˆé€‰æ‹© `openai`ï¼ˆå¦‚æžœæœ‰å¯†é’¥ï¼‰ï¼Œç„¶åŽæ˜¯ `elevenlabs`ï¼ˆå¦‚æžœæœ‰å¯†é’¥ï¼‰ï¼Œå¦åˆ™æ˜¯ `edge`ã€‚
...
æ–œæ å‘½ä»¤å°†æœ¬åœ°è¦†ç›–å†™å…¥ `prefsPath`ï¼ˆé»˜è®¤ï¼š`~/./settings/tts.json`ï¼Œå¯é€šè¿‡ `_TTS_PREFS` æˆ– `messages.tts.prefsPath` è¦†ç›–ï¼‰ã€‚
...
- **Edge TTS**ï¼šä½¿ç”¨ `edge.outputFormat`ï¼ˆé»˜è®¤ `audio-24khz-48kbitrate-mono-mp3`ï¼‰ã€‚
  ...
  - å¦‚æžœé…ç½®çš„ Edge è¾“å‡ºæ ¼å¼å¤±è´¥ï¼Œ ä¼šä½¿ç”¨ MP3 é‡è¯•ã€‚
...
Discord æ³¨æ„ï¼š`/tts` æ˜¯ Discord çš„å†…ç½®å‘½ä»¤ï¼Œæ‰€ä»¥  åœ¨é‚£é‡Œæ³¨å†Œ `/voice` ä½œä¸ºåŽŸç”Ÿå‘½ä»¤ã€‚æ–‡æœ¬ `/tts ...` ä»ç„¶æœ‰æ•ˆã€‚
