...
- æ”¶ç´§æœ¬åœ°æƒé™ï¼ˆ`~/.` â†’ `700`ï¼Œé…ç½®æ–‡ä»¶ â†’ `600`ï¼Œä»¥åŠå¸¸è§çŠ¶æ€æ–‡ä»¶å¦‚ `credentials/*.json`ã€`agents/*/agent/auth-profiles.json` å’Œ `agents/*/sessions/sessions.json`ï¼‰ã€‚

åœ¨ä½ çš„æœºå™¨ä¸Šè¿è¡Œå…·æœ‰ shell è®¿é—®æƒé™çš„ AI æ™ºèƒ½ä½“æ˜¯â€¦â€¦_æœ‰é£Žé™©çš„_ã€‚ä»¥ä¸‹æ˜¯å¦‚ä½•é¿å…è¢«æ”»å‡»çš„æ–¹æ³•ã€‚

 æ—¢æ˜¯äº§å“ä¹Ÿæ˜¯å®žéªŒï¼šä½ æ­£åœ¨å°†å‰æ²¿æ¨¡åž‹çš„è¡Œä¸ºè¿žæŽ¥åˆ°çœŸå®žçš„æ¶ˆæ¯å¹³å°å’ŒçœŸå®žçš„å·¥å…·ã€‚**ä¸å­˜åœ¨"å®Œç¾Žå®‰å…¨"çš„è®¾ç½®ã€‚** ç›®æ ‡æ˜¯æœ‰æ„è¯†åœ°è€ƒè™‘ï¼š
...
å¦‚æžœè¿è¡Œ `--deep`ï¼Œ è¿˜ä¼šå°è¯•å°½åŠ›è¿›è¡Œå®žæ—¶ Gateway ç½‘å…³æŽ¢æµ‹ã€‚
...
- **é…å¯¹ç™½åå•**ï¼š`~/./credentials/<channel>-allowFrom.json`
- **æ¨¡åž‹è®¤è¯é…ç½®**ï¼š`~/./agents/<agentId>/agent/auth-profiles.json`
...
` security audit` ä¼šåœ¨å¯ç”¨æ­¤è®¾ç½®æ—¶å‘å‡ºè­¦å‘Šã€‚
...
## æœ¬åœ°ä¼šè¯æ—¥å¿—å­˜å‚¨åœ¨ç£ç›˜ä¸Š
 ...
 å°†ä¼šè¯è®°å½•å­˜å‚¨åœ¨ `~/./agents/<agentId>/sessions/*.jsonl` ä¸‹çš„ç£ç›˜ä¸Šã€‚è¿™æ˜¯ä¼šè¯è¿žç»­æ€§å’Œï¼ˆå¯é€‰ï¼‰ä¼šè¯è®°å¿†ç´¢å¼•æ‰€å¿…éœ€çš„ï¼Œä½†è¿™ä¹Ÿæ„å‘³ç€**ä»»ä½•å…·æœ‰æ–‡ä»¶ç³»ç»Ÿè®¿é—®æƒé™çš„è¿›ç¨‹/ç”¨æˆ·éƒ½å¯ä»¥è¯»å–è¿™äº›æ—¥å¿—**ã€‚å°†ç£ç›˜è®¿é—®è§†ä¸ºä¿¡ä»»è¾¹ç•Œï¼Œå¹¶é”å®š `~/.` çš„æƒé™ï¼ˆå‚è§ä¸‹é¢çš„å®¡è®¡éƒ¨åˆ†ï¼‰ã€‚å¦‚æžœä½ éœ€è¦åœ¨æ™ºèƒ½ä½“ä¹‹é—´è¿›è¡Œæ›´å¼ºçš„éš”ç¦»ï¼Œè¯·åœ¨å•ç‹¬çš„æ“ä½œç³»ç»Ÿç”¨æˆ·æˆ–å•ç‹¬çš„ä¸»æœºä¸‹è¿è¡Œå®ƒä»¬ã€‚
...
## åŠ¨æ€ Skillsï¼ˆç›‘è§†å™¨/è¿œç¨‹èŠ‚ç‚¹ï¼‰
 ...
 å¯ä»¥åœ¨ä¼šè¯ä¸­åˆ·æ–° Skills åˆ—è¡¨ï¼š
...
- å¦‚æžœä½ ä»Ž npm å®‰è£…æ’ä»¶ï¼ˆ` plugins install <npm-spec>`ï¼‰ï¼Œå°†å…¶è§†ä¸ºè¿è¡Œä¸å—ä¿¡ä»»çš„ä»£ç ï¼š
  - å®‰è£…è·¯å¾„æ˜¯ `~/./extensions/<pluginId>/`ï¼ˆæˆ– `$_STATE_DIR/extensions/<pluginId>/`ï¼‰ã€‚
  -  ä½¿ç”¨ `npm pack` ç„¶åŽåœ¨è¯¥ç›®å½•ä¸­è¿è¡Œ `npm install --omit=dev`ï¼ˆnpm ç”Ÿå‘½å‘¨æœŸè„šæœ¬å¯ä»¥åœ¨å®‰è£…æœŸé—´æ‰§è¡Œä»£ç ï¼‰ã€‚
...
è¯¦æƒ… + ç£ç›˜ä¸Šçš„æ–‡ä»¶ï¼š[é…å¯¹](/start/pairing)

## ç§ä¿¡ä¼šè¯éš”ç¦»ï¼ˆå¤šç”¨æˆ·æ¨¡å¼ï¼‰

é»˜è®¤æƒ…å†µä¸‹ï¼Œ å°†**æ‰€æœ‰ç§ä¿¡è·¯ç”±åˆ°ä¸»ä¼šè¯**ï¼Œä»¥ä¾¿ä½ çš„åŠ©æ‰‹åœ¨è®¾å¤‡å’Œæ¸ é“ä¹‹é—´ä¿æŒè¿žç»­æ€§ã€‚å¦‚æžœ**å¤šäºº**å¯ä»¥ç»™æœºå™¨äººå‘ç§ä¿¡ï¼ˆå¼€æ”¾ç§ä¿¡æˆ–å¤šäººç™½åå•ï¼‰ï¼Œè¯·è€ƒè™‘éš”ç¦»ç§ä¿¡ä¼šè¯ï¼š
...
## ç™½åå•ï¼ˆç§ä¿¡ + ç¾¤ç»„ï¼‰â€”â€”æœ¯è¯­
 ...
 æœ‰ä¸¤ä¸ªç‹¬ç«‹çš„"è°å¯ä»¥è§¦å‘æˆ‘ï¼Ÿ"å±‚ï¼š
...
- **ç§ä¿¡ç™½åå•**ï¼ˆ`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`ï¼‰ï¼šè°è¢«å…è®¸åœ¨ç§ä¿¡ä¸­ä¸Žæœºå™¨äººäº¤è°ˆã€‚
  - å½“ `dmPolicy="pairing"` æ—¶ï¼Œæ‰¹å‡†ä¼šå†™å…¥ `~/./credentials/<channel>-allowFrom.json`ï¼ˆä¸Žé…ç½®ç™½åå•åˆå¹¶ï¼‰ã€‚
...
è¯¦æƒ…ï¼š[é…ç½®](/gateway/configuration)å’Œ[ç¾¤ç»„](/concepts/groups)
...
4. **é‡æ–°è¿è¡Œå®¡è®¡**
   - ` security audit --deep` å¹¶ç¡®è®¤æŠ¥å‘Šæ˜¯å¹²å‡€çš„ã€‚
...
- `~/./.json`ï¼š`600`ï¼ˆä»…ç”¨æˆ·è¯»/å†™ï¼‰
- `~/.`ï¼š`700`ï¼ˆä»…ç”¨æˆ·ï¼‰

` doctor` å¯ä»¥è­¦å‘Šå¹¶æä¾›æ”¶ç´§è¿™äº›æƒé™çš„é€‰é¡¹ã€‚
...
- é»˜è®¤ï¼š`18789`
- é…ç½®/æ ‡å¿—/çŽ¯å¢ƒå˜é‡ï¼š`gateway.port`ã€`--port`ã€`_GATEWAY_PORT`
...
Gateway ç½‘å…³é€šè¿‡ mDNSï¼ˆç«¯å£ 5353 ä¸Šçš„ `_-gw._tcp`ï¼‰å¹¿æ’­å…¶å­˜åœ¨ä»¥ç”¨äºŽæœ¬åœ°è®¾å¤‡å‘çŽ°ã€‚åœ¨å®Œæ•´æ¨¡å¼ä¸‹ï¼Œè¿™åŒ…æ‹¬å¯èƒ½æš´éœ²è¿è¥è¯¦æƒ…çš„ TXT è®°å½•ï¼š
...
4. **çŽ¯å¢ƒå˜é‡**ï¼ˆæ›¿ä»£æ–¹æ¡ˆï¼‰ï¼šè®¾ç½® `_DISABLE_BONJOUR=1` ä»¥åœ¨ä¸æ›´æ”¹é…ç½®çš„æƒ…å†µä¸‹ç¦ç”¨ mDNSã€‚
...
Doctor å¯ä»¥ä¸ºä½ ç”Ÿæˆä¸€ä¸ªï¼š` doctor --generate-gateway-token`ã€‚
...
- `gateway.auth.mode: "password"`ï¼šå¯†ç è®¤è¯ï¼ˆä¼˜å…ˆé€šè¿‡çŽ¯å¢ƒå˜é‡è®¾ç½®ï¼š`_GATEWAY_PASSWORD`ï¼‰ã€‚

è½®æ¢æ¸…å•ï¼ˆä»¤ç‰Œ/å¯†ç ï¼‰ï¼š

1. ç”Ÿæˆ/è®¾ç½®ä¸€ä¸ªæ–°çš„ç§˜å¯†ï¼ˆ`gateway.auth.token` æˆ– `_GATEWAY_PASSWORD`ï¼‰ã€‚
...
å½“ `gateway.auth.allowTailscale` ä¸º `true`ï¼ˆServe çš„é»˜è®¤å€¼ï¼‰æ—¶ï¼Œ æŽ¥å— Tailscale Serve èº«ä»½å¤´ï¼ˆ`tailscale-user-login`ï¼‰ä½œä¸ºè®¤è¯ã€‚ é€šè¿‡æœ¬åœ° Tailscale å®ˆæŠ¤è¿›ç¨‹ï¼ˆ`tailscale whois`ï¼‰è§£æž `x-forwarded-for` åœ°å€å¹¶å°†å…¶ä¸Žå¤´åŒ¹é…æ¥éªŒè¯èº«ä»½ã€‚è¿™ä»…å¯¹å‘½ä¸­å›žçŽ¯å¹¶åŒ…å« `x-forwarded-for`ã€`x-forwarded-proto` å’Œ `x-forwarded-host`ï¼ˆç”± Tailscale æ³¨å…¥ï¼‰çš„è¯·æ±‚è§¦å‘ã€‚
...
-  å°†ä¿¡ä»»æ¥è‡ªè¿™äº› IP çš„ `x-forwarded-for`ï¼ˆæˆ– `x-real-ip`ï¼‰æ¥ç¡®å®šå®¢æˆ·ç«¯ IP ä»¥è¿›è¡Œæœ¬åœ°é…å¯¹æ£€æŸ¥å’Œ HTTP è®¤è¯/æœ¬åœ°æ£€æŸ¥ã€‚
...
å‡è®¾ `~/./`ï¼ˆæˆ– `$_STATE_DIR/`ï¼‰ä¸‹çš„ä»»ä½•å†…å®¹éƒ½å¯èƒ½åŒ…å«ç§˜å¯†æˆ–ç§æœ‰æ•°æ®ï¼š

- `.json`ï¼šé…ç½®å¯èƒ½åŒ…å«ä»¤ç‰Œï¼ˆGateway ç½‘å…³ã€è¿œç¨‹ Gateway ç½‘å…³ï¼‰ã€æä¾›å•†è®¾ç½®å’Œç™½åå•ã€‚
...
- å…±äº«è¯Šæ–­ä¿¡æ¯æ—¶ï¼Œä¼˜å…ˆä½¿ç”¨ ` status --all`ï¼ˆå¯ç²˜è´´ï¼Œç§˜å¯†å·²è„±æ•ï¼‰è€Œä¸æ˜¯åŽŸå§‹æ—¥å¿—ã€‚
...
- `agents.defaults.sandbox.workspaceAccess: "none"`ï¼ˆé»˜è®¤ï¼‰ä½¿æ™ºèƒ½ä½“å·¥ä½œåŒºä¸å¯è®¿é—®ï¼›å·¥å…·é’ˆå¯¹ `~/./sandboxes` ä¸‹çš„æ²™ç®±å·¥ä½œåŒºè¿è¡Œ
...
- ä¼˜å…ˆä¸ºæ™ºèƒ½ä½“ä½¿ç”¨ä¸“ç”¨é…ç½®æ–‡ä»¶ï¼ˆé»˜è®¤çš„ `` é…ç½®æ–‡ä»¶ï¼‰ã€‚
...
- Chrome æ‰©å±•ä¸­ç»§çš„ CDP ç«¯ç‚¹æ˜¯è®¤è¯é—¨æŽ§çš„ï¼›åªæœ‰  å®¢æˆ·ç«¯å¯ä»¥è¿žæŽ¥ã€‚
...
é€šè¿‡å¤šæ™ºèƒ½ä½“è·¯ç”±ï¼Œæ¯ä¸ªæ™ºèƒ½ä½“å¯ä»¥æœ‰è‡ªå·±çš„æ²™ç®± + å·¥å…·ç­–ç•¥ï¼šä½¿ç”¨è¿™ä¸ªä¸ºæ¯ä¸ªæ™ºèƒ½ä½“æä¾›**å®Œå…¨è®¿é—®**ã€**åªè¯»**æˆ–**æ— è®¿é—®**æƒé™ã€‚å‚è§[å¤šæ™ºèƒ½ä½“æ²™ç®±å’Œå·¥å…·](/multi-agent-sandbox-tools)äº†è§£è¯¦æƒ…å’Œä¼˜å…ˆçº§è§„åˆ™ã€‚
...
1. **åœæ­¢å®ƒï¼š** åœæ­¢ macOS åº”ç”¨ï¼ˆå¦‚æžœå®ƒç›‘ç£ Gateway ç½‘å…³ï¼‰æˆ–ç»ˆæ­¢ä½ çš„ ` gateway` è¿›ç¨‹ã€‚
...
1. è½®æ¢ Gateway ç½‘å…³è®¤è¯ï¼ˆ`gateway.auth.token` / `_GATEWAY_PASSWORD`ï¼‰å¹¶é‡å¯ã€‚
...
1. æ£€æŸ¥ Gateway ç½‘å…³æ—¥å¿—ï¼š`/tmp//-YYYY-MM-DD.log`ï¼ˆæˆ– `logging.file`ï¼‰ã€‚
2. å®¡æŸ¥ç›¸å…³è®°å½•ï¼š`~/./agents/<agentId>/sessions/*.jsonl`ã€‚
...
- æ—¶é—´æˆ³ã€Gateway ç½‘å…³ä¸»æœºæ“ä½œç³»ç»Ÿ +  ç‰ˆæœ¬
...
åœ¨  ä¸­å‘çŽ°æ¼æ´žï¼Ÿè¯·è´Ÿè´£ä»»åœ°æŠ¥å‘Šï¼š

1. ç”µå­é‚®ä»¶ï¼šsecurity@.ai
