...
summary: åœ¨åŒä¸€ä¸»æœºä¸Šè¿è¡Œå¤šä¸ª  Gateway ç½‘å…³ï¼ˆéš”ç¦»ã€ç«¯å£å’Œé…ç½®æ–‡ä»¶ï¼‰
...
- `_CONFIG_PATH` â€” æ¯ä¸ªå®žä¾‹çš„é…ç½®æ–‡ä»¶
- `_STATE_DIR` â€” æ¯ä¸ªå®žä¾‹çš„ä¼šè¯ã€å‡­è¯ã€ç¼“å­˜
...
é…ç½®æ–‡ä»¶è‡ªåŠ¨é™å®š `_STATE_DIR` + `_CONFIG_PATH` èŒƒå›´å¹¶ä¸ºæœåŠ¡åç§°æ·»åŠ åŽç¼€ã€‚
...
åŸºç¡€ç«¯å£ = `gateway.port`ï¼ˆæˆ– `_GATEWAY_PORT` / `--port`ï¼‰ã€‚
