...
- æ›¿ä»£æ—§ç‰ˆ `gateway` å‘½ä»¤ã€‚CLI å…¥å£ç‚¹ï¼š` gateway`ã€‚
...
- é…ç½®çƒ­é‡è½½ç›‘è§† `~/./.json`ï¼ˆæˆ– `_CONFIG_PATH`ï¼‰ã€‚
...
- é»˜è®¤åœ¨ `canvasHost.port`ï¼ˆé»˜è®¤ `18793`ï¼‰ä¸Šå¯åŠ¨ Canvas æ–‡ä»¶æœåŠ¡å™¨ï¼Œä»Ž `~/./workspace/canvas` æä¾› `http://<gateway-host>:18793/____/canvas/`ã€‚ä½¿ç”¨ `canvasHost.enabled=false` æˆ– `_SKIP_CANVAS_HOST=1` ç¦ç”¨ã€‚
...
- é»˜è®¤éœ€è¦ Gateway ç½‘å…³è®¤è¯ï¼šè®¾ç½® `gateway.auth.token`ï¼ˆæˆ– `_GATEWAY_TOKEN`ï¼‰æˆ– `gateway.auth.password`ã€‚å®¢æˆ·ç«¯å¿…é¡»å‘é€ `connect.params.auth.token/password`ï¼Œé™¤éžä½¿ç”¨ Tailscale Serve èº«ä»½ã€‚
- å‘å¯¼çŽ°åœ¨é»˜è®¤ç”Ÿæˆä»¤ç‰Œï¼Œå³ä½¿åœ¨ loopback ä¸Šä¹Ÿæ˜¯å¦‚æ­¤ã€‚
- ç«¯å£ä¼˜å…ˆçº§ï¼š`--port` > `_GATEWAY_PORT` > `gateway.port` > é»˜è®¤ `18789`ã€‚
...
- macOSï¼š`bot.molt.<profile>`ï¼ˆæ—§ç‰ˆ `com..*` å¯èƒ½ä»ç„¶å­˜åœ¨ï¼‰
- Linuxï¼š`-gateway-<profile>.service`
- Windowsï¼š` Gateway (<profile>)`
...
- `_GATEWAY_PORT=19001`ï¼ˆGateway ç½‘å…³ WS + HTTPï¼‰
...
- å½“ä½ åœ¨ `--dev` ä¸‹è¿è¡Œ `setup`/`onboard` æ—¶ï¼Œ`agents.defaults.workspace` é»˜è®¤å˜ä¸º `~/./workspace-dev`ã€‚

æ´¾ç”Ÿç«¯å£ï¼ˆç»éªŒæ³•åˆ™ï¼‰ï¼š

- åŸºç¡€ç«¯å£ = `gateway.port`ï¼ˆæˆ– `_GATEWAY_PORT` / `--port`ï¼‰
- æµè§ˆå™¨æŽ§åˆ¶æœåŠ¡ç«¯å£ = åŸºç¡€ + 2ï¼ˆä»… loopbackï¼‰
- `canvasHost.port = åŸºç¡€ + 4`ï¼ˆæˆ– `_CANVAS_HOST_PORT` / é…ç½®è¦†ç›–ï¼‰
...
- `health` â€” å®Œæ•´å¥åº·å¿«ç…§ï¼ˆä¸Ž ` health --json` å½¢çŠ¶ç›¸åŒï¼‰ã€‚
- `status` â€” ç®€çŸ­æ‘˜è¦ã€‚
...
- LaunchAgents æ˜¯æŒ‰ç”¨æˆ·çš„ï¼Œéœ€è¦å·²ç™»å½•çš„ä¼šè¯ï¼›å¯¹äºŽæ— å¤´è®¾ç½®ï¼Œä½¿ç”¨è‡ªå®šä¹‰ LaunchDaemonï¼ˆæœªéšé™„ï¼‰ã€‚
  - ` gateway install` å†™å…¥ `~/Library/LaunchAgents/bot.molt.gateway.plist`
    ï¼ˆæˆ– `bot.molt.<profile>.plist`ï¼›æ—§ç‰ˆ `com..*` ä¼šè¢«æ¸…ç†ï¼‰ã€‚
  - ` doctor` å®¡è®¡ LaunchAgent é…ç½®ï¼Œå¯ä»¥å°†å…¶æ›´æ–°ä¸ºå½“å‰é»˜è®¤å€¼ã€‚
...
- å¦‚æžœæ£€æµ‹åˆ°å…¶ä»–ç±»ä¼¼ Gateway ç½‘å…³çš„æœåŠ¡ï¼ŒCLI ä¼šå‘å‡ºè­¦å‘Šï¼Œé™¤éžå®ƒä»¬æ˜¯  é…ç½®æ–‡ä»¶æœåŠ¡ã€‚
  ...
  - æ¸…ç†ï¼š` gateway uninstall`ï¼ˆå½“å‰æœåŠ¡ï¼‰å’Œ ` doctor`ï¼ˆæ—§ç‰ˆè¿ç§»ï¼‰ã€‚
- `gateway install` åœ¨å·²å®‰è£…æ—¶æ˜¯æ— æ“ä½œçš„ï¼›ä½¿ç”¨ ` gateway install --force` é‡æ–°å®‰è£…ï¼ˆé…ç½®æ–‡ä»¶/env/è·¯å¾„æ›´æ”¹ï¼‰ã€‚

æ†ç»‘çš„ mac åº”ç”¨ï¼š

- .app å¯ä»¥æ†ç»‘åŸºäºŽ Node çš„ Gateway ç½‘å…³ä¸­ç»§å¹¶å®‰è£…æ ‡è®°ä¸º
  `bot.molt.gateway`ï¼ˆæˆ– `bot.molt.<profile>`ï¼›æ—§ç‰ˆ `com..*` æ ‡ç­¾ä»èƒ½å¹²å‡€å¸è½½ï¼‰çš„æŒ‰ç”¨æˆ· LaunchAgentã€‚
- è¦å¹²å‡€åœ°åœæ­¢å®ƒï¼Œä½¿ç”¨ ` gateway stop`ï¼ˆæˆ– `launchctl bootout gui/$UID/bot.molt.gateway`ï¼‰ã€‚
- è¦é‡å¯ï¼Œä½¿ç”¨ ` gateway restart`ï¼ˆæˆ– `launchctl kickstart -k gui/$UID/bot.molt.gateway`ï¼‰ã€‚
  - `launchctl` ä»…åœ¨ LaunchAgent å·²å®‰è£…æ—¶æœ‰æ•ˆï¼›å¦åˆ™å…ˆä½¿ç”¨ ` gateway install`ã€‚
  - è¿è¡Œå‘½åé…ç½®æ–‡ä»¶æ—¶ï¼Œå°†æ ‡ç­¾æ›¿æ¢ä¸º `bot.molt.<profile>`ã€‚

## ç›‘ç®¡ï¼ˆsystemd ç”¨æˆ·å•å…ƒï¼‰

 åœ¨ Linux/WSL2 ä¸Šé»˜è®¤å®‰è£… **systemd ç”¨æˆ·æœåŠ¡**ã€‚æˆ‘ä»¬
...
` gateway install` å†™å…¥ç”¨æˆ·å•å…ƒã€‚` doctor` å®¡è®¡
...
åˆ›å»º `/etc/systemd/system/-gateway[-<profile>].service`ï¼ˆå¤åˆ¶ä¸Šé¢çš„å•å…ƒï¼Œ
...
- ` gateway health|status` â€” é€šè¿‡ Gateway ç½‘å…³ WS è¯·æ±‚ health/statusã€‚
- ` message send --target <num> --message "hi" [--media ...]` â€” é€šè¿‡ Gateway ç½‘å…³å‘é€ï¼ˆå¯¹ WhatsApp æ˜¯å¹‚ç­‰çš„ï¼‰ã€‚
- ` agent --message "hi" --to <num>` â€” è¿è¡Œæ™ºèƒ½ä½“è½®æ¬¡ï¼ˆé»˜è®¤ç­‰å¾…æœ€ç»ˆç»“æžœï¼‰ã€‚
- ` gateway call <method> --params '{"k":"v"}'` â€” ç”¨äºŽè°ƒè¯•çš„åŽŸå§‹æ–¹æ³•è°ƒç”¨å™¨ã€‚
- ` gateway stop|restart` â€” åœæ­¢/é‡å¯å—ç›‘ç®¡çš„ Gateway ç½‘å…³æœåŠ¡ï¼ˆlaunchd/systemdï¼‰ã€‚
- Gateway ç½‘å…³è¾…åŠ©å­å‘½ä»¤å‡è®¾ `--url` ä¸Šæœ‰è¿è¡Œä¸­çš„ Gateway ç½‘å…³ï¼›å®ƒä»¬ä¸å†è‡ªåŠ¨ç”Ÿæˆä¸€ä¸ªã€‚

## è¿ç§»æŒ‡å—

- æ·˜æ±° ` gateway` å’Œæ—§ç‰ˆ TCP æŽ§åˆ¶ç«¯å£çš„ä½¿ç”¨ã€‚
