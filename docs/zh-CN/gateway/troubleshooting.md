...
summary:  å¸¸è§æ•…éšœçš„å¿«é€Ÿæ•…éšœæŽ’é™¤æŒ‡å—
title: æ•…éšœæŽ’é™¤
x-i18n:
...
å½“  å‡ºçŽ°å¼‚å¸¸æ—¶ï¼Œä»¥ä¸‹æ˜¯è§£å†³æ–¹æ³•ã€‚
...
| ---------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------- |
| ` status`                  | æœ¬åœ°æ‘˜è¦ï¼šæ“ä½œç³»ç»Ÿ + æ›´æ–°ã€Gateway ç½‘å…³å¯è¾¾æ€§/æ¨¡å¼ã€æœåŠ¡ã€æ™ºèƒ½ä½“/ä¼šè¯ã€æä¾›å•†é…ç½®çŠ¶æ€ | é¦–æ¬¡æ£€æŸ¥ï¼Œå¿«é€Ÿæ¦‚è§ˆ                    |
| ` status --all`            | å®Œæ•´æœ¬åœ°è¯Šæ–­ï¼ˆåªè¯»ã€å¯ç²˜è´´ã€ç›¸å¯¹å®‰å…¨ï¼‰åŒ…æ‹¬æ—¥å¿—å°¾éƒ¨                                    | å½“ä½ éœ€è¦åˆ†äº«è°ƒè¯•æŠ¥å‘Šæ—¶                |
| ` status --deep`           | è¿è¡Œ Gateway ç½‘å…³å¥åº·æ£€æŸ¥ï¼ˆåŒ…æ‹¬æä¾›å•†æŽ¢æµ‹ï¼›éœ€è¦å¯è¾¾çš„ Gateway ç½‘å…³ï¼‰                  | å½“"å·²é…ç½®"ä¸æ„å‘³ç€"æ­£å¸¸å·¥ä½œ"æ—¶        |
| ` gateway probe`           | Gateway ç½‘å…³å‘çŽ° + å¯è¾¾æ€§ï¼ˆæœ¬åœ° + è¿œç¨‹ç›®æ ‡ï¼‰                                          | å½“ä½ æ€€ç–‘æ­£åœ¨æŽ¢æµ‹é”™è¯¯çš„ Gateway ç½‘å…³æ—¶ |
| ` channels status --probe` | å‘è¿è¡Œä¸­çš„ Gateway ç½‘å…³æŸ¥è¯¢æ¸ é“çŠ¶æ€ï¼ˆå¹¶å¯é€‰æŽ¢æµ‹ï¼‰                                     | å½“ Gateway ç½‘å…³å¯è¾¾ä½†æ¸ é“å¼‚å¸¸æ—¶       |
| ` gateway status`          | ç›‘ç®¡ç¨‹åºçŠ¶æ€ï¼ˆlaunchd/systemd/schtasksï¼‰ã€è¿è¡Œæ—¶ PID/é€€å‡ºã€æœ€åŽçš„ Gateway ç½‘å…³é”™è¯¯    | å½“æœåŠ¡"çœ‹èµ·æ¥å·²åŠ è½½"ä½†æ²¡æœ‰è¿è¡Œæ—¶      |
| ` logs --follow`           | å®žæ—¶æ—¥å¿—ï¼ˆè¿è¡Œæ—¶é—®é¢˜çš„æœ€ä½³ä¿¡å·ï¼‰                                                      | å½“ä½ éœ€è¦å®žé™…çš„æ•…éšœåŽŸå› æ—¶              |

**åˆ†äº«è¾“å‡ºï¼š** ä¼˜å…ˆä½¿ç”¨ ` status --all`ï¼ˆå®ƒä¼šéšè—ä»¤ç‰Œï¼‰ã€‚å¦‚æžœä½ ç²˜è´´ ` status`ï¼Œè€ƒè™‘å…ˆè®¾ç½® `_SHOW_SECRETS=0`ï¼ˆä»¤ç‰Œé¢„è§ˆï¼‰ã€‚
...
- ä¼˜å…ˆï¼š` logs --follow`
- æ–‡ä»¶æ—¥å¿—ï¼ˆå§‹ç»ˆï¼‰ï¼š`/tmp//-YYYY-MM-DD.log`ï¼ˆæˆ–ä½ é…ç½®çš„ `logging.file`ï¼‰
- macOS LaunchAgentï¼ˆå¦‚æžœå·²å®‰è£…ï¼‰ï¼š`$_STATE_DIR/logs/gateway.log` å’Œ `gateway.err.log`
- Linux systemdï¼ˆå¦‚æžœå·²å®‰è£…ï¼‰ï¼š`journalctl --user -u -gateway[-<profile>].service -n 200 --no-pager`
- Windowsï¼š`schtasks /Query /TN " Gateway (<profile>)" /V /FO LIST`
...
**è¿˜æ²¡æœ‰é…ç½®æ–‡ä»¶ï¼Ÿ** è¿è¡Œ ` setup` åˆ›å»ºåˆå§‹é…ç½®ï¼Œç„¶åŽé‡æ–°è¿è¡Œ
Gateway ç½‘å…³ã€‚
...
å˜é‡å¦‚ `DISPLAY` åº”è¯¥æ”¾åœ¨ `~/./.env` ä¸­ï¼ˆç”± Gateway ç½‘å…³æ—©æœŸåŠ è½½ï¼‰ã€‚
...
æœåŠ¡æ˜¯ç”¨ Bun æˆ–ç‰ˆæœ¬ç®¡ç†çš„ Node è·¯å¾„å®‰è£…çš„ï¼Œè¿è¡Œ ` doctor`
...
- ç„¶åŽè¿è¡Œ ` sandbox recreate --agent <id>`ï¼ˆæˆ– `--all`ï¼‰
...
- `gateway.mode` å¿…é¡»ä¸º `local` æ‰èƒ½è¿è¡Œ ` gateway` å’ŒæœåŠ¡ã€‚
- å¦‚æžœä½ è®¾ç½®äº† `gateway.mode=remote`ï¼Œ**CLI é»˜è®¤**ä½¿ç”¨è¿œç¨‹ URLã€‚æœåŠ¡å¯èƒ½ä»åœ¨æœ¬åœ°è¿è¡Œï¼Œä½†ä½ çš„ CLI å¯èƒ½åœ¨æŽ¢æµ‹é”™è¯¯çš„ä½ç½®ã€‚ä½¿ç”¨ ` gateway status` æŸ¥çœ‹æœåŠ¡è§£æžçš„ç«¯å£ + æŽ¢æµ‹ç›®æ ‡ï¼ˆæˆ–ä¼ é€’ `--url`ï¼‰ã€‚
- ` gateway status` å’Œ ` doctor` åœ¨æœåŠ¡çœ‹èµ·æ¥æ­£åœ¨è¿è¡Œä½†ç«¯å£å…³é—­æ—¶ä¼šæ˜¾ç¤ºæ—¥å¿—ä¸­çš„**æœ€åŽ Gateway ç½‘å…³é”™è¯¯**ã€‚
- éžæœ¬åœ°å›žçŽ¯ç»‘å®šï¼ˆ`lan`/`tailnet`/`custom`ï¼Œæˆ–æœ¬åœ°å›žçŽ¯ä¸å¯ç”¨æ—¶çš„ `auto`ï¼‰éœ€è¦è®¤è¯ï¼š
  `gateway.auth.token`ï¼ˆæˆ– `_GATEWAY_TOKEN`ï¼‰ã€‚
- `gateway.remote.token` ä»…ç”¨äºŽè¿œç¨‹ CLI è°ƒç”¨ï¼›å®ƒ**ä¸**å¯ç”¨æœ¬åœ°è®¤è¯ã€‚
- `gateway.token` è¢«å¿½ç•¥ï¼›ä½¿ç”¨ `gateway.auth.token`ã€‚

**å¦‚æžœ ` gateway status` æ˜¾ç¤ºé…ç½®ä¸åŒ¹é…**

- `Config (cli): ...` å’Œ `Config (service): ...` é€šå¸¸åº”è¯¥åŒ¹é…ã€‚
- å¦‚æžœä¸åŒ¹é…ï¼Œä½ å‡ ä¹Žè‚¯å®šæ˜¯åœ¨ç¼–è¾‘ä¸€ä¸ªé…ç½®è€ŒæœåŠ¡è¿è¡Œçš„æ˜¯å¦ä¸€ä¸ªã€‚
- ä¿®å¤ï¼šä»Žä½ å¸Œæœ›æœåŠ¡ä½¿ç”¨çš„ç›¸åŒ `--profile` / `_STATE_DIR` é‡æ–°è¿è¡Œ ` gateway install --force`ã€‚

**å¦‚æžœ ` gateway status` æŠ¥å‘ŠæœåŠ¡é…ç½®é—®é¢˜**

- ç›‘ç®¡ç¨‹åºé…ç½®ï¼ˆlaunchd/systemd/schtasksï¼‰ç¼ºå°‘å½“å‰é»˜è®¤å€¼ã€‚
- ä¿®å¤ï¼šè¿è¡Œ ` doctor` æ›´æ–°å®ƒï¼ˆæˆ– ` gateway install --force` å®Œå…¨é‡å†™ï¼‰ã€‚
...
- ä¿®å¤ï¼šè®¾ç½® `gateway.auth.mode` + `gateway.auth.token`ï¼ˆæˆ–å¯¼å‡º `_GATEWAY_TOKEN`ï¼‰å¹¶é‡å¯æœåŠ¡ã€‚

**å¦‚æžœ ` gateway status` æ˜¾ç¤º `bind=tailnet` ä½†æœªæ‰¾åˆ° tailnet æŽ¥å£**
...
å¦‚æžœä½ ä»Žæ—§ç‰ˆæœ¬å‡çº§ï¼Œä½ çš„ç£ç›˜ä¸Šå¯èƒ½ä»æœ‰ `~/`ã€‚
...
ç—‡çŠ¶ï¼š`pwd` æˆ–æ–‡ä»¶å·¥å…·æ˜¾ç¤º `~/./sandboxes/...` å³ä½¿ä½ 
...
### "Agent failed before reply: Unknown model: anthropic/claude-haiku-3-5"
 ...
 æœ‰æ„æ‹’ç»**è¾ƒæ—§/ä¸å®‰å…¨çš„æ¨¡åž‹**ï¼ˆå°¤å…¶æ˜¯é‚£äº›æ›´å®¹æ˜“å—åˆ°æç¤ºè¯æ³¨å…¥æ”»å‡»çš„æ¨¡åž‹ï¼‰ã€‚å¦‚æžœä½ çœ‹åˆ°æ­¤é”™è¯¯ï¼Œè¯¥æ¨¡åž‹åç§°å·²ä¸å†æ”¯æŒã€‚

**ä¿®å¤ï¼š**

- ä¸ºæä¾›å•†é€‰æ‹©**æœ€æ–°**æ¨¡åž‹å¹¶æ›´æ–°ä½ çš„é…ç½®æˆ–æ¨¡åž‹åˆ«åã€‚
- å¦‚æžœä½ ä¸ç¡®å®šå“ªäº›æ¨¡åž‹å¯ç”¨ï¼Œè¿è¡Œ ` models list` æˆ–
  ` models scan` å¹¶é€‰æ‹©ä¸€ä¸ªæ”¯æŒçš„æ¨¡åž‹ã€‚
...
```bash
 channels logout
trash "${_STATE_DIR:-$HOME/.}/credentials" # å¦‚æžœ logout æ— æ³•å®Œå…¨æ¸…é™¤æ‰€æœ‰å†…å®¹
 channels login --verbose       # é‡æ–°æ‰«æäºŒç»´ç 
```
...
å½“é…ç½®åŒ…å«æœªçŸ¥é”®ã€æ ¼å¼é”™è¯¯çš„å€¼æˆ–æ— æ•ˆç±»åž‹æ—¶ï¼Œ çŽ°åœ¨æ‹’ç»å¯åŠ¨ã€‚
...
- ` doctor` æŠ¥å‘Šæ¯ä¸ªæ— æ•ˆæ¡ç›®ã€‚
- ` doctor --fix` åº”ç”¨è¿ç§»/ä¿®å¤å¹¶é‡å†™é…ç½®ã€‚
- è¯Šæ–­å‘½ä»¤å¦‚ ` logs`ã€` health`ã€` status`ã€` gateway status` å’Œ ` gateway probe` å³ä½¿é…ç½®æ— æ•ˆä¹Ÿèƒ½è¿è¡Œã€‚
...
- `/tmp//â€¦` ä¸­çš„ **Gateway ç½‘å…³æ—¥å¿—**ä»¥èŽ·å–ç¡®åˆ‡çš„æä¾›å•†é”™è¯¯ã€‚
- **æ¨¡åž‹çŠ¶æ€**ï¼šä½¿ç”¨ `/model status`ï¼ˆèŠå¤©ï¼‰æˆ– ` models status`ï¼ˆCLIï¼‰ã€‚
...
5. è¿è¡Œ ` channels status --probe` èŽ·å–å®¡è®¡æç¤ºã€‚
...
ç«¯ç‚¹æŽ¥å— JSON Schema çš„ä¸¥æ ¼å­é›†ã€‚ åœ¨å½“å‰ `main` ä¸­æ¸…ç†/è§„èŒƒåŒ–å·¥å…·
...
å¦‚æžœé‡ç½®ä¸èµ·ä½œç”¨ï¼Œåœ¨ [`scripts/package-mac-app.sh`](https://github.com///blob/main/scripts/package-mac-app.sh) ä¸­æ›´æ”¹ `BUNDLE_ID`ï¼ˆä¾‹å¦‚ï¼Œæ·»åŠ  `.test` åŽç¼€ï¼‰å¹¶é‡æ–°æž„å»ºã€‚è¿™ä¼šå¼ºåˆ¶ macOS å°†å…¶è§†ä¸ºæ–°åº”ç”¨ã€‚
...
```bash
 gateway status
 gateway stop
# æˆ–ï¼šlaunchctl bootout gui/$UID/bot.molt.gatewayï¼ˆç”¨ bot.molt.<profile> æ›¿æ¢ï¼›æ—§ç‰ˆ com..* ä»ç„¶æœ‰æ•ˆï¼‰
```
...
ç¡®ä¿å…¨å±€ `` CLI å·²å®‰è£…ä¸”ä¸Žåº”ç”¨ç‰ˆæœ¬åŒ¹é…ï¼š
...
| æ—¥å¿—                             | ä½ç½®                                                                                                                                                                                                                                                                                                                      |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway ç½‘å…³æ–‡ä»¶æ—¥å¿—ï¼ˆç»“æž„åŒ–ï¼‰   | `/tmp//-YYYY-MM-DD.log`ï¼ˆæˆ– `logging.file`ï¼‰                                                                                                                                                                                                                                                              |
| Gateway ç½‘å…³æœåŠ¡æ—¥å¿—ï¼ˆç›‘ç®¡ç¨‹åºï¼‰ | macOSï¼š`$_STATE_DIR/logs/gateway.log` + `gateway.err.log`ï¼ˆé»˜è®¤ï¼š`~/./logs/...`ï¼›é…ç½®æ–‡ä»¶ä½¿ç”¨ `~/.-<profile>/logs/...`ï¼‰<br />Linuxï¼š`journalctl --user -u -gateway[-<profile>].service -n 200 --no-pager`<br />Windowsï¼š`schtasks /Query /TN " Gateway (<profile>)" /V /FO LIST` |
| ä¼šè¯æ–‡ä»¶                         | `$_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                          |
| åª’ä½“ç¼“å­˜                         | `$_STATE_DIR/media/`                                                                                                                                                                                                                                                                                              |
| å‡­è¯                             | `$_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                        |
...
trash "${_STATE_DIR:-$HOME/.}"
 channels login         # é‡æ–°é…å¯¹ WhatsApp
 gateway restart           # æˆ–ï¼š gateway
```
...
1. é¦–å…ˆæ£€æŸ¥æ—¥å¿—ï¼š`/tmp//`ï¼ˆé»˜è®¤ï¼š`-YYYY-MM-DD.log`ï¼Œæˆ–ä½ é…ç½®çš„ `logging.file`ï¼‰
