...
` doctor` æ˜¯  çš„ä¿®å¤ + è¿ç§»å·¥å…·ã€‚å®ƒä¿®å¤è¿‡æ—¶çš„é…ç½®/çŠ¶æ€ï¼Œæ£€æŸ¥å¥åº·çŠ¶å†µï¼Œå¹¶æä¾›å¯æ“ä½œçš„ä¿®å¤æ­¥éª¤ã€‚
...
å½“é…ç½®åŒ…å«å·²å¼ƒç”¨çš„é”®æ—¶ï¼Œå…¶ä»–å‘½ä»¤ä¼šæ‹’ç»è¿è¡Œå¹¶è¦æ±‚ä½ è¿è¡Œ ` doctor`ã€‚

Doctor å°†ï¼š

- è§£é‡Šæ‰¾åˆ°äº†å“ªäº›é—ç•™é”®ã€‚
- æ˜¾ç¤ºå®ƒåº”ç”¨çš„è¿ç§»ã€‚
- ä½¿ç”¨æ›´æ–°åŽçš„ schema é‡å†™ `~/./.json`ã€‚
...
- ä¼šè¯å­˜å‚¨ + è®°å½•ï¼š
  - ä»Ž `~/./sessions/` åˆ° `~/./agents/<agentId>/sessions/`
- æ™ºèƒ½ä½“ç›®å½•ï¼š
  - ä»Ž `~/./agent/` åˆ° `~/./agents/<agentId>/agent/`
- WhatsApp è®¤è¯çŠ¶æ€ï¼ˆBaileysï¼‰ï¼š
  - ä»Žé—ç•™çš„ `~/./credentials/*.json`ï¼ˆé™¤ `oauth.json` å¤–ï¼‰
  - åˆ° `~/./credentials/whatsapp/<accountId>/...`ï¼ˆé»˜è®¤è´¦æˆ· idï¼š`default`ï¼‰

è¿™äº›è¿ç§»æ˜¯å°½åŠ›è€Œä¸ºä¸”å¹‚ç­‰çš„ï¼›å½“ doctor å°†ä»»ä½•é—ç•™æ–‡ä»¶å¤¹ä½œä¸ºå¤‡ä»½ä¿ç•™æ—¶ä¼šå‘å‡ºè­¦å‘Šã€‚Gateway ç½‘å…³/CLI ä¹Ÿä¼šåœ¨å¯åŠ¨æ—¶è‡ªåŠ¨è¿ç§»é—ç•™ä¼šè¯ + æ™ºèƒ½ä½“ç›®å½•ï¼Œå› æ­¤åŽ†å²/è®¤è¯/æ¨¡åž‹ä¼šè½åœ¨æ¯æ™ºèƒ½ä½“è·¯å¾„ä¸­ï¼Œæ— éœ€æ‰‹åŠ¨è¿è¡Œ doctorã€‚WhatsApp è®¤è¯æœ‰æ„ä»…é€šè¿‡ ` doctor` è¿ç§»ã€‚
...
- **å¤šä¸ªçŠ¶æ€ç›®å½•**ï¼šå½“å¤šä¸ª `~/.` æ–‡ä»¶å¤¹å­˜åœ¨äºŽä¸åŒ home ç›®å½•æˆ–å½“ `_STATE_DIR` æŒ‡å‘åˆ«å¤„æ—¶å‘å‡ºè­¦å‘Šï¼ˆåŽ†å²å¯èƒ½åœ¨å®‰è£…ä¹‹é—´åˆ†è£‚ï¼‰ã€‚
- **è¿œç¨‹æ¨¡å¼æé†’**ï¼šå¦‚æžœ `gateway.mode=remote`ï¼Œdoctor ä¼šæé†’ä½ åœ¨è¿œç¨‹ä¸»æœºä¸Šè¿è¡Œå®ƒï¼ˆçŠ¶æ€åœ¨é‚£é‡Œï¼‰ã€‚
- **é…ç½®æ–‡ä»¶æƒé™**ï¼šå½“ `~/./.json` å¯¹ç»„/å…¶ä»–ç”¨æˆ·å¯è¯»æ—¶å‘å‡ºè­¦å‘Šï¼Œå¹¶æä¾›æ”¶ç´§åˆ° `600` çš„é€‰é¡¹ã€‚
...
Doctor æ£€æµ‹é—ç•™çš„ Gateway ç½‘å…³æœåŠ¡ï¼ˆlaunchd/systemd/schtasksï¼‰ï¼Œå¹¶æä¾›åˆ é™¤å®ƒä»¬å¹¶ä½¿ç”¨å½“å‰ Gateway ç½‘å…³ç«¯å£å®‰è£…  æœåŠ¡çš„é€‰é¡¹ã€‚å®ƒè¿˜å¯ä»¥æ‰«æé¢å¤–çš„ç±» Gateway ç½‘å…³æœåŠ¡å¹¶æ‰“å°æ¸…ç†æç¤ºã€‚
é…ç½®æ–‡ä»¶å‘½åçš„  Gateway ç½‘å…³æœåŠ¡è¢«è§†ä¸ºä¸€ç­‰å…¬æ°‘ï¼Œä¸ä¼šè¢«æ ‡è®°ä¸º"é¢å¤–çš„"ã€‚
...
å½“æœ¬åœ° Gateway ç½‘å…³ç¼ºå°‘ `gateway.auth` æ—¶ï¼ŒDoctor ä¼šå‘å‡ºè­¦å‘Šå¹¶æä¾›ç”Ÿæˆ token çš„é€‰é¡¹ã€‚ä½¿ç”¨ ` doctor --generate-gateway-token` åœ¨è‡ªåŠ¨åŒ–ä¸­å¼ºåˆ¶åˆ›å»º tokenã€‚
...
- ` doctor` åœ¨é‡å†™ supervisor é…ç½®å‰æç¤ºã€‚
- ` doctor --yes` æŽ¥å—é»˜è®¤ä¿®å¤æç¤ºã€‚
- ` doctor --repair` æ— éœ€æç¤ºåº”ç”¨æŽ¨èçš„ä¿®å¤ã€‚
- ` doctor --repair --force` è¦†ç›–è‡ªå®šä¹‰ supervisor é…ç½®ã€‚
- ä½ å§‹ç»ˆå¯ä»¥é€šè¿‡ ` gateway install --force` å¼ºåˆ¶å®Œå…¨é‡å†™ã€‚
