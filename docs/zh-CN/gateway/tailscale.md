...
# Tailscaleï¼ˆGateway ç½‘å…³ä»ªè¡¨ç›˜ï¼‰
 ...
 å¯ä»¥ä¸º Gateway ç½‘å…³ä»ªè¡¨ç›˜å’Œ WebSocket ç«¯å£è‡ªåŠ¨é…ç½® Tailscale **Serve**ï¼ˆtailnetï¼‰æˆ– **Funnel**ï¼ˆå…¬å…±ï¼‰ã€‚è¿™ä½¿ Gateway ç½‘å…³ä¿æŒç»‘å®šåˆ° loopbackï¼ŒåŒæ—¶ Tailscale æä¾› HTTPSã€è·¯ç”±å’Œï¼ˆå¯¹äºŽ Serveï¼‰èº«ä»½å¤´ã€‚

## æ¨¡å¼

- `serve`ï¼šä»…é™ Tailnet çš„ Serveï¼Œé€šè¿‡ `tailscale serve`ã€‚Gateway ç½‘å…³ä¿æŒåœ¨ `127.0.0.1` ä¸Šã€‚
- `funnel`ï¼šé€šè¿‡ `tailscale funnel` çš„å…¬å…± HTTPSã€‚ éœ€è¦å…±äº«å¯†ç ã€‚
...
- `token`ï¼ˆè®¾ç½® `_GATEWAY_TOKEN` æ—¶çš„é»˜è®¤å€¼ï¼‰
- `password`ï¼ˆé€šè¿‡ `_GATEWAY_PASSWORD` æˆ–é…ç½®çš„å…±äº«å¯†é’¥ï¼‰

å½“ `tailscale.mode = "serve"` ä¸” `gateway.auth.allowTailscale` ä¸º `true` æ—¶ï¼Œ
æœ‰æ•ˆçš„ Serve ä»£ç†è¯·æ±‚å¯ä»¥é€šè¿‡ Tailscale èº«ä»½å¤´ï¼ˆ`tailscale-user-login`ï¼‰è¿›è¡Œè®¤è¯ï¼Œæ— éœ€æä¾›ä»¤ç‰Œ/å¯†ç ã€‚ é€šè¿‡æœ¬åœ° Tailscale å®ˆæŠ¤è¿›ç¨‹ï¼ˆ`tailscale whois`ï¼‰è§£æž `x-forwarded-for` åœ°å€å¹¶å°†å…¶ä¸Žå¤´åŒ¹é…æ¥éªŒè¯èº«ä»½ï¼Œç„¶åŽæ‰æŽ¥å—å®ƒã€‚
 ä»…åœ¨è¯·æ±‚ä»Ž loopback åˆ°è¾¾å¹¶å¸¦æœ‰ Tailscale çš„ `x-forwarded-for`ã€`x-forwarded-proto` å’Œ `x-forwarded-host` å¤´æ—¶æ‰å°†å…¶è§†ä¸º Serve è¯·æ±‚ã€‚
...
ä¼˜å…ˆä½¿ç”¨ `_GATEWAY_PASSWORD` è€Œä¸æ˜¯å°†å¯†ç æäº¤åˆ°ç£ç›˜ã€‚
...
- å¦‚æžœä½ å¸Œæœ›  åœ¨å…³é—­æ—¶æ’¤é”€ `tailscale serve` æˆ– `tailscale funnel` é…ç½®ï¼Œè®¾ç½® `gateway.tailscale.resetOnExit`ã€‚
