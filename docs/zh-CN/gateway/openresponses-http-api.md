...
# OpenResponses APIï¼ˆHTTPï¼‰
 ...
 çš„ Gateway ç½‘å…³å¯ä»¥æä¾›å…¼å®¹ OpenResponses çš„ `POST /v1/responses` ç«¯ç‚¹ã€‚
...
åº•å±‚å®žçŽ°ä¸­ï¼Œè¯·æ±‚ä½œä¸ºæ­£å¸¸çš„ Gateway ç½‘å…³æ™ºèƒ½ä½“è¿è¡Œæ‰§è¡Œï¼ˆä¸Ž ` agent` ç›¸åŒçš„ä»£ç è·¯å¾„ï¼‰ï¼Œå› æ­¤è·¯ç”±/æƒé™/é…ç½®ä¸Žä½ çš„ Gateway ç½‘å…³ä¸€è‡´ã€‚
...
- `model: ":<agentId>"`ï¼ˆç¤ºä¾‹ï¼š`":main"`ã€`":beta"`ï¼‰
