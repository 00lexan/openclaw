...
# OpenAI Chat Completionsï¼ˆHTTPï¼‰
 ...
 çš„ Gateway ç½‘å…³å¯ä»¥æä¾›ä¸€ä¸ªå°åž‹çš„ OpenAI å…¼å®¹ Chat Completions ç«¯ç‚¹ã€‚
...
åº•å±‚å®žçŽ°ä¸­ï¼Œè¯·æ±‚ä½œä¸ºæ™®é€šçš„ Gateway ç½‘å…³æ™ºèƒ½ä½“è¿è¡Œæ‰§è¡Œï¼ˆä¸Ž ` agent` ç›¸åŒçš„ä»£ç è·¯å¾„ï¼‰ï¼Œå› æ­¤è·¯ç”±/æƒé™/é…ç½®ä¸Žä½ çš„ Gateway ç½‘å…³ä¸€è‡´ã€‚
...
- `model: ":<agentId>"`ï¼ˆä¾‹å¦‚ï¼š`":main"`ã€`":beta"`ï¼‰
- `model: "agent:<agentId>"`ï¼ˆåˆ«åï¼‰

æˆ–é€šè¿‡å¤´æŒ‡å®šç‰¹å®šçš„  æ™ºèƒ½ä½“ï¼š

- `x--agent-id: <agentId>`ï¼ˆé»˜è®¤ï¼š`main`ï¼‰

é«˜çº§é€‰é¡¹ï¼š

- `x--session-key: <sessionKey>` å®Œå…¨æŽ§åˆ¶ä¼šè¯è·¯ç”±ã€‚
