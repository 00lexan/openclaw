...
# Bonjour / mDNS è®¾å¤‡å‘çŽ°
 ...
 ä½¿ç”¨ Bonjourï¼ˆmDNS / DNSâ€‘SDï¼‰ä½œä¸º**ä»…é™å±€åŸŸç½‘çš„ä¾¿æ·æ–¹å¼**æ¥å‘çŽ°
...
2. åœ¨ä¸“ç”¨åŒºåŸŸä¸‹å‘å¸ƒ `_-gw._tcp` çš„ DNSâ€‘SD è®°å½•
   ï¼ˆç¤ºä¾‹ï¼š`.internal.`ï¼‰ã€‚
3. é…ç½® Tailscale **åˆ†å‰² DNS**ï¼Œä½¿ä½ é€‰æ‹©çš„åŸŸåé€šè¿‡è¯¥
   DNS æœåŠ¡å™¨ä¸ºå®¢æˆ·ç«¯ï¼ˆåŒ…æ‹¬ iOSï¼‰è§£æžã€‚

 æ”¯æŒä»»ä½•å‘çŽ°åŸŸåï¼›`.internal.` åªæ˜¯ä¸€ä¸ªç¤ºä¾‹ã€‚
...
- ä»Ž `~/./dns/<domain>.db` æä¾›ä½ é€‰æ‹©çš„åŸŸåæœåŠ¡ï¼ˆç¤ºä¾‹ï¼š`.internal.`ï¼‰
...
ä½ çš„å‘çŽ°åŸŸåä¸­æµè§ˆ `_-gw._tcp`ï¼Œæ— éœ€å¤šæ’­ã€‚
...
- åœ¨ `~/./.json` ä¸­è®¾ç½® `gateway.bind: "tailnet"`ã€‚
- é‡å¯ Gateway ç½‘å…³ï¼ˆæˆ–é‡å¯ macOS èœå•æ åº”ç”¨ï¼‰ã€‚

## ä»€ä¹ˆåœ¨å¹¿æ’­

åªæœ‰ Gateway ç½‘å…³å¹¿æ’­ `_-gw._tcp`ã€‚

## æœåŠ¡ç±»åž‹

- `_-gw._tcp` â€” Gateway ç½‘å…³ä¼ è¾“ä¿¡æ ‡ï¼ˆè¢« macOS/iOS/Android èŠ‚ç‚¹ä½¿ç”¨ï¼‰ã€‚
...
- `transport=gateway`
- `cliPath=<path>`ï¼ˆå¯é€‰ï¼›å¯è¿è¡Œçš„ `` å…¥å£ç‚¹çš„ç»å¯¹è·¯å¾„ï¼‰
...
iOS èŠ‚ç‚¹ä½¿ç”¨ `NWBrowser` æ¥å‘çŽ° `_-gw._tcp`ã€‚
...
- `_DISABLE_BONJOUR=1` ç¦ç”¨å¹¿æ’­ï¼ˆæ—§ç‰ˆï¼š`_DISABLE_BONJOUR`ï¼‰ã€‚
- `~/./.json` ä¸­çš„ `gateway.bind` æŽ§åˆ¶ Gateway ç½‘å…³ç»‘å®šæ¨¡å¼ã€‚
- `_SSH_PORT` è¦†ç›– TXT ä¸­å¹¿æ’­çš„ SSH ç«¯å£ï¼ˆæ—§ç‰ˆï¼š`_SSH_PORT`ï¼‰ã€‚
- `_TAILNET_DNS` åœ¨ TXT ä¸­å‘å¸ƒ MagicDNS æç¤ºï¼ˆæ—§ç‰ˆï¼š`_TAILNET_DNS`ï¼‰ã€‚
- `_CLI_PATH` è¦†ç›–å¹¿æ’­çš„ CLI è·¯å¾„ï¼ˆæ—§ç‰ˆï¼š`_CLI_PATH`ï¼‰ã€‚
