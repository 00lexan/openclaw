...
- **ç¤ºä¾‹ï¼š** [exe.dev](/platforms/exe-dev)ï¼ˆç®€æ˜“ VMï¼‰æˆ– [Hetzner](/platforms/hetzner)ï¼ˆç”Ÿäº§ VPSï¼‰ã€‚
...
- ä½¿ç”¨ macOS åº”ç”¨çš„ **Remote over SSH** æ¨¡å¼ï¼ˆè®¾ç½® â†’ é€šç”¨ â†’ " runs"ï¼‰ã€‚
...
- ` health` å’Œ ` status --deep` çŽ°åœ¨é€šè¿‡ `ws://127.0.0.1:18789` è®¿é—®è¿œç¨‹ Gateway ç½‘å…³ã€‚
- ` gateway {status,health,send,agent,call}` åœ¨éœ€è¦æ—¶ä¹Ÿå¯ä»¥é€šè¿‡ `--url` æŒ‡å®šè½¬å‘çš„ URLã€‚

æ³¨æ„ï¼šå°† `18789` æ›¿æ¢ä¸ºä½ é…ç½®çš„ `gateway.port`ï¼ˆæˆ– `--port`/`_GATEWAY_PORT`ï¼‰ã€‚
