...
# è®¾å¤‡å‘çŽ° & ä¼ è¾“åè®®
 ...
 æœ‰ä¸¤ä¸ªè¡¨é¢ä¸Šçœ‹èµ·æ¥ç›¸ä¼¼çš„ä¸åŒé—®é¢˜ï¼š
...
è®¾è®¡ç›®æ ‡æ˜¯å°†æ‰€æœ‰ç½‘ç»œå‘çŽ°/å¹¿æ’­ä¿ç•™åœ¨ **Node Gateway ç½‘å…³**ï¼ˆ` gateway`ï¼‰ä¸­ï¼Œå¹¶è®©å®¢æˆ·ç«¯ï¼ˆmac åº”ç”¨ã€iOSï¼‰ä½œä¸ºæ¶ˆè´¹è€…ã€‚
...
- æœåŠ¡ç±»åž‹ï¼š
  - `_-gw._tcp`ï¼ˆGateway ç½‘å…³ä¼ è¾“ä¿¡æ ‡ï¼‰
- TXT é”®ï¼ˆéžæœºå¯†ï¼‰ï¼š
  ...
  - `canvasPort=18793`ï¼ˆé»˜è®¤ç”»å¸ƒä¸»æœºç«¯å£ï¼›æœåŠ¡äºŽ `/____/canvas/`ï¼‰
  - `cliPath=<path>`ï¼ˆå¯é€‰ï¼›å¯è¿è¡Œçš„ `` å…¥å£ç‚¹æˆ–äºŒè¿›åˆ¶æ–‡ä»¶çš„ç»å¯¹è·¯å¾„ï¼‰
  - `tailnetDns=<magicdns>`ï¼ˆå¯é€‰æç¤ºï¼›å½“ Tailscale å¯ç”¨æ—¶è‡ªåŠ¨æ£€æµ‹ï¼‰

ç¦ç”¨/è¦†ç›–ï¼š

- `_DISABLE_BONJOUR=1` ç¦ç”¨å¹¿æ’­ã€‚
- `~/./.json` ä¸­çš„ `gateway.bind` æŽ§åˆ¶ Gateway ç½‘å…³ç»‘å®šæ¨¡å¼ã€‚
- `_SSH_PORT` è¦†ç›– TXT ä¸­å¹¿æ’­çš„ SSH ç«¯å£ï¼ˆé»˜è®¤ä¸º 22ï¼‰ã€‚
- `_TAILNET_DNS` å‘å¸ƒ `tailnetDns` æç¤ºï¼ˆMagicDNSï¼‰ã€‚
- `_CLI_PATH` è¦†ç›–å¹¿æ’­çš„ CLI è·¯å¾„ã€‚
