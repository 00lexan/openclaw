...
- [ ] æ›´æ–° CLI/ç‰ˆæœ¬å­—ç¬¦ä¸²ï¼š[`src/cli/program.ts`](https://github.com///blob/main/src/cli/program.ts) å’Œ [`src/provider-web.ts`](https://github.com///blob/main/src/provider-web.ts) ä¸­çš„ Baileys user agentã€‚
- [ ] ç¡®è®¤åŒ…å…ƒæ•°æ®ï¼ˆnameã€descriptionã€repositoryã€keywordsã€licenseï¼‰ä»¥åŠ `bin` æ˜ å°„æŒ‡å‘ [`.mjs`](https://github.com///blob/main/.mjs) ä½œä¸º ``ã€‚
- [ ] å¦‚æžœä¾èµ–é¡¹æœ‰å˜åŒ–ï¼Œè¿è¡Œ `pnpm install` ç¡®ä¿ `pnpm-lock.yaml` æ˜¯æœ€æ–°çš„ã€‚

2. **æž„å»ºå’Œäº§ç‰©**

- [ ] å¦‚æžœ A2UI è¾“å…¥æœ‰å˜åŒ–ï¼Œè¿è¡Œ `pnpm canvas:a2ui:bundle` å¹¶æäº¤æ›´æ–°åŽçš„ [`src/canvas-host/a2ui/a2ui.bundle.js`](https://github.com///blob/main/src/canvas-host/a2ui/a2ui.bundle.js)ã€‚
...
- [ ] `_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`ï¼ˆDocker å®‰è£…å†’çƒŸæµ‹è¯•ï¼Œå¿«é€Ÿè·¯å¾„ï¼›å‘å¸ƒå‰å¿…éœ€ï¼‰
  - å¦‚æžœå·²çŸ¥ä¸Šä¸€ä¸ª npm å‘å¸ƒç‰ˆæœ¬æœ‰é—®é¢˜ï¼Œä¸ºé¢„å®‰è£…æ­¥éª¤è®¾ç½® `_INSTALL_SMOKE_PREVIOUS=<last-good-version>` æˆ– `_INSTALL_SMOKE_SKIP_PREVIOUS=1`ã€‚
- [ ]ï¼ˆå¯é€‰ï¼‰å®Œæ•´å®‰è£…ç¨‹åºå†’çƒŸæµ‹è¯•ï¼ˆæ·»åŠ éž root + CLI è¦†ç›–ï¼‰ï¼š`pnpm test:install:smoke`
- [ ]ï¼ˆå¯é€‰ï¼‰å®‰è£…ç¨‹åº E2Eï¼ˆDockerï¼Œè¿è¡Œ `curl -fsSL https://.ai/install.sh | bash`ï¼Œæ–°æ‰‹å¼•å¯¼ï¼Œç„¶åŽè¿è¡ŒçœŸå®žå·¥å…·è°ƒç”¨ï¼‰ï¼š
...
- [ ] ç”Ÿæˆ Sparkle appcastï¼ˆé€šè¿‡ [`scripts/make_appcast.sh`](https://github.com///blob/main/scripts/make_appcast.sh) ç”Ÿæˆ HTML æ³¨é‡Šï¼‰å¹¶æ›´æ–° `appcast.xml`ã€‚
...
- [ ] æŒ‰ç…§ [macOS å‘å¸ƒ](/platforms/mac/release) èŽ·å–ç¡®åˆ‡å‘½ä»¤å’Œæ‰€éœ€çŽ¯å¢ƒå˜é‡ã€‚
  ...
  - å¦‚æžœè¿›è¡Œå…¬è¯ï¼Œä½¿ç”¨ä»Ž App Store Connect API çŽ¯å¢ƒå˜é‡åˆ›å»ºçš„ `-notary` é’¥åŒ™ä¸²é…ç½®æ–‡ä»¶ï¼ˆå‚è§ [macOS å‘å¸ƒ](/platforms/mac/release)ï¼‰ã€‚
...
- [ ] éªŒè¯æ³¨å†Œè¡¨ï¼š`npm view  version`ã€`npm view  dist-tags` å’Œ `npx -y @X.Y.Z --version`ï¼ˆæˆ– `--help`ï¼‰ã€‚

### æ•…éšœæŽ’é™¤ï¼ˆæ¥è‡ª 2.0.0-beta2 å‘å¸ƒçš„ç¬”è®°ï¼‰

- **npm pack/publish æŒ‚èµ·æˆ–äº§ç”Ÿå·¨å¤§ tarball**ï¼š`dist/.app` ä¸­çš„ macOS åº”ç”¨åŒ…ï¼ˆå’Œå‘å¸ƒ zipï¼‰è¢«æ‰«å…¥åŒ…ä¸­ã€‚é€šè¿‡ `package.json` çš„ `files` ç™½åå•å‘å¸ƒå†…å®¹æ¥ä¿®å¤ï¼ˆåŒ…å« dist å­ç›®å½•ã€docsã€skillsï¼›æŽ’é™¤åº”ç”¨åŒ…ï¼‰ã€‚ç”¨ `npm pack --dry-run` ç¡®è®¤ `dist/.app` æœªåˆ—å‡ºã€‚
...
- [ ] ä¸º `vX.Y.Z` åˆ›å»º/åˆ·æ–° GitHub å‘å¸ƒï¼Œ**æ ‡é¢˜ä¸º ` X.Y.Z`**ï¼ˆä¸ä»…ä»…æ˜¯æ ‡ç­¾ï¼‰ï¼›æ­£æ–‡åº”åŒ…å«è¯¥ç‰ˆæœ¬çš„**å®Œæ•´**å˜æ›´æ—¥å¿—éƒ¨åˆ†ï¼ˆäº®ç‚¹ + æ›´æ”¹ + ä¿®å¤ï¼‰ï¼Œå†…è”æ˜¾ç¤ºï¼ˆæ— è£¸é“¾æŽ¥ï¼‰ï¼Œä¸”**ä¸å¾—åœ¨æ­£æ–‡ä¸­é‡å¤æ ‡é¢˜**ã€‚
- [ ] é™„åŠ äº§ç‰©ï¼š`npm pack` tarballï¼ˆå¯é€‰ï¼‰ã€`-X.Y.Z.zip` å’Œ `-X.Y.Z.dSYM.zip`ï¼ˆå¦‚æžœç”Ÿæˆï¼‰ã€‚
- [ ] æäº¤æ›´æ–°åŽçš„ `appcast.xml` å¹¶æŽ¨é€ï¼ˆSparkle ä»Ž main èŽ·å–æºï¼‰ã€‚
- [ ] ä»Žå¹²å‡€çš„ä¸´æ—¶ç›®å½•ï¼ˆæ—  `package.json`ï¼‰ï¼Œè¿è¡Œ `npx -y @X.Y.Z send --help` ç¡®è®¤å®‰è£…/CLI å…¥å£ç‚¹æ­£å¸¸å·¥ä½œã€‚
- [ ] å®£å¸ƒ/åˆ†äº«å‘å¸ƒè¯´æ˜Žã€‚

## æ’ä»¶å‘å¸ƒèŒƒå›´ï¼ˆnpmï¼‰

æˆ‘ä»¬åªå‘å¸ƒ `@/*` èŒƒå›´ä¸‹çš„**çŽ°æœ‰ npm æ’ä»¶**ã€‚ä¸åœ¨ npm ä¸Šçš„å†…ç½®æ’ä»¶ä¿æŒ**ä»…ç£ç›˜æ ‘**ï¼ˆä»åœ¨ `extensions/**` ä¸­å‘å¸ƒï¼‰ã€‚

èŽ·å–åˆ—è¡¨çš„æµç¨‹ï¼š

1. `npm search @ --json` å¹¶æ•èŽ·åŒ…åã€‚
