...
summary:  ä»Žå“ªé‡ŒåŠ è½½çŽ¯å¢ƒå˜é‡ä»¥åŠä¼˜å…ˆçº§é¡ºåº
title: çŽ¯å¢ƒå˜é‡
x-i18n:
...
# çŽ¯å¢ƒå˜é‡
 ...
 ä»Žå¤šä¸ªæ¥æºæ‹‰å–çŽ¯å¢ƒå˜é‡ã€‚è§„åˆ™æ˜¯**æ°¸ä¸è¦†ç›–çŽ°æœ‰å€¼**ã€‚
...
3. **å…¨å±€ `.env`** ä½äºŽ `~/./.env`ï¼ˆå³ `$_STATE_DIR/.env`ï¼›ä¸è¦†ç›–ï¼‰ã€‚
4. **é…ç½® `env` å—** ä½äºŽ `~/./.json`ï¼ˆä»…åœ¨ç¼ºå¤±æ—¶åº”ç”¨ï¼‰ã€‚
5. **å¯é€‰çš„ç™»å½• shell å¯¼å…¥**ï¼ˆ`env.shellEnv.enabled` æˆ– `_LOAD_SHELL_ENV=1`ï¼‰ï¼Œä»…å¯¹ç¼ºå¤±çš„é¢„æœŸé”®ååº”ç”¨ã€‚
