...
summary: "` models` çš„ CLI å‚è€ƒï¼ˆstatus/list/set/scanã€åˆ«åã€å›žé€€ã€è®¤è¯ï¼‰"
title: models
x-i18n:
...
` models status` æ˜¾ç¤ºå·²è§£æžçš„é»˜è®¤æ¨¡åž‹/å›žé€€é…ç½®ä»¥åŠè®¤è¯æ¦‚è§ˆã€‚
...
å‘½ä»¤ä¼šä½¿ç”¨ `_AGENT_DIR`/`PI_CODING_AGENT_DIR`ï¼ˆå¦‚å·²è®¾ç½®ï¼‰ï¼Œå¦åˆ™ä½¿ç”¨
...
- å¦‚æžœçœç•¥æä¾›å•†ï¼Œ ä¼šå°†è¾“å…¥è§†ä¸ºåˆ«åæˆ–**é»˜è®¤æä¾›å•†**çš„æ¨¡åž‹ï¼ˆä»…åœ¨æ¨¡åž‹ ID ä¸åŒ…å« `/` æ—¶æœ‰æ•ˆï¼‰ã€‚
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>`ï¼ˆå·²é…ç½®çš„æ™ºèƒ½ä½“ IDï¼›è¦†ç›– `_AGENT_DIR`/`PI_CODING_AGENT_DIR`ï¼‰
...
` plugins list` æŸ¥çœ‹å·²å®‰è£…çš„æä¾›å•†ã€‚
