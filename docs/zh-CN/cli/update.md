...
summary: "` update` çš„ CLI å‚è€ƒï¼ˆç›¸å¯¹å®‰å…¨çš„æºç æ›´æ–° + Gateway ç½‘å…³è‡ªåŠ¨é‡å¯ï¼‰"
title: update
x-i18n:
...
å®‰å…¨æ›´æ–°  å¹¶åœ¨ stable/beta/dev æ¸ é“ä¹‹é—´åˆ‡æ¢ã€‚
...
å½“ä½ æ˜¾å¼åˆ‡æ¢æ¸ é“ï¼ˆ`--channel ...`ï¼‰æ—¶ï¼Œ ä¹Ÿä¼šä¿æŒå®‰è£…æ–¹å¼ä¸€è‡´ï¼š

- `dev` â†’ ç¡®ä¿å­˜åœ¨ git æ£€å‡ºï¼ˆé»˜è®¤ï¼š`~/`ï¼Œå¯é€šè¿‡ `_GIT_DIR` è¦†ç›–ï¼‰ï¼Œæ›´æ–°å®ƒï¼Œå¹¶ä»Žè¯¥æ£€å‡ºå®‰è£…å…¨å±€ CLIã€‚
...
8. è¿è¡Œ ` doctor` ä½œä¸ºæœ€ç»ˆçš„"å®‰å…¨æ›´æ–°"æ£€æŸ¥ã€‚
9. å°†æ’ä»¶åŒæ­¥åˆ°å½“å‰æ¸ é“ï¼ˆdev ä½¿ç”¨æ†ç»‘çš„æ‰©å±•ï¼›stable/beta ä½¿ç”¨ npmï¼‰å¹¶æ›´æ–° npm å®‰è£…çš„æ’ä»¶ã€‚

## `--update` ç®€å†™

` --update` ä¼šé‡å†™ä¸º ` update`ï¼ˆä¾¿äºŽ shell å’Œå¯åŠ¨è„šæœ¬ä½¿ç”¨ï¼‰ã€‚

## å¦è¯·å‚é˜…

- ` doctor`ï¼ˆåœ¨ git æ£€å‡ºä¸Šä¼šæä¾›å…ˆè¿è¡Œæ›´æ–°çš„é€‰é¡¹ï¼‰
