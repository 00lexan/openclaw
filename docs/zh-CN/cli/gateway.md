...
summary:  Gateway ç½‘å…³ CLIï¼ˆ` gateway`ï¼‰â€” è¿è¡Œã€æŸ¥è¯¢å’Œå‘çŽ° Gateway ç½‘å…³
title: gateway
x-i18n:
...
Gateway ç½‘å…³æ˜¯  çš„ WebSocket æœåŠ¡å™¨ï¼ˆæ¸ é“ã€èŠ‚ç‚¹ã€ä¼šè¯ã€hooksï¼‰ã€‚

æœ¬é¡µä¸­çš„å­å‘½ä»¤ä½äºŽ ` gateway â€¦` ä¸‹ã€‚
...
- é»˜è®¤æƒ…å†µä¸‹ï¼Œé™¤éžåœ¨ `~/./.json` ä¸­è®¾ç½®äº† `gateway.mode=local`ï¼Œå¦åˆ™ Gateway ç½‘å…³å°†æ‹’ç»å¯åŠ¨ã€‚ä½¿ç”¨ `--allow-unconfigured` è¿›è¡Œä¸´æ—¶/å¼€å‘è¿è¡Œã€‚
...
- `--token <token>`ï¼šä»¤ç‰Œè¦†ç›–ï¼ˆåŒæ—¶ä¸ºè¿›ç¨‹è®¾ç½® `_GATEWAY_TOKEN`ï¼‰ã€‚
- `--password <password>`ï¼šå¯†ç è¦†ç›–ï¼ˆåŒæ—¶ä¸ºè¿›ç¨‹è®¾ç½® `_GATEWAY_PASSWORD`ï¼‰ã€‚
...
`gateway discover` æ‰«æ Gateway ç½‘å…³ä¿¡æ ‡ï¼ˆ`_-gw._tcp`ï¼‰ã€‚

- ç»„æ’­ DNS-SDï¼š`local.`
- å•æ’­ DNS-SDï¼ˆå¹¿åŸŸ Bonjourï¼‰ï¼šé€‰æ‹©ä¸€ä¸ªåŸŸï¼ˆç¤ºä¾‹ï¼š`.internal.`ï¼‰å¹¶è®¾ç½®åˆ†å‰² DNS + DNS æœåŠ¡å™¨ï¼›å‚è§ [/gateway/bonjour](/gateway/bonjour)
