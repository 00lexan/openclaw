...
summary:  çš„å¯é€‰ Docker è®¾ç½®å’Œæ–°æ‰‹å¼•å¯¼
title: Docker
x-i18n:
...
- **æ˜¯**ï¼šä½ æƒ³è¦ä¸€ä¸ªéš”ç¦»çš„ã€å¯ä¸¢å¼ƒçš„ Gateway ç½‘å…³çŽ¯å¢ƒï¼Œæˆ–åœ¨æ²¡æœ‰æœ¬åœ°å®‰è£…çš„ä¸»æœºä¸Šè¿è¡Œ ã€‚
...
- å®¹å™¨åŒ– Gateway ç½‘å…³ï¼ˆå®Œæ•´çš„  åœ¨ Docker ä¸­ï¼‰
...
- `_DOCKER_APT_PACKAGES` â€” åœ¨æž„å»ºæœŸé—´å®‰è£…é¢å¤–çš„ apt åŒ…
- `_EXTRA_MOUNTS` â€” æ·»åŠ é¢å¤–çš„ä¸»æœºç»‘å®šæŒ‚è½½
- `_HOME_VOLUME` â€” åœ¨å‘½åå·ä¸­æŒä¹…åŒ– `/home/node`

å®ŒæˆåŽï¼š

- åœ¨æµè§ˆå™¨ä¸­æ‰“å¼€ `http://127.0.0.1:18789/`ã€‚
- å°†ä»¤ç‰Œç²˜è´´åˆ°æŽ§åˆ¶ UIï¼ˆè®¾ç½® â†’ tokenï¼‰ã€‚
- éœ€è¦å†æ¬¡èŽ·å–å¸¦ä»¤ç‰Œçš„ URLï¼Ÿè¿è¡Œ `docker compose run --rm -cli dashboard --no-open`ã€‚
...
åœ¨ VPS ä¸Šè¿è¡Œï¼Ÿå‚é˜… [Hetznerï¼ˆDocker VPSï¼‰](/platforms/hetzner)ã€‚
...
æ³¨æ„ï¼šä»Žä»“åº“æ ¹ç›®å½•è¿è¡Œ `docker compose ...`ã€‚å¦‚æžœä½ å¯ç”¨äº† `_EXTRA_MOUNTS` æˆ– `_HOME_VOLUME`ï¼Œè®¾ç½®è„šæœ¬ä¼šå†™å…¥ `docker-compose.extra.yml`ï¼›åœ¨å…¶ä»–åœ°æ–¹è¿è¡Œ Compose æ—¶åŒ…å«å®ƒï¼š
...
docker compose run --rm -cli devices approve <requestId>
```
...
å¦‚æžœä½ æƒ³å°†é¢å¤–çš„ä¸»æœºç›®å½•æŒ‚è½½åˆ°å®¹å™¨ä¸­ï¼Œåœ¨è¿è¡Œ `docker-setup.sh` ä¹‹å‰è®¾ç½® `_EXTRA_MOUNTS`ã€‚è¿™æŽ¥å—é€—å·åˆ†éš”çš„ Docker ç»‘å®šæŒ‚è½½åˆ—è¡¨ï¼Œå¹¶é€šè¿‡ç”Ÿæˆ `docker-compose.extra.yml` å°†å®ƒä»¬åº”ç”¨åˆ° `-gateway` å’Œ `-cli`ã€‚

ç¤ºä¾‹ï¼š

```bash
export _EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- å¦‚æžœä½ ç¼–è¾‘ `_EXTRA_MOUNTS`ï¼Œé‡æ–°è¿è¡Œ `docker-setup.sh` ä»¥é‡æ–°ç”Ÿæˆé¢å¤–çš„ compose æ–‡ä»¶ã€‚
- `docker-compose.extra.yml` æ˜¯ç”Ÿæˆçš„ã€‚ä¸è¦æ‰‹åŠ¨ç¼–è¾‘å®ƒã€‚

### æŒä¹…åŒ–æ•´ä¸ªå®¹å™¨ homeï¼ˆå¯é€‰ï¼‰

å¦‚æžœä½ æƒ³è®© `/home/node` åœ¨å®¹å™¨é‡å»ºåŽæŒä¹…åŒ–ï¼Œé€šè¿‡ `_HOME_VOLUME` è®¾ç½®ä¸€ä¸ªå‘½åå·ã€‚è¿™ä¼šåˆ›å»ºä¸€ä¸ª Docker å·å¹¶å°†å…¶æŒ‚è½½åˆ° `/home/node`ï¼ŒåŒæ—¶ä¿æŒæ ‡å‡†çš„é…ç½®/å·¥ä½œåŒºç»‘å®šæŒ‚è½½ã€‚è¿™é‡Œä½¿ç”¨å‘½åå·ï¼ˆä¸æ˜¯ç»‘å®šè·¯å¾„ï¼‰ï¼›å¯¹äºŽç»‘å®šæŒ‚è½½ï¼Œä½¿ç”¨ `_EXTRA_MOUNTS`ã€‚

ç¤ºä¾‹ï¼š

```bash
export _HOME_VOLUME="_home"
./docker-setup.sh
```
...
- å¦‚æžœä½ æ›´æ”¹ `_HOME_VOLUME`ï¼Œé‡æ–°è¿è¡Œ `docker-setup.sh` ä»¥é‡æ–°ç”Ÿæˆé¢å¤–çš„ compose æ–‡ä»¶ã€‚
- å‘½åå·ä¼šæŒä¹…åŒ–ç›´åˆ°ä½¿ç”¨ `docker volume rm <name>` åˆ é™¤ã€‚

### å®‰è£…é¢å¤–çš„ apt åŒ…ï¼ˆå¯é€‰ï¼‰

å¦‚æžœä½ éœ€è¦é•œåƒå†…çš„ç³»ç»ŸåŒ…ï¼ˆä¾‹å¦‚æž„å»ºå·¥å…·æˆ–åª’ä½“åº“ï¼‰ï¼Œåœ¨è¿è¡Œ `docker-setup.sh` ä¹‹å‰è®¾ç½® `_DOCKER_APT_PACKAGES`ã€‚è¿™ä¼šåœ¨é•œåƒæž„å»ºæœŸé—´å®‰è£…åŒ…ï¼Œå› æ­¤å³ä½¿å®¹å™¨è¢«åˆ é™¤å®ƒä»¬ä¹Ÿä¼šæŒä¹…åŒ–ã€‚

ç¤ºä¾‹ï¼š

```bash
export _DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- å¦‚æžœä½ æ›´æ”¹ `_DOCKER_APT_PACKAGES`ï¼Œé‡æ–°è¿è¡Œ `docker-setup.sh` ä»¥é‡å»ºé•œåƒã€‚
...
```bash
export _DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
å¦‚æžœä½ éœ€è¦ Playwright å®‰è£…ç³»ç»Ÿä¾èµ–ï¼Œä½¿ç”¨ `_DOCKER_APT_PACKAGES` é‡å»ºé•œåƒï¼Œè€Œä¸æ˜¯åœ¨è¿è¡Œæ—¶ä½¿ç”¨ `--with-deps`ã€‚

4. **æŒä¹…åŒ– Playwright æµè§ˆå™¨ä¸‹è½½**ï¼š

- åœ¨ `docker-compose.yml` ä¸­è®¾ç½® `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright`ã€‚
- ç¡®ä¿ `/home/node` é€šè¿‡ `_HOME_VOLUME` æŒä¹…åŒ–ï¼Œæˆ–é€šè¿‡ `_EXTRA_MOUNTS` æŒ‚è½½ `/home/node/.cache/ms-playwright`ã€‚

### æƒé™ + EACCES

é•œåƒä»¥ `node`ï¼ˆuid 1000ï¼‰è¿è¡Œã€‚å¦‚æžœä½ åœ¨ `/home/node/.` ä¸Šçœ‹åˆ°æƒé™é”™è¯¯ï¼Œç¡®ä¿ä½ çš„ä¸»æœºç»‘å®šæŒ‚è½½ç”± uid 1000 æ‹¥æœ‰ã€‚
...
```bash
docker compose run --rm -cli channels add --channel telegram --token "<token>"
```

Discordï¼ˆbot tokenï¼‰ï¼š

```bash
docker compose run --rm -cli channels add --channel discord --token "<token>"
```
...
```bash
docker compose exec -gateway node dist/index.js health --token "$_GATEWAY_TOKEN"
```
...
- Gateway ç½‘å…³å®¹å™¨æ˜¯ä¼šè¯çš„çœŸå®žæ¥æºï¼ˆ`~/./agents/<agentId>/sessions/`ï¼‰ã€‚
...
å‚é˜…[å¤šæ™ºèƒ½ä½“æ²™ç®±ä¸Žå·¥å…·](/multi-agent-sandbox-tools)äº†è§£ç¤ºä¾‹ã€ä¼˜å…ˆçº§å’Œæ•…éšœæŽ’é™¤ã€‚

### é»˜è®¤è¡Œä¸º

- é•œåƒï¼š`-sandbox:bookworm-slim`
- æ¯ä¸ªæ™ºèƒ½ä½“ä¸€ä¸ªå®¹å™¨
- æ™ºèƒ½ä½“å·¥ä½œåŒºè®¿é—®ï¼š`workspaceAccess: "none"`ï¼ˆé»˜è®¤ï¼‰ä½¿ç”¨ `~/./sandboxes`
...
- `user` å¿…é¡»æ˜¯ root æ‰èƒ½è¿è¡Œ `apt-get`ï¼ˆçœç•¥ `user` æˆ–è®¾ç½® `user: "0:0"`ï¼‰ã€‚
  å½“ `setupCommand`ï¼ˆæˆ– docker é…ç½®ï¼‰æ›´æ”¹æ—¶ï¼Œ ä¼šè‡ªåŠ¨é‡å»ºå®¹å™¨ï¼Œé™¤éžå®¹å™¨æ˜¯**æœ€è¿‘ä½¿ç”¨çš„**ï¼ˆåœ¨çº¦ 5 åˆ†é’Ÿå†…ï¼‰ã€‚çƒ­å®¹å™¨ä¼šè®°å½•è­¦å‘Šï¼ŒåŒ…å«ç¡®åˆ‡çš„ ` sandbox recreate ...` å‘½ä»¤ã€‚
...
è¿™ä½¿ç”¨ `Dockerfile.sandbox` æž„å»º `-sandbox:bookworm-slim`ã€‚
...
è¿™æž„å»º `-sandbox-common:bookworm-slim`ã€‚è¦ä½¿ç”¨å®ƒï¼š

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
è¿™ä½¿ç”¨ `Dockerfile.sandbox-browser` æž„å»º `-sandbox-browser:bookworm-slim`ã€‚å®¹å™¨è¿è¡Œå¯ç”¨ CDP çš„ Chromium å’Œå¯é€‰çš„ noVNC è§‚å¯Ÿå™¨ï¼ˆé€šè¿‡ Xvfb æœ‰å¤´ï¼‰ã€‚
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my--browser" } },
    },
  },
}
```
...
```bash
docker build -t my--sbx -f Dockerfile.sandbox .
```
...
- é•œåƒç¼ºå¤±ï¼šä½¿ç”¨ [`scripts/sandbox-setup.sh`](https://github.com///blob/main/scripts/sandbox-setup.sh) æž„å»ºæˆ–è®¾ç½® `agents.defaults.sandbox.docker.image`ã€‚
