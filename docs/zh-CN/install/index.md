...
read_when:
  - å®‰è£… 
  - ä½ æƒ³ä»Ž GitHub å®‰è£…
summary: å®‰è£… ï¼ˆæŽ¨èå®‰è£…å™¨ã€å…¨å±€å®‰è£…æˆ–ä»Žæºä»£ç å®‰è£…ï¼‰
title: å®‰è£…
x-i18n:
...
é€šè¿‡ npm å…¨å±€å®‰è£… `` å¹¶è¿è¡Œæ–°æ‰‹å¼•å¯¼ã€‚
...
```bash
curl -fsSL https://.ai/install.sh | bash -s -- --no-onboard
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g @latest
```
...
```bash
pnpm add -g @latest
pnpm approve-builds -g                # æ‰¹å‡† ã€node-llama-cppã€sharp ç­‰
pnpm add -g @latest           # é‡æ–°è¿è¡Œä»¥æ‰§è¡Œ postinstall è„šæœ¬
```
...
æç¤ºï¼šå¦‚æžœä½ è¿˜æ²¡æœ‰å…¨å±€å®‰è£…ï¼Œè¯·é€šè¿‡ `pnpm  ...` è¿è¡Œä»“åº“å‘½ä»¤ã€‚
...
- è¿è¡Œæ–°æ‰‹å¼•å¯¼ï¼š` onboard --install-daemon`
- å¿«é€Ÿæ£€æŸ¥ï¼š` doctor`
- æ£€æŸ¥ Gateway ç½‘å…³å¥åº·çŠ¶æ€ï¼š` status` + ` health`
- æ‰“å¼€ä»ªè¡¨æ¿ï¼š` dashboard`
...
```bash
# æ˜¾å¼ npm
curl -fsSL https://.ai/install.sh | bash -s -- --install-method npm
...
å¦‚æžœ `$(npm prefix -g)/bin`ï¼ˆmacOS/Linuxï¼‰æˆ– `$(npm prefix -g)`ï¼ˆWindowsï¼‰**ä¸**åœ¨ `echo "$PATH"` çš„è¾“å‡ºä¸­ï¼Œä½ çš„ shell æ— æ³•æ‰¾åˆ°å…¨å±€ npm äºŒè¿›åˆ¶æ–‡ä»¶ï¼ˆåŒ…æ‹¬ ``ï¼‰ã€‚
