---
read_when:
  - ä½ æ­£åœ¨å°†  è¿ç§»åˆ°æ–°çš„ç¬”è®°æœ¬ç”µè„‘/æœåŠ¡å™¨
  - ä½ æƒ³ä¿ç•™ä¼šè¯ã€è®¤è¯å’Œæ¸ é“ç™»å½•ï¼ˆWhatsApp ç­‰ï¼‰
summary: å°†  å®‰è£…ä»Žä¸€å°æœºå™¨è¿ç§»åˆ°å¦ä¸€å°
title: è¿ç§»æŒ‡å—
x-i18n:
...
æœ¬æŒ‡å—å°†  Gateway ç½‘å…³ä»Žä¸€å°æœºå™¨è¿ç§»åˆ°å¦ä¸€å°ï¼Œ**æ— éœ€é‡æ–°è¿›è¡Œæ–°æ‰‹å¼•å¯¼**ã€‚

è¿ç§»åœ¨æ¦‚å¿µä¸Šå¾ˆç®€å•ï¼š

- å¤åˆ¶**çŠ¶æ€ç›®å½•**ï¼ˆ`$_STATE_DIR`ï¼Œé»˜è®¤ï¼š`~/./`ï¼‰â€” è¿™åŒ…æ‹¬é…ç½®ã€è®¤è¯ã€ä¼šè¯å’Œæ¸ é“çŠ¶æ€ã€‚
- å¤åˆ¶ä½ çš„**å·¥ä½œåŒº**ï¼ˆé»˜è®¤ `~/./workspace/`ï¼‰â€” è¿™åŒ…æ‹¬ä½ çš„æ™ºèƒ½ä½“æ–‡ä»¶ï¼ˆè®°å¿†ã€æç¤ºç­‰ï¼‰ã€‚
...
- `--profile <name>`ï¼ˆé€šå¸¸å˜æˆ `~/.-<profile>/`ï¼‰
...
åœ¨è¾“å‡ºä¸­æŸ¥æ‰¾ `_STATE_DIR` / profile çš„æåŠã€‚å¦‚æžœä½ è¿è¡Œå¤šä¸ª Gateway ç½‘å…³ï¼Œå¯¹æ¯ä¸ªé…ç½®æ–‡ä»¶é‡å¤æ­¤æ“ä½œã€‚

### 2ï¼‰ç¡®å®šä½ çš„å·¥ä½œåŒº

å¸¸è§é»˜è®¤å€¼ï¼š

- `~/./workspace/`ï¼ˆæŽ¨èçš„å·¥ä½œåŒºï¼‰
...
å¦‚æžœä½ æœ‰å¤šä¸ªé…ç½®æ–‡ä»¶/çŠ¶æ€ç›®å½•ï¼ˆä¾‹å¦‚ `~/.-main`ã€`~/.-work`ï¼‰ï¼Œåˆ†åˆ«å½’æ¡£æ¯ä¸ªã€‚

### æ­¥éª¤ 1 â€” åœ¨æ–°æœºå™¨ä¸Šå®‰è£… 

åœ¨**æ–°**æœºå™¨ä¸Šï¼Œå®‰è£… CLIï¼ˆå¦‚æžœéœ€è¦è¿˜æœ‰ Nodeï¼‰ï¼š

- å‚è§ï¼š[å®‰è£…](/install)

åœ¨è¿™ä¸ªé˜¶æ®µï¼Œå¦‚æžœæ–°æ‰‹å¼•å¯¼åˆ›å»ºäº†ä¸€ä¸ªæ–°çš„ `~/./` ä¹Ÿæ²¡å…³ç³» â€” ä½ å°†åœ¨ä¸‹ä¸€æ­¥è¦†ç›–å®ƒã€‚
...
- `$_STATE_DIR`ï¼ˆé»˜è®¤ `~/./`ï¼‰
- ä½ çš„å·¥ä½œåŒºï¼ˆé»˜è®¤ `~/./workspace/`ï¼‰
...
å¦‚æžœä½ åœ¨æ—§ Gateway ç½‘å…³ä¸Šä½¿ç”¨äº†é…ç½®æ–‡ä»¶ï¼ˆæˆ– `_STATE_DIR`ï¼‰ï¼Œè€Œæ–° Gateway ç½‘å…³ä½¿ç”¨äº†ä¸åŒçš„é…ç½®ï¼Œä½ ä¼šçœ‹åˆ°å¦‚ä¸‹ç—‡çŠ¶ï¼š
...
`.json` æ˜¯ä¸å¤Ÿçš„ã€‚è®¸å¤šæä¾›å•†åœ¨ä»¥ä¸‹ä½ç½®å­˜å‚¨çŠ¶æ€ï¼š

- `$_STATE_DIR/credentials/`
- `$_STATE_DIR/agents/<agentId>/...`

å§‹ç»ˆè¿ç§»æ•´ä¸ª `$_STATE_DIR` æ–‡ä»¶å¤¹ã€‚
...
`$_STATE_DIR` åŒ…å«å¯†é’¥ï¼ˆAPI å¯†é’¥ã€OAuth ä»¤ç‰Œã€WhatsApp å‡­è¯ï¼‰ã€‚å°†å¤‡ä»½è§†ä¸ºç”Ÿäº§å¯†é’¥ï¼š
...
- ` status` æ˜¾ç¤º Gateway ç½‘å…³æ­£åœ¨è¿è¡Œ
