...
read_when:
  - æ›´æ–° 
  - æ›´æ–°åŽå‡ºçŽ°é—®é¢˜
summary: å®‰å…¨æ›´æ–° ï¼ˆå…¨å±€å®‰è£…æˆ–æºç ï¼‰ï¼Œä»¥åŠå›žæ»šç­–ç•¥
title: æ›´æ–°
x-i18n:
...
# æ›´æ–°
 ...
 å‘å±•è¿…é€Ÿï¼ˆå°šæœªåˆ°"1.0"ï¼‰ã€‚å°†æ›´æ–°è§†ä¸ºå‘å¸ƒåŸºç¡€è®¾æ–½ï¼šæ›´æ–° â†’ è¿è¡Œæ£€æŸ¥ â†’ é‡å¯ï¼ˆæˆ–ä½¿ç”¨ä¼šé‡å¯çš„ ` update`ï¼‰â†’ éªŒè¯ã€‚

## æŽ¨èï¼šé‡æ–°è¿è¡Œç½‘ç«™å®‰è£…ç¨‹åºï¼ˆåŽŸåœ°å‡çº§ï¼‰

**é¦–é€‰**çš„æ›´æ–°è·¯å¾„æ˜¯é‡æ–°è¿è¡Œç½‘ç«™ä¸Šçš„å®‰è£…ç¨‹åºã€‚å®ƒä¼šæ£€æµ‹çŽ°æœ‰å®‰è£…ã€åŽŸåœ°å‡çº§ï¼Œå¹¶åœ¨éœ€è¦æ—¶è¿è¡Œ ` doctor`ã€‚
...
- å¯¹äºŽ**å…¨å±€å®‰è£…**ï¼Œè„šæœ¬åº•å±‚ä½¿ç”¨ `npm install -g @latest`ã€‚
...
- å¦‚æžœä½ çš„ Gateway ç½‘å…³ä½œä¸ºæœåŠ¡è¿è¡Œï¼Œ` gateway restart` ä¼˜äºŽæ€æ­» PIDã€‚
...
- å®‰è£…ä¾èµ–ã€æž„å»ºã€æž„å»ºæŽ§åˆ¶ UIï¼Œå¹¶è¿è¡Œ ` doctor`ã€‚
- é»˜è®¤é‡å¯ Gateway ç½‘å…³ï¼ˆä½¿ç”¨ `--no-restart` è·³è¿‡ï¼‰ã€‚

å¦‚æžœä½ é€šè¿‡ **npm/pnpm** å®‰è£…ï¼ˆæ²¡æœ‰ git å…ƒæ•°æ®ï¼‰ï¼Œ` update` å°†å°è¯•é€šè¿‡ä½ çš„åŒ…ç®¡ç†å™¨æ›´æ–°ã€‚å¦‚æžœæ— æ³•æ£€æµ‹åˆ°å®‰è£…ï¼Œè¯·æ”¹ç”¨"æ›´æ–°ï¼ˆå…¨å±€å®‰è£…ï¼‰"ã€‚

## æ›´æ–°ï¼ˆæŽ§åˆ¶ UI / RPCï¼‰

æŽ§åˆ¶ UI æœ‰**æ›´æ–°å¹¶é‡å¯**ï¼ˆRPCï¼š`update.run`ï¼‰ã€‚å®ƒï¼š

1. è¿è¡Œä¸Ž ` update` ç›¸åŒçš„æºç æ›´æ–°æµç¨‹ï¼ˆä»…é™ git checkoutï¼‰ã€‚
...
- å½“ä½ è¿è¡Œæ‰“åŒ…çš„ `` äºŒè¿›åˆ¶æ–‡ä»¶ï¼ˆ[`.mjs`](https://github.com///blob/main/.mjs)ï¼‰æˆ–ä½¿ç”¨ Node è¿è¡Œ `dist/` æ—¶ï¼Œ`pnpm build` å¾ˆé‡è¦ã€‚
- å¦‚æžœä½ ä»Žä»“åº“ checkout è¿è¡Œè€Œæ²¡æœ‰å…¨å±€å®‰è£…ï¼ŒCLI å‘½ä»¤ä½¿ç”¨ `pnpm  ...`ã€‚
- å¦‚æžœä½ ç›´æŽ¥ä»Ž TypeScript è¿è¡Œï¼ˆ`pnpm  ...`ï¼‰ï¼Œé€šå¸¸ä¸éœ€è¦é‡æ–°æž„å»ºï¼Œä½†**é…ç½®è¿ç§»ä»ç„¶é€‚ç”¨** â†’ è¿è¡Œ doctorã€‚
- åœ¨å…¨å±€å’Œ git å®‰è£…ä¹‹é—´åˆ‡æ¢å¾ˆå®¹æ˜“ï¼šå®‰è£…å¦ä¸€ç§æ–¹å¼ï¼Œç„¶åŽè¿è¡Œ ` doctor` ä»¥ä¾¿å°† Gateway ç½‘å…³æœåŠ¡å…¥å£ç‚¹é‡å†™ä¸ºå½“å‰å®‰è£…ã€‚

## å§‹ç»ˆè¿è¡Œï¼š` doctor`

Doctor æ˜¯"å®‰å…¨æ›´æ–°"å‘½ä»¤ã€‚å®ƒæ•…æ„å¾ˆæ— èŠï¼šä¿®å¤ + è¿ç§» + è­¦å‘Šã€‚

æ³¨æ„ï¼šå¦‚æžœä½ æ˜¯**æºç å®‰è£…**ï¼ˆgit checkoutï¼‰ï¼Œ` doctor` ä¼šæä¾›å…ˆè¿è¡Œ ` update`ã€‚
...
- æ£€æµ‹å¹¶å°†æ—§ç‰ˆ Gateway ç½‘å…³æœåŠ¡ï¼ˆlaunchd/systemdï¼›æ—§ç‰ˆ schtasksï¼‰è¿ç§»åˆ°å½“å‰  æœåŠ¡ã€‚
...
- macOS launchdï¼ˆåº”ç”¨æ†ç»‘çš„ LaunchAgentï¼‰ï¼š`launchctl kickstart -k gui/$UID/bot.molt.gateway`ï¼ˆä½¿ç”¨ `bot.molt.<profile>`ï¼›æ—§ç‰ˆ `com..*` ä»ç„¶æœ‰æ•ˆï¼‰
- Linux systemd ç”¨æˆ·æœåŠ¡ï¼š`systemctl --user restart -gateway[-<profile>].service`
- Windowsï¼ˆWSL2ï¼‰ï¼š`systemctl --user restart -gateway[-<profile>].service`
  - `launchctl`/`systemctl` ä»…åœ¨æœåŠ¡å·²å®‰è£…æ—¶æœ‰æ•ˆï¼›å¦åˆ™è¿è¡Œ ` gateway install`ã€‚
...
æç¤ºï¼šè¦æŸ¥çœ‹å½“å‰å‘å¸ƒçš„ç‰ˆæœ¬ï¼Œè¿è¡Œ `npm view  version`ã€‚
...
- å†æ¬¡è¿è¡Œ ` doctor` å¹¶ä»”ç»†é˜…è¯»è¾“å‡ºï¼ˆå®ƒé€šå¸¸ä¼šå‘Šè¯‰ä½ ä¿®å¤æ–¹æ³•ï¼‰ã€‚
