...
æœ€å¿«èŠå¤©ï¼šæ‰“å¼€ Control UIï¼ˆæ— éœ€æ¸ é“è®¾ç½®ï¼‰ã€‚è¿è¡Œ ` dashboard` å¹¶åœ¨æµè§ˆå™¨ä¸­èŠå¤©ï¼Œæˆ–åœ¨ Gateway ç½‘å…³ä¸»æœºä¸Šæ‰“å¼€ `http://127.0.0.1:18789/`ã€‚æ–‡æ¡£ï¼š[Dashboard](/web/dashboard) å’Œ [Control UI](/web/control-ui)ã€‚

æŽ¨èè·¯å¾„ï¼šä½¿ç”¨ **CLI æ–°æ‰‹å¼•å¯¼å‘å¯¼**ï¼ˆ` onboard`ï¼‰ã€‚å®ƒè®¾ç½®ï¼š
...
å¦‚æžœä½ æƒ³è¦æ›´æ·±å…¥çš„å‚è€ƒé¡µé¢ï¼Œè·³è½¬åˆ°ï¼š[å‘å¯¼](/start/wizard)ã€[è®¾ç½®](/start/setup)ã€[é…å¯¹](/start/pairing)ã€[å®‰å…¨](/gateway/security)ã€‚
...
- **æŽ¨èï¼š**Brave Search API å¯†é’¥ç”¨äºŽç½‘é¡µæœç´¢ã€‚æœ€ç®€å•çš„æ–¹å¼ï¼š` configure --section web`ï¼ˆå­˜å‚¨ `tools.web.search.apiKey`ï¼‰ã€‚å‚è§ [Web å·¥å…·](/tools/web)ã€‚
...
- OAuth å‡­è¯ï¼ˆæ—§ç‰ˆå¯¼å…¥ï¼‰ï¼š`~/./credentials/oauth.json`
- è®¤è¯é…ç½®æ–‡ä»¶ï¼ˆOAuth + API å¯†é’¥ï¼‰ï¼š`~/./agents/<agentId>/agent/auth-profiles.json`
...
é…å¯¹æ–‡æ¡£ï¼š[é…å¯¹](/start/pairing)

## ä»Žæºä»£ç ï¼ˆå¼€å‘ï¼‰

å¦‚æžœä½ æ­£åœ¨å¼€å‘  æœ¬èº«ï¼Œä»Žæºä»£ç è¿è¡Œï¼š
...
å¦‚æžœä½ è¿˜æ²¡æœ‰å…¨å±€å®‰è£…ï¼Œä»Žä»“åº“é€šè¿‡ `pnpm  ...` è¿è¡Œæ–°æ‰‹å¼•å¯¼æ­¥éª¤ã€‚`pnpm build` ä¹Ÿä¼šæ‰“åŒ… A2UI èµ„æºï¼›å¦‚æžœä½ åªéœ€è¦è¿è¡Œé‚£ä¸ªæ­¥éª¤ï¼Œä½¿ç”¨ `pnpm canvas:a2ui:bundle`ã€‚

Gateway ç½‘å…³ï¼ˆä»Žæ­¤ä»“åº“ï¼‰ï¼š

```bash
node .mjs gateway --port 18789 --verbose
```
...
å¦‚æžœ ` health` æ˜¾ç¤º"æœªé…ç½®è®¤è¯"ï¼Œå›žåˆ°å‘å¯¼è®¾ç½® OAuth/å¯†é’¥è®¤è¯â€”â€”æ²¡æœ‰å®ƒæ™ºèƒ½ä½“å°†æ— æ³•å“åº”ã€‚

æç¤ºï¼š` status --all` æ˜¯æœ€ä½³çš„å¯ç²˜è´´ã€åªè¯»è°ƒè¯•æŠ¥å‘Šã€‚
å¥åº·æŽ¢æµ‹ï¼š` health`ï¼ˆæˆ– ` status --deep`ï¼‰å‘è¿è¡Œä¸­çš„ Gateway ç½‘å…³è¯·æ±‚å¥åº·å¿«ç…§ã€‚
