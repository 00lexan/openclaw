...
macOS ä¹Ÿå¯ä»¥åœ¨**èŠ‚ç‚¹æ¨¡å¼**ä¸‹è¿è¡Œï¼šèœå•æ åº”ç”¨è¿žæŽ¥åˆ° Gateway ç½‘å…³çš„ WS æœåŠ¡å™¨ï¼Œå¹¶å°†å…¶æœ¬åœ° canvas/camera å‘½ä»¤ä½œä¸ºèŠ‚ç‚¹æš´éœ²ï¼ˆå› æ­¤ ` nodes â€¦` å¯ä»¥é’ˆå¯¹è¿™å° Mac å·¥ä½œï¼‰ã€‚
...
```bash
 ...
 nodes status
 nodes describe --node <idOrNameOrIp>
```

æ³¨æ„äº‹é¡¹ï¼š

- å½“èŠ‚ç‚¹çš„è®¾å¤‡é…å¯¹è§’è‰²åŒ…å« `node` æ—¶ï¼Œ`nodes status` å°†èŠ‚ç‚¹æ ‡è®°ä¸º**å·²é…å¯¹**ã€‚
- `node.pair.*`ï¼ˆCLIï¼š` nodes pending/approve/reject`ï¼‰æ˜¯ä¸€ä¸ªå•ç‹¬çš„ Gateway ç½‘å…³æ‹¥æœ‰çš„
...
- **æ‰¹å‡†**ï¼šé€šè¿‡ `~/./exec-approvals.json` åœ¨èŠ‚ç‚¹ä¸»æœºä¸Šæ‰§è¡Œã€‚
...
```bash
 node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
export _GATEWAY_TOKEN="<gateway-token>"
 node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

æ³¨æ„äº‹é¡¹ï¼š

- ä»¤ç‰Œæ˜¯ Gateway ç½‘å…³é…ç½®ä¸­çš„ `gateway.auth.token`ï¼ˆGateway ç½‘å…³ä¸»æœºä¸Šçš„ `~/./.json`ï¼‰ã€‚
- ` node run` è¯»å– `_GATEWAY_TOKEN` è¿›è¡Œè®¤è¯ã€‚

### å¯åŠ¨èŠ‚ç‚¹ä¸»æœºï¼ˆæœåŠ¡ï¼‰

```bash
 node install --host <gateway-host> --port 18789 --display-name "Build Node"
 node restart
```
...
- åœ¨ ` node run` / ` node install` ä¸Šä½¿ç”¨ `--display-name`ï¼ˆæŒä¹…åŒ–åœ¨èŠ‚ç‚¹ä¸Šçš„ `~/./node.json` ä¸­ï¼‰ã€‚
- ` nodes rename --node <id|name|ip> --name "Build Node"`ï¼ˆGateway ç½‘å…³è¦†ç›–ï¼‰ã€‚
...
```bash
 approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
 approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

æ‰¹å‡†å­˜å‚¨åœ¨èŠ‚ç‚¹ä¸»æœºçš„ `~/./exec-approvals.json` ä¸­ã€‚
...
```bash
 ...
 config set tools.exec.node "<id-or-name>"
```
...
```bash
 nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
 nodes canvas snapshot --node <idOrNameOrIp> --format png
 nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas æŽ§åˆ¶

```bash
 nodes canvas present --node <idOrNameOrIp> --target https://example.com
 nodes canvas hide --node <idOrNameOrIp>
 nodes canvas navigate https://example.com --node <idOrNameOrIp>
 nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
 nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
 nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
 nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
 nodes camera list --node <idOrNameOrIp>
 nodes camera snap --node <idOrNameOrIp>            # é»˜è®¤ï¼šä¸¤ä¸ªæœå‘ï¼ˆ2 ä¸ª MEDIA è¡Œï¼‰
 nodes camera snap --node <idOrNameOrIp> --facing front
```

è§†é¢‘ç‰‡æ®µï¼ˆ`mp4`ï¼‰ï¼š

```bash
 nodes camera clip --node <idOrNameOrIp> --duration 10s
 nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
 nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
 nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
 nodes location get --node <idOrNameOrIp>
 nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
 nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from "}'
```
...
```bash
 nodes run --node <idOrNameOrIp> -- echo "Hello from mac node"
 nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
```
...
- åœ¨æ— å¤´èŠ‚ç‚¹ä¸»æœºä¸Šï¼Œ`system.run` å— exec æ‰¹å‡†é™åˆ¶ï¼ˆ`~/./exec-approvals.json`ï¼‰ã€‚
...
```bash
 config set tools.exec.node "node-id-or-name"
```
...
```bash
 config unset tools.exec.node
 config unset agents.list[0].tools.exec.node
```
...
## æ— å¤´èŠ‚ç‚¹ä¸»æœºï¼ˆè·¨å¹³å°ï¼‰
 ...
 å¯ä»¥è¿è¡Œ**æ— å¤´èŠ‚ç‚¹ä¸»æœº**ï¼ˆæ—  UIï¼‰ï¼Œå®ƒè¿žæŽ¥åˆ° Gateway ç½‘å…³
...
- èŠ‚ç‚¹ä¸»æœºå°†å…¶èŠ‚ç‚¹ idã€ä»¤ç‰Œã€æ˜¾ç¤ºåç§°å’Œ Gateway ç½‘å…³è¿žæŽ¥ä¿¡æ¯å­˜å‚¨åœ¨ `~/./node.json` ä¸­ã€‚
- Exec æ‰¹å‡†é€šè¿‡ `~/./exec-approvals.json` åœ¨æœ¬åœ°æ‰§è¡Œ
  ï¼ˆå‚è§ [Exec æ‰¹å‡†](/tools/exec-approvals)ï¼‰ã€‚
- åœ¨ macOS ä¸Šï¼Œå½“é…å¥—åº”ç”¨ exec ä¸»æœºå¯è¾¾æ—¶ï¼Œæ— å¤´èŠ‚ç‚¹ä¸»æœºä¼˜å…ˆä½¿ç”¨å®ƒï¼Œ
  å¦‚æžœåº”ç”¨ä¸å¯ç”¨åˆ™å›žé€€åˆ°æœ¬åœ°æ‰§è¡Œã€‚è®¾ç½® `_NODE_EXEC_HOST=app` è¦æ±‚
  ä½¿ç”¨åº”ç”¨ï¼Œæˆ–è®¾ç½® `_NODE_EXEC_FALLBACK=0` ç¦ç”¨å›žé€€ã€‚
- å½“ Gateway ç½‘å…³ WS ä½¿ç”¨ TLS æ—¶ï¼Œæ·»åŠ  `--tls` / `--tls-fingerprint`ã€‚

## Mac èŠ‚ç‚¹æ¨¡å¼

- macOS èœå•æ åº”ç”¨ä½œä¸ºèŠ‚ç‚¹è¿žæŽ¥åˆ° Gateway ç½‘å…³ WS æœåŠ¡å™¨ï¼ˆå› æ­¤ ` nodes â€¦` å¯ä»¥é’ˆå¯¹è¿™å° Mac å·¥ä½œï¼‰ã€‚
