...
# æµè§ˆå™¨ï¼ˆ æ‰˜ç®¡ï¼‰
 ...
 å¯ä»¥è¿è¡Œä¸€ä¸ªç”±æ™ºèƒ½ä½“æŽ§åˆ¶çš„**ä¸“ç”¨ Chrome/Brave/Edge/Chromium é…ç½®æ–‡ä»¶**ã€‚
...
- `` é…ç½®æ–‡ä»¶**ä¸ä¼š**è§¦åŠä½ çš„ä¸ªäººæµè§ˆå™¨é…ç½®æ–‡ä»¶ã€‚
- æ™ºèƒ½ä½“å¯ä»¥åœ¨å®‰å…¨çš„é€šé“ä¸­**æ‰“å¼€æ ‡ç­¾é¡µã€è¯»å–é¡µé¢ã€ç‚¹å‡»å’Œè¾“å…¥**ã€‚
- é»˜è®¤çš„ `chrome` é…ç½®æ–‡ä»¶é€šè¿‡æ‰©å±•ä¸­ç»§ä½¿ç”¨**ç³»ç»Ÿé»˜è®¤çš„ Chromium æµè§ˆå™¨**ï¼›åˆ‡æ¢åˆ° `` å¯ä½¿ç”¨éš”ç¦»çš„æ‰˜ç®¡æµè§ˆå™¨ã€‚

## åŠŸèƒ½æ¦‚è§ˆ

- ä¸€ä¸ªåä¸º **** çš„ç‹¬ç«‹æµè§ˆå™¨é…ç½®æ–‡ä»¶ï¼ˆé»˜è®¤æ©™è‰²ä¸»é¢˜ï¼‰ã€‚
- ç¡®å®šæ€§æ ‡ç­¾é¡µæŽ§åˆ¶ï¼ˆåˆ—å‡º/æ‰“å¼€/èšç„¦/å…³é—­ï¼‰ã€‚
- æ™ºèƒ½ä½“æ“ä½œï¼ˆç‚¹å‡»/è¾“å…¥/æ‹–åŠ¨/é€‰æ‹©ï¼‰ã€å¿«ç…§ã€æˆªå›¾ã€PDFã€‚
- å¯é€‰çš„å¤šé…ç½®æ–‡ä»¶æ”¯æŒï¼ˆ``ã€`work`ã€`remote` ç­‰ï¼‰ã€‚
...
```bash
 browser --browser-profile  status
...
## é…ç½®æ–‡ä»¶ï¼š`` ä¸Ž `chrome`

- ``ï¼šæ‰˜ç®¡çš„éš”ç¦»æµè§ˆå™¨ï¼ˆæ— éœ€æ‰©å±•ï¼‰ã€‚
- `chrome`ï¼šåˆ°ä½ **ç³»ç»Ÿæµè§ˆå™¨**çš„æ‰©å±•ä¸­ç»§ï¼ˆéœ€è¦å°†  æ‰©å±•é™„åŠ åˆ°æ ‡ç­¾é¡µï¼‰ã€‚

å¦‚æžœä½ å¸Œæœ›é»˜è®¤ä½¿ç”¨æ‰˜ç®¡æ¨¡å¼ï¼Œè¯·è®¾ç½® `browser.defaultProfile: ""`ã€‚

## é…ç½®

æµè§ˆå™¨è®¾ç½®ä½äºŽ `~/./.json`ã€‚
...
- å¦‚æžœä½ è¦†ç›–äº† Gateway ç½‘å…³ç«¯å£ï¼ˆ`gateway.port` æˆ– `_GATEWAY_PORT`ï¼‰ï¼Œæ´¾ç”Ÿçš„æµè§ˆå™¨ç«¯å£ä¼šç›¸åº”è°ƒæ•´ä»¥ä¿æŒåœ¨åŒä¸€"ç³»åˆ—"ä¸­ã€‚
...
- é»˜è®¤é…ç½®æ–‡ä»¶æ˜¯ `chrome`ï¼ˆæ‰©å±•ä¸­ç»§ï¼‰ã€‚ä½¿ç”¨ `defaultProfile: ""` æ¥ä½¿ç”¨æ‰˜ç®¡æµè§ˆå™¨ã€‚
...
- æœ¬åœ° `` é…ç½®æ–‡ä»¶ä¼šè‡ªåŠ¨åˆ†é… `cdpPort`/`cdpUrl` â€” ä»…ä¸ºè¿œç¨‹ CDP è®¾ç½®è¿™äº›ã€‚

## ä½¿ç”¨ Braveï¼ˆæˆ–å…¶ä»–åŸºäºŽ Chromium çš„æµè§ˆå™¨ï¼‰

å¦‚æžœä½ çš„**ç³»ç»Ÿé»˜è®¤**æµè§ˆå™¨æ˜¯åŸºäºŽ Chromium çš„ï¼ˆChrome/Brave/Edge ç­‰ï¼‰ï¼Œ ä¼šè‡ªåŠ¨ä½¿ç”¨å®ƒã€‚è®¾ç½® `browser.executablePath` å¯è¦†ç›–è‡ªåŠ¨æ£€æµ‹ï¼š
...
- **è¿œç¨‹ CDPï¼š** è®¾ç½® `browser.profiles.<name>.cdpUrl`ï¼ˆæˆ– `browser.cdpUrl`ï¼‰ä»¥é™„åŠ åˆ°è¿œç¨‹çš„åŸºäºŽ Chromium çš„æµè§ˆå™¨ã€‚åœ¨è¿™ç§æƒ…å†µä¸‹ï¼Œ ä¸ä¼šå¯åŠ¨æœ¬åœ°æµè§ˆå™¨ã€‚
...
- HTTP Basic è®¤è¯ï¼ˆä¾‹å¦‚ `https://user:pass@provider.example`ï¼‰
 ...
 åœ¨è°ƒç”¨ `/json/*` ç«¯ç‚¹å’Œè¿žæŽ¥ CDP WebSocket æ—¶ä¼šä¿ç•™è®¤è¯ä¿¡æ¯ã€‚å»ºè®®ä½¿ç”¨çŽ¯å¢ƒå˜é‡æˆ–å¯†é’¥ç®¡ç†å™¨å­˜å‚¨ä»¤ç‰Œï¼Œè€Œä¸æ˜¯å°†å…¶æäº¤åˆ°é…ç½®æ–‡ä»¶ä¸­ã€‚

## èŠ‚ç‚¹æµè§ˆå™¨ä»£ç†ï¼ˆé›¶é…ç½®é»˜è®¤ï¼‰

å¦‚æžœä½ åœ¨æœ‰æµè§ˆå™¨çš„æœºå™¨ä¸Šè¿è¡Œ**èŠ‚ç‚¹ä¸»æœº**ï¼Œ å¯ä»¥è‡ªåŠ¨å°†æµè§ˆå™¨å·¥å…·è°ƒç”¨è·¯ç”±åˆ°è¯¥èŠ‚ç‚¹ï¼Œæ— éœ€ä»»ä½•é¢å¤–çš„æµè§ˆå™¨é…ç½®ã€‚è¿™æ˜¯è¿œç¨‹ Gateway ç½‘å…³çš„é»˜è®¤è·¯å¾„ã€‚
...
[Browserless](https://browserless.io) æ˜¯ä¸€ä¸ªæ‰˜ç®¡çš„ Chromium æœåŠ¡ï¼Œé€šè¿‡ HTTPS æš´éœ² CDP ç«¯ç‚¹ã€‚ä½ å¯ä»¥å°†  æµè§ˆå™¨é…ç½®æ–‡ä»¶æŒ‡å‘ Browserless åŒºåŸŸç«¯ç‚¹ï¼Œå¹¶ä½¿ç”¨ä½ çš„ API å¯†é’¥è¿›è¡Œè®¤è¯ã€‚
...
## é…ç½®æ–‡ä»¶ï¼ˆå¤šæµè§ˆå™¨ï¼‰
 ...
 æ”¯æŒå¤šä¸ªå‘½åé…ç½®æ–‡ä»¶ï¼ˆè·¯ç”±é…ç½®ï¼‰ã€‚é…ç½®æ–‡ä»¶å¯ä»¥æ˜¯ï¼š

- ** æ‰˜ç®¡**ï¼šå…·æœ‰ç‹¬ç«‹ç”¨æˆ·æ•°æ®ç›®å½•å’Œ CDP ç«¯å£çš„ä¸“ç”¨åŸºäºŽ Chromium çš„æµè§ˆå™¨å®žä¾‹
...
- å¦‚æžœç¼ºå°‘ `` é…ç½®æ–‡ä»¶ï¼Œä¼šè‡ªåŠ¨åˆ›å»ºã€‚
...
## Chrome æ‰©å±•ä¸­ç»§ï¼ˆä½¿ç”¨ä½ çŽ°æœ‰çš„ Chromeï¼‰
 ...
 è¿˜å¯ä»¥é€šè¿‡æœ¬åœ° CDP ä¸­ç»§ + Chrome æ‰©å±•é©±åŠ¨**ä½ çŽ°æœ‰çš„ Chrome æ ‡ç­¾é¡µ**ï¼ˆæ— éœ€å•ç‹¬çš„""Chrome å®žä¾‹ï¼‰ã€‚
...
- ä½ ç‚¹å‡»æ ‡ç­¾é¡µä¸Šçš„ ** Browser Relay** æ‰©å±•å›¾æ ‡æ¥é™„åŠ ï¼ˆå®ƒä¸ä¼šè‡ªåŠ¨é™„åŠ ï¼‰ã€‚
...
- "åŠ è½½å·²è§£åŽ‹çš„æ‰©å±•ç¨‹åº" â†’ é€‰æ‹© ` browser extension path` æ‰“å°çš„ç›®å½•
...
æœ¬åœ°å¯åŠ¨æ—¶ï¼Œ é€‰æ‹©ç¬¬ä¸€ä¸ªå¯ç”¨çš„ï¼š
...
æŸäº›åŠŸèƒ½ï¼ˆnavigate/act/AI å¿«ç…§/è§’è‰²å¿«ç…§ã€å…ƒç´ æˆªå›¾ã€PDFï¼‰éœ€è¦ Playwrightã€‚å¦‚æžœæœªå®‰è£… Playwrightï¼Œè¿™äº›ç«¯ç‚¹ä¼šè¿”å›žæ˜Žç¡®çš„ 501 é”™è¯¯ã€‚ARIA å¿«ç…§å’ŒåŸºæœ¬æˆªå›¾å¯¹äºŽ  æ‰˜ç®¡çš„ Chrome ä»ç„¶æœ‰æ•ˆã€‚å¯¹äºŽ Chrome æ‰©å±•ä¸­ç»§é©±åŠ¨ç¨‹åºï¼ŒARIA å¿«ç…§å’Œæˆªå›¾éœ€è¦ Playwrightã€‚

å¦‚æžœä½ çœ‹åˆ° `Playwright is not available in this gateway build`ï¼Œè¯·å®‰è£…å®Œæ•´çš„ Playwright åŒ…ï¼ˆä¸æ˜¯ `playwright-core`ï¼‰å¹¶é‡å¯ Gateway ç½‘å…³ï¼Œæˆ–è€…é‡æ–°å®‰è£…å¸¦æµè§ˆå™¨æ”¯æŒçš„ ã€‚
...
è¦æŒä¹…åŒ–æµè§ˆå™¨ä¸‹è½½ï¼Œè®¾ç½® `PLAYWRIGHT_BROWSERS_PATH`ï¼ˆä¾‹å¦‚ `/home/node/.cache/ms-playwright`ï¼‰å¹¶ç¡®ä¿ `/home/node` é€šè¿‡ `_HOME_VOLUME` æˆ–ç»‘å®šæŒ‚è½½æŒä¹…åŒ–ã€‚å‚è§ [Docker](/install/docker)ã€‚
...
- ` browser screenshot --ref e12`
- ` browser snapshot`
- ` browser snapshot --format aria --limit 200`
- ` browser snapshot --interactive --compact --depth 6`
- ` browser snapshot --efficient`
- ` browser snapshot --labels`
- ` browser snapshot --selector "#main" --interactive`
- ` browser snapshot --frame "iframe#main" --interactive`
- ` browser console --level error`
- ` browser errors --clear`
- ` browser requests --filter api --clear`
- ` browser pdf`
- ` browser responsebody "**/api" --max-chars 5000`
...
- ` browser click e12 --double`
- ` browser type 23 "hello" --submit`
- ` browser press Enter`
- ` browser hover 44`
- ` browser scrollintoview e12`
- ` browser drag 10 11`
- ` browser select 9 OptionA OptionB`
- ` browser download e12 /tmp/report.pdf`
- ` browser waitfordownload /tmp/report.pdf`
- ` browser upload /tmp/file.pdf`
- ` browser fill --fields '[{"ref":"1","type":"text","value":"Ada"}]'`
- ` browser dialog --accept`
- ` browser wait --text "Done"`
- ` browser wait "#main" --url "**/dash" --load networkidle --fn "window.ready===true"`
...
- ` browser set headers --json '{"X-Debug":"1"}'`
- ` browser set credentials user pass`
- ` browser set credentials --clear`
- ` browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- ` browser set geo --clear`
- ` browser set media dark`
- ` browser set timezone America/New_York`
- ` browser set locale en-US`
...
- `snapshot`ï¼š
  ...
  - é…ç½®é»˜è®¤å€¼ï¼ˆä»…é™å·¥å…·/CLIï¼‰ï¼šè®¾ç½® `browser.snapshotDefaults.mode: "efficient"` ä»¥åœ¨è°ƒç”¨è€…æœªä¼ é€’æ¨¡å¼æ—¶ä½¿ç”¨é«˜æ•ˆå¿«ç…§ï¼ˆå‚è§ [Gateway ç½‘å…³é…ç½®](/gateway/configuration#browser--managed-browser)ï¼‰ã€‚
...
## å¿«ç…§å’Œ ref
 ...
 æ”¯æŒä¸¤ç§"å¿«ç…§"é£Žæ ¼ï¼š

- **AI å¿«ç…§ï¼ˆæ•°å­— refï¼‰**ï¼š` browser snapshot`ï¼ˆé»˜è®¤ï¼›`--format ai`ï¼‰
  - è¾“å‡ºï¼šåŒ…å«æ•°å­— ref çš„æ–‡æœ¬å¿«ç…§ã€‚
  - æ“ä½œï¼š` browser click 12`ã€` browser type 23 "hello"`ã€‚
  - å†…éƒ¨é€šè¿‡ Playwright çš„ `aria-ref` è§£æž refã€‚

- **è§’è‰²å¿«ç…§ï¼ˆè§’è‰² ref å¦‚ `e12`ï¼‰**ï¼š` browser snapshot --interactive`ï¼ˆæˆ– `--compact`ã€`--depth`ã€`--selector`ã€`--frame`ï¼‰
  - è¾“å‡ºï¼šå¸¦æœ‰ `[ref=e12]`ï¼ˆå’Œå¯é€‰çš„ `[nth=1]`ï¼‰çš„åŸºäºŽè§’è‰²çš„åˆ—è¡¨/æ ‘ã€‚
  - æ“ä½œï¼š` browser click e12`ã€` browser highlight e12`ã€‚
...
1. ` browser snapshot --interactive`
2. ä½¿ç”¨ `click <ref>` / `type <ref>`ï¼ˆåœ¨äº¤äº’æ¨¡å¼ä¸‹ä¼˜å…ˆä½¿ç”¨è§’è‰² refï¼‰
3. å¦‚æžœä»ç„¶å¤±è´¥ï¼š` browser highlight <ref>` æŸ¥çœ‹ Playwright å®šä½çš„ç›®æ ‡
...
5. æ·±åº¦è°ƒè¯•ï¼šå½•åˆ¶ traceï¼š
   - ` browser trace start`
   - é‡çŽ°é—®é¢˜
   - ` browser trace stop`ï¼ˆæ‰“å° `TRACE:<path>`ï¼‰
...
```bash
 browser status --json
 browser snapshot --interactive --json
 browser requests --filter api --json
...
-  æµè§ˆå™¨é…ç½®æ–‡ä»¶å¯èƒ½åŒ…å«å·²ç™»å½•çš„ä¼šè¯ï¼›è¯·å°†å…¶è§†ä¸ºæ•æ„Ÿä¿¡æ¯ã€‚
- `browser act kind=evaluate` / ` browser evaluate` å’Œ `wait --fn` åœ¨é¡µé¢ä¸Šä¸‹æ–‡ä¸­æ‰§è¡Œä»»æ„ JavaScriptã€‚æç¤ºæ³¨å…¥å¯èƒ½ä¼šæ“çºµå®ƒã€‚å¦‚æžœä¸éœ€è¦ï¼Œè¯·ä½¿ç”¨ `browser.evaluateEnabled=false` ç¦ç”¨å®ƒã€‚
...
- `browser` æŽ¥å—ï¼š
  - `profile` æ¥é€‰æ‹©å‘½åçš„æµè§ˆå™¨é…ç½®æ–‡ä»¶ï¼ˆã€chrome æˆ–è¿œç¨‹ CDPï¼‰ã€‚
