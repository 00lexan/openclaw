...
æœ‰å…³è°ƒè¯•"ä¸ºä»€ä¹ˆè¿™è¢«é˜»æ­¢äº†ï¼Ÿ"ï¼Œè¯·å‚è§[æ²™ç®± vs å·¥å…·ç­–ç•¥ vs ææƒ](/gateway/sandbox-vs-tool-policy-vs-elevated) å’Œ ` sandbox explain`ã€‚
...
æ—§ç‰ˆ `agent.*` é…ç½®ç”± ` doctor` è¿ç§»ï¼›ä»ŠåŽè¯·ä¼˜å…ˆä½¿ç”¨ `agents.defaults` + `agents.list`ã€‚
