...
3. é…ç½®  çš„ `channels.imessage.cliPath` å’Œ `channels.imessage.dbPath`ã€‚
...
- å·²ç™»å½•"ä¿¡æ¯"çš„ macOSã€‚
-  + `imsg` çš„å®Œå…¨ç£ç›˜è®¿é—®æƒé™ï¼ˆè®¿é—®"ä¿¡æ¯"æ•°æ®åº“ï¼‰ã€‚
...
2. åˆ›å»º macOS ç”¨æˆ·ï¼ˆä¾‹å¦‚ï¼š`home`ï¼‰å¹¶ç™»å½•ã€‚
...
å¦‚æžœä½ æƒ³åœ¨å¦ä¸€å° Mac ä¸Šä½¿ç”¨ iMessageï¼Œè¯·å°† `channels.imessage.cliPath` è®¾ç½®ä¸ºé€šè¿‡ SSH åœ¨è¿œç¨‹ macOS ä¸»æœºä¸Šè¿è¡Œ `imsg` çš„åŒ…è£…è„šæœ¬ã€‚ åªéœ€è¦ stdioã€‚
...
**è¿œç¨‹é™„ä»¶ï¼š** å½“ `cliPath` é€šè¿‡ SSH æŒ‡å‘è¿œç¨‹ä¸»æœºæ—¶ï¼Œ"ä¿¡æ¯"æ•°æ®åº“ä¸­çš„é™„ä»¶è·¯å¾„å¼•ç”¨çš„æ˜¯è¿œç¨‹æœºå™¨ä¸Šçš„æ–‡ä»¶ã€‚ å¯ä»¥é€šè¿‡è®¾ç½® `channels.imessage.remoteHost` è‡ªåŠ¨é€šè¿‡ SCP èŽ·å–è¿™äº›æ–‡ä»¶ï¼š
...
å¦‚æžœæœªè®¾ç½® `remoteHost`ï¼Œ ä¼šå°è¯•é€šè¿‡è§£æžåŒ…è£…è„šæœ¬ä¸­çš„ SSH å‘½ä»¤è‡ªåŠ¨æ£€æµ‹ã€‚å»ºè®®æ˜¾å¼é…ç½®ä»¥æé«˜å¯é æ€§ã€‚
...
ç¤ºä¾‹åŒ…è£…è„šæœ¬ï¼ˆ`~/./scripts/imsg-ssh`ï¼‰ï¼š
...
å¤šè´¦æˆ·æ”¯æŒï¼šä½¿ç”¨ `channels.imessage.accounts` é…ç½®æ¯ä¸ªè´¦æˆ·åŠå¯é€‰çš„ `name`ã€‚å‚è§ [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) äº†è§£å…±äº«æ¨¡å¼ã€‚ä¸è¦æäº¤ `~/./.json`ï¼ˆå®ƒé€šå¸¸åŒ…å«ä»¤ç‰Œï¼‰ã€‚
...
- é…å¯¹æ˜¯ iMessage ç§ä¿¡çš„é»˜è®¤ä»¤ç‰Œäº¤æ¢æ–¹å¼ã€‚è¯¦æƒ…ï¼š[é…å¯¹](/start/pairing)
...
å¦‚æžœä½ åœ¨ `channels.imessage.groups` ä¸‹æ˜¾å¼é…ç½®äº† `chat_id`ï¼Œ ä¼šå°†è¯¥ä¼šè¯è§†ä¸º"ç¾¤ç»„"ç”¨äºŽï¼š
