---
summary: "Frequently asked questions about  setup, configuration, and usage"
title: "FAQ"
---
...
- [Quick start and first-run setup](#quick-start-and-firstrun-setup)
  ...
  - [What's the recommended way to install and set up ?](#whats-the-recommended-way-to-install-and-set-up-)
  ...
  - [I can't access docs..ai (SSL error). What now?](#i-cant-access-docsai-ssl-error-what-now)
  - [What's the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
  - [How do I install the beta version, and what's the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or  not recognized](#windows-install-says-git-not-found-or--not-recognized)
  - [The docs didn't answer my question - how do I get a better answer?](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [How do I install  on Linux?](#how-do-i-install--on-linux)
  - [How do I install  on a VPS?](#how-do-i-install--on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask  to update itself?](#can-i-ask--to-update-itself)
  ...
  - [If I buy a Mac mini to run , can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run--can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different  instances?](#can-multiple-people-use-one-whatsapp-number-with-different--instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What's the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run  on a dedicated machine?](#how-important-is-it-to-run--on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run  in a VM and what are the requirements](#can-i-run--in-a-vm-and-what-are-the-requirements)
- [What is ?](#what-is-)
  - [What is , in one paragraph?](#what-is--in-one-paragraph)
  - [What's the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for ](#what-are-the-top-five-everyday-use-cases-for-)
  - [Can  help with lead gen outreach ads and blogs for a SaaS](#can--help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can  run tasks on a schedule or continuously in the background?](#can--run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with  saved locally?](#is-all-data-used-with--saved-locally)
  - [Where does  store its data?](#where-does--store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What's the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall ?](#how-do-i-completely-uninstall-)
...
- [Config basics](#config-basics)
  ...
  - [Can the  browser run headless?](#can-the--browser-run-headless)
...
- [Remote gateways + nodes](#remote-gateways-nodes)
  ...
  - [Can two  instances talk to each other (local + VPS)?](#can-two--instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does  load environment variables?](#how-does--load-environment-variables)
...
- [Sessions & multiple chats](#sessions-multiple-chats)
  ...
  - [Is there a way to make a team of  instances one CEO and many agents](#is-there-a-way-to-make-a-team-of--instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset  but keep it installed?](#how-do-i-completely-reset--but-keep-it-installed)
  ...
  - [Why doesn't  reply in a group?](#why-doesnt--reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do , Flawd, and Krill use for models?](#what-do--flawd-and-krill-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does ` gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does--gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does ` gateway status` show `Config (cli)` and `Config (service)` different?](#why-does--gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run  in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run--in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart ?](#i-closed-my-terminal-on-windows-how-do-i-restart-)
  ...
  - [ELI5: ` gateway restart` vs ` gateway`](#eli5--gateway-restart-vs--gateway)
  - [What's the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media & attachments](#media-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose  to inbound DMs?](#is-it-safe-to-expose--to-inbound-dms)
...
7. **Gateway snapshot**
   ```bash
    health --json
    health --verbose   # shows the target URL + config path on errors
   ```
...
This installs  **from a git checkout**, so the agent can read the code + docs and
...
- ` models status`: checks provider auth + model availability.
- ` doctor`: validates and repairs common config/state issues.

Other useful CLI checks: ` status --all`, ` logs --follow`,
` gateway status`, ` health --verbose`.
...
If you don't have a global install yet, run it via `pnpm  onboard`.
...
- If it asks for auth, run ` dashboard` and use the tokenized link (`?token=...`).
- The token is the same value as `gateway.auth.token` (or `_GATEWAY_TOKEN`) and is stored by the UI after first load.

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run ` gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy auth (no token).
- **Tailnet bind**: run ` gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
- **SSH tunnel**: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...` from ` dashboard`.
...
2. Copy `$_STATE_DIR` (default: `~/.`) from the old machine.
3. Copy your workspace (default: `~/./workspace`).
4. Run ` doctor` and restart the Gateway service.
...
under `~/./` (for example `~/./agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does--store-its-data),
...
### I cant access docs..ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs..ai` via Xfinity
Advanced Security. Disable it or allowlist `docs..ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsai-shows-an-ssl-error-comcastxfinity).
...
### Windows install says git not found or  not recognized
...
**2)  is not recognized after install**
...
` onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run  with **API keys** (Anthropic/OpenAI/others) or with
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with ` models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use ` models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with ` models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro/Max)

Yes - via **setup-token**.  no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so  can keep replying while a provider is rate-limited.
...
### How does Codex auth work
 ...
 supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.2` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes.  fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `.json`.

Steps:

1. Enable the plugin: ` plugins enable google-gemini-cli-auth`
2. Login: ` models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no.  needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No.  runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run  can I connect it to my MacBook Pro
...
- Use ` nodes status` / ` nodes list` to see it.
...
- DM your bot, then run ` logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different  instances
...
If you run  via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the  code install. Your state
(`~/.`) and workspace (`~/./workspace`) stay untouched.
...
**-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run  on a dedicated machine
...
### What are the minimum VPS requirements and recommended OS
 ...
 is lightweight. For a basic Gateway + one chat channel:
...
### Can I run  in a VM and what are the requirements
...
### What is  in one paragraph

 is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

 is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### Can  help with lead gen outreach ads and blogs for a SaaS
...
platform policies, and review anything before it is sent. The safest pattern is to let
 draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

 is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use  when you
...
Showcase: https://.ai/showcase

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/./skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/./.json`). Precedence is `<workspace>/skills` > `~/./skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/./.json` (lowest precedence). Default precedence remains: `<workspace>/skills` â†’ `~/./skills` â†’ bundled â†’ `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which  treats as `<workspace>/skills`.
...
- Confirm cron is enabled (`cron.enabled`) and `_SKIP_CRON` is not set.
...
### Can  run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata..os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run--in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac.  can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/./skills`) to allow Linux:
   ```markdown
   ---
   name: apple-notes
   description: Manage Apple Notes via the memo CLI on macOS.
   metadata: { "": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured  workspace);  treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/./skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
- Persist `/home/node` with `_HOME_VOLUME` so caches survive.
- Bake system deps into the image with `_DOCKER_APT_PACKAGES`.
...
### How does memory work
 ...
 memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

 also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly,  auto-selects a provider when it
...
### Is all data used with  saved locally

No - **'s state is local**, but **external services still see what you send them**.
...
### Where does  store its data

Everything lives under `$_STATE_DIR` (default: `~/.`):

| Path                                                            | Purpose                                                      |
| --------------------------------------------------------------- | ------------------------------------------------------------ |
| `$_STATE_DIR/.json`                             | Main config (JSON5)                                          |
| `$_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use) |
| `$_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth + API keys)                             |
| `$_STATE_DIR/agents/<agentId>/agent/auth.json`          | Runtime auth cache (managed automatically)                   |
| `$_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)      |
| `$_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                        |
| `$_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                     |
| `$_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                 |

Legacy single-agent path: `~/./agent/*` (migrated by ` doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/./workspace`).
...
- **State dir (`~/.`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/./skills`).

Default workspace is `~/./workspace`, configurable via:
...
Do **not** commit anything under `~/.` (credentials, sessions, tokens).
...
`workspace` to the repo root. The  repo is just source code; keep the
...
### What format is the config Where is it
 ...
 reads an optional **JSON5** config from `$_CONFIG_PATH` (default: `~/./.json`):

```
$_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/./workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `_GATEWAY_TOKEN`).
...
If you **really** want open loopback, remove `gateway.auth` from your config. Doctor can generate a token for you any time: ` doctor --generate-gateway-token`.
...
key. **Recommended:** run ` configure --section web` to store it in
...
- Daemons read env vars from `~/./.env` (or the service environment).
...
- Gateway is running: ` gateway status`
- Gateway health: ` status`
- Channel health: ` channels status`
...
### Can two  instances talk to each other local VPS
...
- Restore from backup (git or a copied `~/./.json`).
- If you have no backup, re-run ` doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use ` config set` for small changes.
- Use ` configure` for interactive edits.
...
### How does  load environment variables

 reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/./.env` (aka `$_STATE_DIR/.env`)
...
1. Put the missing keys in `~/./.env` so they're picked up even when the service doesn't inherit your shell env.
...
`_LOAD_SHELL_ENV=1`, `_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

` models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means  won't load
...
### Is there a way to make a team of  instances one CEO and many agents
...
### How do I completely reset  but keep it installed
...
- If you used profiles (`--profile` / `_PROFILE`), reset each state dir (defaults are `~/.-<profile>`).
- Dev reset: ` gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`),  skips the heartbeat run to save API calls.
...
No.  runs on **your own account**, so if you're in the group,  can see it.
...
- **Disk growth:** sessions + transcripts live under `~/./agents/<agentId>/sessions/`.
...
- Use ` doctor` to spot stray workspaces and profile mismatches.
...
's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-5`). If you omit the provider,  currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- ` models set ...` (updates just model config)
- ` configure --section models` (interactive)
- edit `agents.defaults.model` in `~/./.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run ` doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do , Flawd, and Krill use for models

- ** + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-5`) - see [Anthropic](/providers/anthropic).
...
Yes.  ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
- Run ` agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so  can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/./agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/./agent/*` (migrated by ` doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/./.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use ` models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with ` models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use ` models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/./.env` on the **gateway host**.
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand),  will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix:  now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
### What are typical profile IDs
 ...
 uses provider-prefixed IDs like:
...
Yes. Config supports optional metadata for profiles and an ordering per provider (`auth.order.<provider>`). This does **not** store secrets; it maps IDs to provider/mode and sets rotation order.
 ...
 may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run ` models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
...
```
--port > _GATEWAY_PORT > gateway.port > default 18789
```
...
Use ` gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does  gateway status show Config cli and Config service different

You're editing one config file while the service is running another (often a `--profile` / `_STATE_DIR` mismatch).
...
### What does another gateway instance is already listening mean
 ...
 enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with ` gateway --port <port>`.

### How do I run  in remote mode client connects to a Gateway elsewhere
...
- ` gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- The Control UI stores the token in browser localStorage key `.control.settings.v1`.
- The UI can import `?token=...` (and/or `?password=...`) once, then strips it from the URL.

Fix:

- Fastest: ` dashboard` (prints + copies tokenized link, tries to open; shows SSH hint if headless).
- If you don't have a token yet: ` doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...`.
- Set `gateway.auth.token` (or `_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token (or refresh with a one-time `?token=...` link).
- Still stuck? Run ` status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `_CONFIG_PATH` (per-instance config)
- `_STATE_DIR` (per-instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use ` --profile <name> â€¦` per instance (auto-creates `~/.-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: ` --profile <name> gateway install`.

Profiles also suffix service names (`bot.molt.<profile>`; legacy `com..*`, `-gateway-<profile>.service`, ` Gateway (<profile>)`).
...
- macOS: `$_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/./logs/...`; profiles use `~/.-<profile>/logs/...`)
- Linux: `journalctl --user -u -gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN " Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, ` gateway --force` can reclaim the port. See [Gateway](/gateway).
...
1. Is the Gateway running? ` gateway status`
2. Is the Gateway healthy? ` status`
3. Does the UI have the right token? ` dashboard`
...
### ELI5  gateway restart vs  gateway

- ` gateway restart`: restarts the **background service** (launchd/systemd).
- ` gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use ` gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [ assistant setup](/start/) and [Agent send](/tools/agent-send).
...
### Is it safe to expose  to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: ` pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check ` pairing list <channel>` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run ` doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**.  only replies to chats it receives or to explicit sends you trigger.
...
### How do I send a Discord message from Telegram Crosscontext messaging denied
 ...
 blocks **cross-provider** messaging by default. If a tool call is bound
...
**A:** In , credentials and model selection are separate. Setting `ANTHROPIC_API_KEY` (or storing an Anthropic API key in auth profiles) enables authentication, but the actual default model is whatever you configure in `agents.defaults.model.primary` (for example, `anthropic/claude-sonnet-4-5` or `anthropic/claude-opus-4-5`). If you see `No credentials found for profile "anthropic:default"`, it means the Gateway couldn't find Anthropic credentials in the expected `auth-profiles.json` for the agent that's running.

---

Still stuck? Ask in [Discord](https://discord.com/invite/clawd) or open a [GitHub discussion](https://github.com///discussions).
