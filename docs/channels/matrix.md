...
Matrix is an open, decentralized messaging protocol.  connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: ` plugins install @/matrix`
   - From a local checkout: ` plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`:  calls the same
     login endpoint, stores the access token in `~/./credentials/matrix/credentials.json`,
...
- On first connection,  requests device verification from your other sessions.
...
`~/./matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup,  resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
