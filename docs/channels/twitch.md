...
read_when:
  - Setting up Twitch chat integration for 
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection.  connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use _TWITCH_ACCESS_TOKEN env var)
...
```bash
_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
