...
2. Link a bot account:
   - `signal-cli link -n ""` then scan the QR in Signal.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point  at it:
...
This skips auto-spawn and the startup wait inside . For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- **Typing indicators**:  sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true,  forwards read receipts for allowed DMs.
