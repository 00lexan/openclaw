...
service via the external `` CLI (no embedded runtime). This gives you
...
- The app installs a perâ€‘user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`_PROFILE`; legacy `com..*` is supported).
