...
# Peekaboo Bridge (macOS UI automation)
 ...
 can host **PeekabooBridge** as a local, permissionâ€‘aware UI automation
broker. This lets the `peekaboo` CLI drive UI automation while reusing the
macOS appâ€™s TCC permissions.

## What this is (and isnâ€™t)

- **Host**: .app can act as a PeekabooBridge host.
- **Client**: use the `peekaboo` CLI (no separate ` ui ...` surface).
- **UI**: visual overlays stay in Peekaboo.app;  is a thin broker host.
...
When enabled,  starts a local UNIX socket server. If disabled, the host
...
- The bridge validates **caller code signatures**; an allowlist of TeamIDs is
  enforced (Peekaboo host TeamID +  app TeamID).
