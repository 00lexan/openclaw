...
.app no longer bundles Node/Bun or the Gateway runtime. The macOS app
expects an **external** `` CLI install, does not spawn the Gateway as a
child process, and manages a perâ€‘user launchd service to keep the Gateway
running (or attaches to an existing local Gateway if one is already running).

## Install the CLI (required for local mode)

You need Node 22+ on the Mac, then install `` globally:
...
- `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com..*` may remain)
...
- The CLI can also install it: ` gateway install`.

Behavior:

- â€œ Activeâ€ enables/disables the LaunchAgent.
...
- launchd stdout/err: `/tmp//-gateway.log`
