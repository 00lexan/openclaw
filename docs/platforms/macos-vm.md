---
summary: "Run  in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want  isolated from your main macOS environment
...
Run  in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
5. SSH in, install , configure channels
6. Done
...
The VM runs in the background. 's daemon keeps the gateway running.
...
This is the killer feature of running on macOS. Use [BlueBubbles](https://bluebubbles.app) to add iMessage to .
...
| VM IP not showing        | Wait for VM to fully boot, run `lume get ` again                           |
| Lume command not found   | Add `~/.local/bin` to your PATH                                                    |
| WhatsApp QR not scanning | Ensure you're logged into the VM (not host) when running ` channels login` |
