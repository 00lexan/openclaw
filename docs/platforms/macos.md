---
summary: " macOS companion app (menu bar + gateway broker)"
read_when:
...
#  macOS Companion (menu bar + gateway broker)

The macOS app is the **menuâ€‘bar companion** for . It owns permissions,
...
- Installs the global CLI (``) via npm/pnpm on request (bun not recommended for the Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via ` gateway install`.
...
(or `bot.molt.<profile>` when using `--profile`/`_PROFILE`; legacy `com..*` still unloads).
...
The app registers the `://` URL scheme for local actions.
...
```bash
open '://agent?message=Hello%20from%20deep%20link'
```
...
```bash
cd apps/macos
swift run -mac connect --json
swift run -mac discover --timeout 3000 --json
```
...
Tip: compare against ` gateway discover --json` to see whether the
