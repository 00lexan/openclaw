---
summary: "Run  Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal:  Gateway running on an exe.dev VM, reachable from your laptop via: `https://<vm-name>.exe.xyz`
...
Shelley, [exe.dev](https://exe.dev)'s agent, can install  instantly with our
...
Tip: keep this VM **stateful**.  stores state under `~/./` and `~/./workspace/`.
...
## 4) Setup nginx to proxy  to port 8000
