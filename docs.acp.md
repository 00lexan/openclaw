...
This document describes how the  ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

` acp` exposes an ACP agent over stdio and forwards prompts to a running
 Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
In Zed, open the Agent panel and select â€œ ACPâ€ to start a thread.

## Execution Model

- ACP client spawns ` acp` and speaks ACP messages over stdio.
...
` acp` resolves the Gateway URL and auth from CLI flags or config:
