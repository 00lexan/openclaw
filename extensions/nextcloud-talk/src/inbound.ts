...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as Config);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as Config);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as Config);
