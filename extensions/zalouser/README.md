# @/zalouser
 ...
 extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
 channels login --channel zalouser              # Login via QR
 channels login --channel zalouser --account work
...
```bash
# Text
 message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
 message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use ` channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
 channels login --channel zalouser --account work
 message send --channel zalouser --account work --target <id> --message "Hello"
