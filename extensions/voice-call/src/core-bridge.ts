...
function resolveRoot(): string {
  ...
  const override = process.env._ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set _ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveRoot(), "dist", "extensionAPI.js");
