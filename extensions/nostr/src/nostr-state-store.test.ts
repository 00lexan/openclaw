...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env._STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "-nostr-"));
  process.env._STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env._STATE_DIR?.trim() || env._STATE_DIR?.trim();
        if (override) {
          return override;
        }
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env._STATE_DIR = previous;
    }
