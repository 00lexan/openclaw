...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("-bin-sherpa-");
      const modelDir = await makeTempDir("-sherpa-model-");
      ...
      const cfg: Config = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("-bin-whispercpp-");
      const modelDir = await makeTempDir("-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("-bin-gemini-");
      ...
      const cfg: Config = { tools: { media: { image: {} } } };
