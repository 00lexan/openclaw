...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        _GATEWAY_TOKEN: inst.gatewayToken,
        _GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          _GATEWAY_PORT: String(gwA.port),
          _GATEWAY_TOKEN: gwA.gatewayToken,
          _GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          _GATEWAY_PORT: String(gwB.port),
          _GATEWAY_TOKEN: gwB.gatewayToken,
          _GATEWAY_PASSWORD: "",
        }),
