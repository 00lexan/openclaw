...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env._STATE_DIR = path.join(base, ".");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
